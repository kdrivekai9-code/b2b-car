...
//
// 오더 상태(orders.status)는 **아직** 바꾸지 않는다 — 지금 배차 상태의 주인은 콜마너이고
// (배차·알림톡은 콜마너 그대로), 우리가 '운행시작'으로 바꿔두면 콜마너 동기화가 다시 덮거나
// 우리 값이 콜마너보다 앞서 고객 통보가 두 번 나갈 수 있다.
//
// 향후 콜마너와 동기화할 예정이다(사용자 확정 2026-09-08). 붙일 자리는 여기다 — 기록을
// 남긴 직후, 대화 한 줄을 남기는 것과 같은 순서로 콜마너 호출을 넣는다. 지금 못 넣는 이유는
// 정책이 아니라 전문이 없어서다(docs/callmaner-external-api-notes.md 4번 문의).
// 붙일 때 지켜야 할 것: ① 콜마너 호출이 실패해도 우리 기록은 남는다(기사가 다시 누르게
// 만들면 대기시간이 어긋난다), ② 실패한 건은 나중에 다시 보낼 수 있어야 한다 —
// order_trip_steps에 전송 여부 칸을 더하는 편이 재시도 큐를 따로 만드는 것보다 싸다.
//
