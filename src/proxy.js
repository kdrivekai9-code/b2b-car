...
export function proxy(req) {
  ...
  const idSuffixRoutes = [
    ...
    // 법인 설정 하위 화면. 지사와 같은 이유로 플래그 하나로 함께 켠다 — 탭 줄이 이어져 있어
    // 화면마다 따로 켜면 탭을 누를 때마다 EJS와 Next를 오간다.
    { re: /^\/groups\/(\d+)\/(daily-driver-fare-rules|premium-fare-rules|customer-notifications|dispatch-delay)$/, flag: 'NEXT_GROUP_SETTINGS_ENABLED' },
    ...
    { re: /^\/branches\/(\d+)\/(edit|payment-methods|operating-hours|order-status|photo-settings|extra-settings|callmaner|fare-rules|premium-fare-rules|premium-oneway-fare-rules|dispatch-fare-rules|dispatch-delay|customer-notifications)$/, flag: 'NEXT_BRANCH_SETTINGS_ENABLED' },
  ];
