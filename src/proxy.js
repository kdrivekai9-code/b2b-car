...
export function proxy(req) {
  ...
  const idSuffixRoutes = [
    ...
    // 지사 설정 하위 화면. 플래그 하나로 함께 켠다 — 탭 줄이 이어져 있어서 화면마다 따로
    // 켜면 탭을 누를 때마다 EJS와 Next를 오가게 되고, 그 차이가 고장으로 읽힌다.
    { re: /^\/branches\/(\d+)\/(edit|payment-methods|operating-hours|order-status|photo-settings|extra-settings|callmaner|premium-fare-rules|premium-oneway-fare-rules|dispatch-fare-rules|dispatch-delay|customer-notifications)$/, flag: 'NEXT_BRANCH_SETTINGS_ENABLED' },
  ];
