...
export default async function NewUserPage() {
  ...
  const res = await fetchExpressJson('/users/new/data.json', { proto, host, cookie: hdrs.get('cookie') });
  if (res.status === 401) redirect('/login');
  if (!res.ok) throw new Error('사용자 등록 정보를 불러오지 못했습니다 (' + res.status + ')');

  const { currentUser, branches, groups } = await res.json();
  ...
  return (
    <AppShell currentUser={currentUser} activePath="/users">
      ...
      <div className="card">
        <UserForm mode="create" user={{}} branches={branches} groups={groups} />
      </div>
    </AppShell>
  );
}
