// 정산내역 본문 — 관리자용(/groups/:id/settlement)과 고객용(/my/settlement)이 함께 쓴다.
//
// 두 화면은 같은 뷰다(EJS도 settlement.ejs 하나를 두 라우트가 렌더했다). 다른 것은 탭 줄
// 하나뿐이라, 본문을 복사해두면 금액 표시를 고칠 때 한쪽만 바뀐다 — **청구서가 갈린다.**
//
// 금액이 나가는 화면이라 계산은 전부 서버(loadSettlement)가 하고 여기서는 그리기만 한다.
import VehicleClassBadge from '../VehicleClassBadge';
import { SelectAllCheckbox, MonthPicker, IndividualPrintButton } from './SettlementActions';
