...
function cellValue(o, key) {
  switch (key) {
    ...
    // 없다). 그래서 이 칸만 보면 고객이 그 시각을 콕 집어 예약한 것처럼 읽힌다 — 배차
    // 담당자는 그 시각까지 기다려도 되는 건으로 읽게 되고, 정작 그 시각은 접수 시각이라
    // 이미 지나 있다. 확인 문구도 같은 이유로 "즉시"로 보여준다
    // (lib/intakeSummary.js, 사용자 확정 2026-08-13).
    //
