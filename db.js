...
const pool = new Pool({
  ...
  // 커넥션 수명 상한. idleTimeoutMillis만으로는 부족하다 — 관리자 화면이 7~10초마다 폴링하기
  // 때문에(상담 세션 뷰어 7초, 카드보드 10초) 커넥션이 유휴 10초에 닿기 전에 계속 재사용되고,
  // 그러면 유휴 타이머가 영원히 안 터져 소켓이 몇 시간이고 살아남는다. 오래 산 소켓일수록
  // 슬립·NAT 타임아웃으로 반쪽 끊길 확률이 높으므로, 잘 살아 있어도 5분마다 버리고 새로 맺는다.
  // 재접속은 실측 100~150ms라 5분에 한 번은 사실상 공짜다. 쿼리 도중에 끊는 것이 아니라
  // 반납 시점에 정리하므로 진행 중인 요청에는 영향이 없다.
