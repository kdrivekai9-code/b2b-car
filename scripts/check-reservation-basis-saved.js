...
// 있으면 그 시각의 예약으로 잡는다(lib/callmaner.js). 이 API로 넣은 오더는 접수상태가 늘
// 대기(5)라 곧바로 배차되지는 않지만, 대기로 머무는 동안 그 시각을 지나치고 나중에 콜마너에서
// 대기를 접수로 바꿀 때 **이미 지나간 시각**으로 처리된다(사용자 정정 2026-09-14).
//
...
// 예약이 맞다. 즉시로 표시하면 그 시각이 콜마너로 안 넘어가, 나중에 대기를 접수로 바꿀 때
// 그 구간의 출발 시각을 알 길이 없다.
