...
(async () => {
  ...
  // 접수 기준으로 보여주는 줄에는 두 시각이 다 있어야 한다. 실사용 지적(2026-09-08): 덧붙인
  // "접수일이 오늘인 주문"에 접수시간만 나오고 예약시간이 빠졌다 — 언제 수행하는 건인지
  // 알 수 없다. 프롬프트가 "접수시각도 함께"로만 적혀 있어 모델이 예약시간을 **대신**
  // 갈아치운 것으로 읽었다.
  check('접수 기준 줄에 두 시각을 다 넣으라고 지시한다',
    /접수일 기준으로 보여주는 주문에는 접수시간과 예약시간을 둘 다 넣으세요/.test(src), true);
  check('대신 쓰지 말라고 못 박는다', /예약시간 \*\*대신\*\* 쓰면 안 됩니다/.test(src), true);
  // 덧붙는 묶음에도 같은 규칙임을 명시해야 한다 — 그 자리에서 빠졌던 것이 실제 사고다.
  check('다른기준 묶음에도 같은 규칙', /"다른기준"\(접수일\) 묶음에 \*\*똑같이\*\* 적용됩니다/.test(src), true);
  check('예시에 두 시각이 함께 나온다',
    /접수시간: 2026-09-08 11:27\n    예약시간: 2026-09-09 18:30/.test(src), true);
  // 예시를 한 줄로 이어 쓰면 모델이 그 형태를 그대로 베낀다(실측: " / "로 붙여 답했다).
  check('항목을 한 줄에 하나씩 적으라고 지시한다', /\*\*항목은 한 줄에 하나씩\*\* 적으세요/.test(src), true);
