...
//
// 왜 필요한가(사용자 지시 2026-09-14, 근거는 같은 날 정정): 콜마너 오더접수 전문은
// reservation_time이 실려 있으면 그 시각의 예약 건으로 잡는다. 그런데 우리는 즉시여도
// reserved_date/reserved_time에 "지금 시각"을 채워 저장한다 — 폼의 즉시 라디오가 현재 시각을
// 넣고 칸을 잠그기 때문이다. 그래서 그 칸이 늘 채워져 나갔다.
//
// **바로 배차되고 말고의 문제가 아니다.** 이 API로 넣은 오더는 접수상태가 항상 대기(5)로
// 고정돼 들어가므로(lib/callmaner.js) 어느 쪽이든 곧바로 배차되지 않는다. 문제는 지나간
// 예약시각이다 — 대기로 머무는 동안 그 시각을 지나치고, 나중에 콜마너에서 대기를 접수로
// 바꿀 때 이미 지나간 시각으로 처리된다. 즉시 건은 애초에 특정 시각을 요청한 것이 아니므로
// 그 칸을 비우면 이 문제가 생기지 않는다.
//
...
//   · **기준을 모르면(null) 예전처럼 예약으로 보낸다.** 모르는 것을 즉시로 단정하면 고객이
//     정해둔 예약시각을 우리가 지우는 셈이 된다.
//
...
async function checkCallmanerPayload() {
  ...
  const payloadFor = (basis) => buildOrderPayload(
    ...
    // 모르는 것을 즉시로 단정하면 고객이 정해둔 예약시각을 우리가 지우는 셈이 된다.
...
async function main() {
  ...
  // 콜마너로 안 넘어가, 나중에 대기를 접수로 바꿀 때 그 구간의 출발 시각을 알 길이 없다.
  //
