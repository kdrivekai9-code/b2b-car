...
6. **고객용 위치조회 링크 — `web_url`은 옵니다(해결). 남은 것은 `receipt_url`과 소급입니다.**
   (2026-09-10 추가 / 같은 날 일부 해결) 접수 응답에 링크가 **정상적으로 들어옵니다.**
   저희가 읽고서 저장하지 않아 버리고 있었을 뿐입니다. 실측:
   ```
   [콜마너 접수] 응답 필드: reg_cslip, reg_date, web_url
   web_url = https://www.cd1.kr/cust-web/callDetail?supplierId=S-12345
             &callToken=AqGPh0U2Vj5-…-211p4&callSource=U
   ```
   이제 `orders.callmaner_web_url`에 저장하고 오더 상세에 "콜마너 위치조회 열기" 버튼으로
   띄웁니다. 남은 질문 셋입니다.
   - **이 페이지는 운행시작 이후에도 기사 위치를 보여줍니까?** 5번(`TrackingDriver`가
     운행시작 뒤 빈 좌표)의 우회로로 쓰려는 것이라, 이 페이지가 그 구간에도 위치를 보여주는지가
     핵심입니다. 보여준다면 같은 위치를 전문으로도 주실 수 있습니까?
   - **`OrderHistory` 각 행의 `receipt_url`은 왜 항상 빈 문자열입니까?** 접수건 31건 전수
     확인했습니다(2026-09-10). 같은 링크가 담기는 칸이라면 채워주실 수 있습니까 — 그러면
     아래 소급 문제가 함께 풀립니다.
   - **이미 접수된 오더의 링크를 다시 받을 방법이 있습니까?** 저희가 버리던 시절에 등록된
     건들은 링크가 없습니다(예: conf_slip=182527475). `conf_slip`으로 토큰을 재발급·조회하는
     전문이 있으면 알려주십시오. 없으면 `OrderModify` 응답에 `web_url`이 실리는지 확인
     부탁드립니다 — 저희는 수정 응답에서도 받도록 해뒀습니다.
   - 참고로 처음 들은 형태(`/cust-web/reception/?supplierId=…&callToken=…`)와 실제로 받은
     주소(`/cust-web/callDetail?…&callSource=U`)가 경로와 파라미터에서 다릅니다. 고객에게
     그대로 전달해도 되는 주소가 어느 쪽인지 확인 부탁드립니다.
   - 참고로 저희가 확인한 범위에서 위치를 주는 전문은 `TrackingDriver` 하나뿐입니다. 정의서
     목록·상세 시트에 다른 위치 인터페이스가 없고, 콜마너 MCP 도구 12개(`call.*`, `cust.get`,
     `eta.get`, `fare.get`, `place.find.*`, `branch.repno.get`)에도 위치 URL을 주는 것이 없습니다.
