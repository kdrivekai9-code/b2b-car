...
5. **`TrackingDriver`가 운행시작 뒤에는 좌표를 빈 값으로 줍니다.** (2026-09-09 추가)
   ...
   - 이 전문은 **어느 단계까지** 좌표를 주는지요? 배차~픽업 구간 전용입니까?
   - 아니면 기사 앱이 위치 전송을 멈춘 것입니까(앱 종료·권한)? 그렇다면 콜마너가 마지막으로
     받은 좌표와 그 시각을 함께 주실 수 있습니까 — 저희는 지금 우리 쪽에 마지막 좌표를 남겨
     "N분 전에 확인된 위치"로 표시하고 있습니다(orders.driver_last_*).
   - 완료된 건은 `rc=NG "배차된 정보가 없습니다"`로 명확히 갈립니다(OID2075·OID1455 실측).
     운행 중인데 좌표만 비는 위 경우와 응답이 달라서, 빈 좌표가 "위치 없음"인지
     "이 단계에서는 제공 안 함"인지 저희 쪽에서 가릴 수 없습니다.
