...
6. **고객용 위치조회 링크(`receipt_url` / `web_url`)가 저희 건에는 한 번도 오지 않습니다.**
   (2026-09-10 추가) 콜마너가 고객에게 아래 형태로 위치를 제공한다고 들었습니다.
   ```
   https://www.cd1.kr/cust-web/reception/?supplierId=S-12345&callToken=<44자 토큰>
   ```
   `supplierId`는 저희 `providerId`(`B100-12345-AP12345`)의 지사 번호로 만들 수 있지만
   **`callToken`은 불투명한 값이라 저희가 만들 수 없습니다.** 오더별로 받아야 합니다.
   - 받을 자리는 응답에 이미 둘 있어 보입니다: **`OrderHistory` 각 행의 `receipt_url`**과
     **`OrderReceipt` 응답의 `web_url`**. 그런데 실측(2026-09-10) 이력 **31건 전부
     `receipt_url=""`**이고, `web_url`도 값이 온 적이 없습니다. 이 필드들이 위 링크가 맞습니까?
     맞다면 채워지려면 어떤 설정/옵션이 필요합니까(지사 단위 설정인지, 접수 파라미터인지)?
   - 아니라면 링크를 받을 다른 경로를 안내해주실 수 있습니까?
   - 저희 쪽은 두 필드를 모두 읽어 `orders.callmaner_web_url`에 저장하도록 해뒀습니다 —
     값이 오기 시작하면 곧바로 오더 상세에 "콜마너 위치조회 열기" 버튼으로 뜹니다.
