...
챗봇 요금 문의는 세 상품을 갈라 답합니다(`lib/agentAssist.js buildFareSuggestion`). 일일기사는
**경로가 아니라 이용 시간이 입력**이라 구간 추출보다 먼저 처리합니다 — 뒤에 두면 "일일기사
8시간 얼마예요?"가 "출발지·도착지를 못 찾았다"로 떨어집니다. 시간을 안 말했으면 등록된 표를
보여주고 되묻고, 그 답을 받아 계산합니다(`awaitingHours`).

오더 등록 화면의 `/orders/fare-preview`도 `order_type`으로 표를 고릅니다. 일일기사는 거리가
아니라 **이용 시간**(`orders.daily_driver_hours`)으로 계산하므로, 그 상품일 때는 거리를
기다리지 않고 시간이 없으면 자동계산을 하지 않습니다 — 기본값을 정하면 관리자가 정한 적 없는
시간으로 청구됩니다. `reservation_hours_bracket`(within_4h/within_8h/over_8h)은 청구에 쓰지
않습니다. 그건 프리미엄 → 일일기사 자동 승격 판정용이고, over_8h를 10시간으로 환산하면
12시간을 쓴 고객도 10시간으로 청구됩니다.

남은 것: **챗봇 대화가 이용 시간을 묻지 않습니다.** 일일기사 접수 필드 목록
(`lib/intakeFields.js getDailyDriverFields`와 그 사본 `public/js/ai-intake-flow.js`)에 그 항목이
없어서, 챗봇으로 접수하면 시간이 비어 있고 요금은 수동 입력이 됩니다. 필드를 넣으려면 두 목록과
EJS 쪽 수집 처리를 함께 고쳐야 합니다.
