...
한 가지 남은 것: **오더 등록 화면**의 `/orders/fare-preview`는 `order_type`을 받아 탁송·프리미엄을
갈라 계산하지만 **일일기사는 아직 거리로 계산됩니다**(시간을 받는 `/orders/premium-fare-preview`가
있는데 부르는 화면이 없습니다). 챗봇 쪽은 이미 시간으로 계산하므로, 같은 오더를 화면에서
등록하면 다른 금액이 나옵니다. 일일기사 오더를 실제로 쓰기 시작하면 여기를 먼저 이어야 합니다.
