...
| △ | **일일기사 시간 되묻기** | 일일기사 요금은 이용 시간을 받아야 금액이 나온다. 지금은 등록된 표를 알려주고 "몇 시간 이용하실 예정인지 알려주시면"까지만 하고, 답을 받아 계산하는 턴이 없다 | 시간을 말한 고객이 금액을 못 받는다. `calculatePremiumFare`가 이미 있으니 되묻기 턴만 붙이면 된다 |
