...
| ○ | **활동 핑** | `POST /orders/ai-intake/activity` — 입력할 때마다 세션을 살려둔다 | 고객이 입력 중인데 유휴로 판정돼 봇 응대로 돌아간다 |
| ○ | **AI 연결 상태** | `/orders/ai-intake/health`(모델이 실제로 닿는지, 60초 캐시). Next는 대신 SSE 연결 여부(`streamOnline`)를 "실시간 연결중"으로 보여준다 — **뜻이 다르다** | 모델이 죽었는데 화면은 "실시간 연결중"이라 고객이 계속 입력한다 |
...
**지금은 켤 수 없다.** `○` 4개가 남아 있고, 그중 둘(요금 문의·프리미엄 수집)은 고객이 실제로
쓰는 접수 경로다. 나머지 둘(활동 핑·AI 연결 상태)은 작지만 **없으면 조용히 잘못 동작한다** —
입력 중에 세션이 끊기거나, 모델이 죽었는데 정상으로 보인다. 그 종류가 가장 나쁘다.
...
2. `○` 4개를 채운다. 활동 핑과 연결 상태는 각각 몇 줄이고, 요금 문의와 프리미엄 수집이 큰
   덩어리다.
3. `(Next)` 표식을 지운다.
