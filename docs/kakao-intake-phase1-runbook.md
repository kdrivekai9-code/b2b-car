...
- **Phase 2 능동 통보** — `lib/kakaoOrderNotify.js`, 크론 `/kakao-consult/cron/order-notifications`(매분).
  다섯 가지를 고객에게 먼저 알린다: 배차완료 · 운행완료 · 배차취소 · 오더취소 · 미배정 알림.
  상태별 사용 여부·시점·문구는 **지사관리 > 고객 통보**에서 정한다(설정이 없으면 코드 기본값).
  앞의 넷은 `order_status_history`의 상태 전이로 찾고, 미배정 알림만 "상태가 바뀌지 않은 것"이
  사건이라 접수 시각으로 찾는다.
...
- 상태 즉답(`OrderInfo`) · 기사 연락처(`WkContactSearch`) — 고객이 물었을 때 답하는 쪽.
