...
- **계기판 주행거리 완료** — 기사가 사진과 함께 적고(업로드 화면), 오더 상세에 요약이 뜨고,
  고객이 "주행거리 얼마예요"라고 물으면 봇이 답한다. 값은 사진마다 두고 주행거리는 최댓값 −
  최솟값으로 구한다(구간 릴레이는 기사가 여럿이라 오더에 출발/도착 두 칸을 두면 모자란다).
- **사진 재요청 완료** — 사진이 없는데 고객이 물으면 안내 + 상담원에게 알림(푸시)을 보내고,
  같은 대화에서 **다시 물으면** 상담원을 붙인다. 기사에게 직접 알릴 방법이 없어서(푸시는
  관리자·상담원 대상이고 기사 SMS 경로는 없다) 재촉은 상담원을 거친다. 첫 질문부터 사람을
  부르면 자동화 이득이 사라지므로 두 번째부터다.

Phase 3 범위는 이것으로 끝났다.
