...
- **수신 웹훅은 동작 중이다.** 이 문서를 처음 쓸 때는 중계서버가 어떤 URL/헤더/바디로 넘겨주는지
  ...
  흡수한다.
- **Phase 2 첫 조각(배차 통보)** — 배차 완료/배차 취소를 고객에게 먼저 알린다.
  `lib/kakaoOrderNotify.js`, 크론 `/kakao-consult/cron/order-notifications`(매분).
  배차 통보는 감지 1분 뒤에 보내고, 그 사이 배차가 풀리면 보내지 않는다.
  나머지(출발·도착 통보, 상태 즉답, 기사 연락처, 30분 미배정 알림)는 아직이다.
