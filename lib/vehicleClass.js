...
function classifyToFields(rawName, extraKeywords) {
  const flags = classifyVehicleModel(rawName, extraKeywords);
  return {
    ...flags,
    carType: carTypeOf(flags),
    fuelType: fuelTypeOf(flags),
    unclassified: isUnclassified(flags),
  };
}

// 어느 사전에도 안 걸린 이름인가.
//
// carType만 보면 알 수 없다 — "그랜저"(국산 브랜드 사전에 걸림)와 "토레스"(아무것도 안 걸림)가
// 둘 다 '국산'으로 떨어진다. 뒤쪽은 수입차든 1톤 화물이든 할증이 통째로 빠지는 상태인데
// 화면에는 정상으로 보인다. reasons가 비었다는 사실이 그 구분을 이미 갖고 있다.
//
// 국산 사전에 걸린 것은 미확인이 아니다 — 그건 "국산이라고 판정했다"는 뜻이라 정보가 있다.
function isUnclassified(flags) {
  return !flags || !Array.isArray(flags.reasons) || flags.reasons.length === 0;
}
