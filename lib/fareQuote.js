...
async function quoteFareByAddress({ originAddress, destinationAddress, branchId, groupId, vehicleType, reservedDate, reservedTime }) {
  ...
  const fare = await calculateFareWithFerry(branchId || null, route.distanceKm, {
    // 법인 요금표가 있으면 그것을 먼저 쓴다(lib/branchPolicy.js loadFareTable 주석).
    groupId: groupId || null,
    vehicleType: vehicleType || '',
    originAddress: origin.address || from,
    // 오지요금 판정용 — 지오코딩이 준 행정지명이 가장 깔끔하다("부산리"). 주소는 그것이 없을
    // 때의 폴백이다(lib/branchPolicy.js isRemoteArea).
    originDong: origin.dong || null,
    destinationAddress: destination.address || to,
    destinationDong: destination.dong || null,
