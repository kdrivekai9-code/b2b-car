...
// (도착지 연락처 없음, 최종 목적지) — 억지로 같은 parsed 스키마에 끼워 맞추면 탁송 쪽까지
// 손대게 된다.
//
// 경유지는 최대 하나만 다룬다(lib/intakeFields.js getDailyDriverFields의 waypoint 필드 —
// 탁송과 달리 항상 물어본다, 사용자 확정 규칙). 추출 스키마(lib/hybridChat.js) 자체가
// waypointAddress를 단일 필드로만 받아서, parsed.waypoints는 항상 0개 또는 1개다.
...
const { fullAddress } = require('./kakaoIntakeParser');
...
async function createPremiumOrderFromIntake({ session, account, parsed, orderType, sourceChannel }) {
  ...
  // 경유지도 좌표가 있어야 콜마너 접수(viaList)에 실제로 실린다 — 실패해도(주소 인식 불가)
  // 등록 자체는 막지 않는다(도착지와 같은 원칙, lib/kakaoIntakeService.js geocodeBoth 참고).
  const geoCache = new Map();
  const waypointList = parsed.waypoints || [];
  const [originGeo, destinationGeo, waypointGeos] = await Promise.all([
    ...
    Promise.all(waypointList.map((w) => geocodeAddress(w.address, geoCache).catch(() => null))),
  ]);
  ...
  const created = await createOrder({
    ...
    // 주소/상세주소는 나눠서 저장한다(탁송과 동일 — lib/kakaoIntakeService.js insertOrder 참고).
    ...
    // 예약 기준 — 탁송(lib/kakaoIntakeService.js insertOrder)과 같은 이유·같은 규칙이다.
    // 나뉜 건에는 붙이지 않는다. splitIntake는 나뉘지 않은 건도 parts:[{...}]로 돌려주므로
    // part.reservedDate가 비었는지로 가르면 안 되고 **나뉘었는지**(parts.length)로 본다.
    reservationBasis: reservation.immediate && split.parts.length === 1 ? 'immediate' : null,
    ...
    waypoints: waypointList.map((w, i) => {
      const g = waypointGeos[i];
      return {
        address: w.address,
...
function buildPremiumSummaryLines(parsed) {
  ...
  // 접수 확인 문구라 연락처도 같이 보여준다(탁송 접수 확인과 같은 원칙, lib/intakeSummary.js
  // 참고) — 프리미엄·일일기사는 도착지 연락처를 받지 않아(위 주석) 출발지·경유지만 붙는다.
  // 저장은 주소/상세주소 두 칸이지만 되읽어줄 때는 합쳐서 보여준다(fullAddress).
  const waypoint = (parsed.waypoints || [])[0];
  const originPart = `${fullAddress(parsed.origin)}${parsed.origin.contact ? ` (${parsed.origin.contact})` : ''}`;
  const waypointPart = waypoint && waypoint.address
    ? `${fullAddress(waypoint)}${waypoint.contact ? ` (${waypoint.contact})` : ''}`
    : null;
  lines.push(waypointPart
    ? `· ${originPart} → ${waypointPart} → ${fullAddress(parsed.destination)}`
    : `· ${originPart} → ${fullAddress(parsed.destination)}`);
...
// 등록 전 요약(buildPremiumPreviewMessage)에서 이미 전체 입력사항을 확인받았다 — 등록 후
// 통보까지 똑같은 내용을 다시 보여줄 필요는 없다("네" 한 마디로 접수한 고객에게는 접수번호만 짧게).
function buildPremiumConfirmationMessage(parsed, created, orderType) {
  return `접수했습니다. 주문번호는 (${created.oid})이며 기사님 배정시 알려드리겠습니다.`;
}
