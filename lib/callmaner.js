...
async function buildOrderPayload(order, paymentMethodName, waypoints) {
  ...
  //
  // 콜마너는 reservation_time이 실려 있으면 그 시각의 예약 건으로 잡는다. 그런데 우리는
  // 즉시여도 reserved_date/reserved_time에 "지금 시각"을 채워 저장한다 — 폼의 즉시 라디오가
  // 현재 시각을 넣고 칸을 잠그기 때문이다(src/app/orders/new/OrderForm.js). 그래서 이 칸이
  // 늘 채워져 나갔고, 즉시 배차돼야 할 건이 지금 시각 예약으로 잡혔다. 기사 앱에도 예약으로
  // 보인다.
  //
  // reservation_basis가 null인 오더는 예전 그대로 싣는다 — 이 컬럼이 생기기 전에 쌓인 건과
  // 기준을 안 보내는 접수 경로(카카오 상담톡 등)가 여기 해당한다. 모르는 것을 즉시로 단정하면
  // 반대로 예약 건이 지금 배차돼버린다. 덜 위험한 쪽으로 둔다.
