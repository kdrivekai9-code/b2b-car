...
const { vehiclePrefix } = require('./intakeMemoSplit');
...
// 적요1에는 차량번호 + 기사 전달사항 요약만 싣는다.
//
// 인수증 업로드 링크는 여기서 뺐다(2026-09-07, 사용자 지시). 그 링크는 38Byte로 이 칸의
// 100Byte 예산 중 3분의 1을 넘게 먹는데, 예산 계산(intakeMemoSplit briefBudgetBytes)은
// 차량번호만 빼고 있었다 — 그래서 우편발송 요청 건은 요약이 예산 안에 들어갔다고 판단된 뒤
// 실제로는 truncateBytes에 잘려나갔다. 정작 기사가 봐야 할 서류·키 위치가 사라지는 쪽이다.
//
// 링크는 이제 기사 챗봇 전달사항(orders.memo_driver_chat)에 남고, 기사 화면의 "해주실 일"
// 목록에도 등기 항목으로 뜬다(routes/driverChat.js buildDriverTasks). 길이 제한이 없는 쪽이
// 링크가 있어야 할 자리다.
function memoWithVehicle(order) {
  // **저장된 값을 그대로 보낸다.** 전송 시점에 조립하지 않는다(2026-09-07, 사용자 지시) —
  // 조립하면 콜마너에만 있는 문자열이 생겨서, 관리자 화면에서 무엇이 전달됐는지 확인할 수 없다.
  // 차종·차량번호는 저장할 때 이미 맨 앞에 붙는다(lib/intakeMemoSplit.js withVehiclePrefix,
  // lib/orderCreate.js). 이 함수에 남은 일은 어느 칸을 쓸지 고르는 것뿐이다.
  const brief = String(order.memo_driver_brief || '').trim();
  if (brief) return brief;
  const full = String(order.memo_customer || '').trim();
  if (full) return full;
  // 요청사항이 아예 없는 오더도 있다. 그때는 차량 표시만이라도 보낸다 — 콜마너 payload에
  // 차량 칸이 없어서 이 칸이 비면 기사는 무슨 차인지 모른다.
  return vehiclePrefix(order.vehicle_type, order.vehicle_number) || '';
}
...
async function buildOrderPayload(order, paymentMethodName, waypoints) {
  ...
  // 즉시 접수 건은 예약시각을 **싣지 않는다**(사용자 지시 2026-09-14).
  //
  // 왜 문제가 되나(사용자 정정 2026-09-14): 이 API로 넣은 오더는 바로 배차되지 않는다 —
  // 아래에서 status를 항상 대기(5)로 고정하기 때문이다. 그러니 "즉시로 보내면 바로
  // 배차된다"가 아니다. 문제는 **지나간 예약시각**이다.
  //
  // 즉시여도 reserved_date/reserved_time에는 "지금 시각"이 채워져 저장된다(폼의 즉시 라디오가
  // 현재 시각을 넣고 칸을 잠근다 — src/app/orders/new/OrderForm.js). 그 시각을 실어 보내면
  // 오더는 대기 상태로 머무는 동안 그 시각을 지나치고, 나중에 콜마너에서 대기를 접수로
  // 바꿀 때 **이미 지나간 시각**으로 처리된다.
  //
  // 즉시 건은 애초에 특정 시각을 요청한 것이 아니므로 그 칸을 비우면 이 문제가 생기지 않는다.
  //
  // (고객이 직접 고른 예약 건은 시각을 그대로 싣는다. 그것도 대기 중에 시각이 지날 수 있지만
  //  고객이 정한 값이라 우리가 지울 수 없다 — 그건 대기해제 운영에서 다룰 문제다.)
  //
  // reservation_basis가 null인 오더는 예전 그대로 싣는다 — 이 컬럼이 생기기 전에 쌓인 건이
  // 여기 해당한다. 모르는 것을 즉시로 단정하면 고객이 정해둔 예약시각을 우리가 지우는 셈이다.
  const reservationTime = order.reservation_basis === 'immediate'
    ? null
    : toReservationTime(order.reserved_date, order.reserved_time);
...
async function orderReceipt(order, branch, paymentMethodName, waypoints) {
  ...
  // 응답 **필드 이름**을 남긴다(값은 안 남긴다). 우리가 안 읽는 칸에 뭐가 들어오는지 알 방법이
  // 이것뿐이어서다 — 고객용 위치조회 링크(web_url)가 실제로 오는지 이 로그로 확인한다.
  // 탁송사진 쪽에 같은 장치가 이미 있다(collectPhotos).
  console.log(`[콜마너 접수] 응답 필드: ${Object.keys(rs || {}).join(', ') || '(없음)'}`);
...
async function orderModify(order, branch, paymentMethodName, waypoints, confSlip) {
  ...
  const rs = result.rs || result;
  // 접수 응답과 같은 이유로 필드 이름을 남긴다. 접수 때는 web_url(고객용 위치조회 링크)이
  // 오는 것을 확인했으므로(2026-09-10), 수정 응답에도 오면 **기존 오더의 링크를 얻는
  // 유일한 경로**가 된다 — 이미 접수된 건은 그때 버려서 링크가 없다.
  console.log(`[콜마너 수정] 응답 필드: ${Object.keys(rs || {}).join(', ') || '(없음)'}`);
  return { confSlip: rs.reg_cslip || confSlip, regDate: rs.reg_date || rs.regdate, webUrl: rs.web_url };
}
...
async function orderHistory(branch, userHp, options) {
  ...
  return {
    ...
    orders: list.map((o) => ({
      ...
      endTime: o.end_time || '',
      wkInfo: o.wk_info || '',
      // 콜마너가 고객에게 주는 위치조회 링크(cust-web/reception). 실측 2026-09-10 기준 우리
      // 접수건 31건 전부 빈 문자열이라 아직 온 적이 없다 — 오는 순간 잡으려고 읽어둔다.
      receiptUrl: String(o.receipt_url || '').trim() || null,
    })),
  };
}
...
// 배차기사 현위치 — 정의서 상세 시트의 TrackingDriver(order.do).
//
// 왜 필요한가(실측 2026-09-08, OID2075): 위치를 MCP에서 가져오고 있었는데 MCP가 주는
// driver.xy는 실제 GPS가 아닐 수 있다 — 응답에 source='route_interpolated'가 붙어 있었고,
// 값은 출발지↔도착지의 **정확히 1/3 지점**(위도·경도 모두 33%)이었다. 이 전문이 준 실제
// 좌표와 7.04km 떨어져 있었다. 화면은 그 보간값을 "기사님 위치"로 그리고 있었다.
//
// OrderInfo에는 좌표가 없어서 "위치는 MCP 경로에만 있다"고 적혀 있었는데, 정의서 목록에
// 이 전문이 따로 있었다. conf_slip으로 찾는다(사번만 보내면 "배차된 정보가 없습니다").
// 좌표가 없으면 null — 없는 것을 0,0으로 만들면 지도에 아프리카 앞바다가 찍힌다.
async function trackingDriver(branch, confSlip, userHp) {
  const rq = {
    userHp: normalizeUserHp([userHp, branch.main_phone]),
    providerId: buildProviderId(branch),
    conf_slip: String(confSlip || '').trim(),
  };
  const result = await callCallmaner('order', 'TrackingDriver', rq);
  const rs = result.rs || {};
  const lat = Number(rs.lat);
  const lon = Number(rs.lng);
  if (!Number.isFinite(lat) || !Number.isFinite(lon)) return null;
  if (!lat && !lon) return null;
  return { lat, lon, driverName: String(rs.wk_name || '').trim() || null };
}
...
//
// 지금 화면은 순번으로 항목을 정한다(lib/callmanerPhotos.js PHOTO_ITEMS) — 촬영 순서가
// **운행전과 운행후가 다르다**는 것을 사람에게 확인받아 표로 적었다(운행후는 계기판이 맨 앞).
// 콜마너가 항목 이름을 함께 준다면 그걸 쓰는 편이 낫다: 순서가 바뀌어도 따라가고, 표가
// 어긋나 생기는 사고(주행거리를 바퀴 사진에서 읽으려 한 일)가 애초에 생기지 않는다.
// 정의서에는 picLink만 적혀 있지만 실응답에 더 있을 수 있어, 추측하지 않고 실제 응답에서 알아낸다.
...
module.exports = {
  ...
  buildProviderId,
  // 검사에서 직접 부른다(scripts/check-order-reservation-basis.js) — 즉시 건에 예약시각이
  // 실리지 않는지는 정규식으로 보는 것보다 실제로 전문을 만들어 보는 편이 확실하다.
  // 경유지가 없으면 네트워크에 닿지 않는다(viaList만 lookupRegion을 쓴다).
  buildOrderPayload,
  callCallmaner,
