...
function loadBlowfishModule() {
  if (!blowfishModulePromise) blowfishModulePromise = import('egoroof-blowfish');
...
// `탁송(yn)` 플래그뿐이라 번호를 담을 수 없다. 적요 말고는 기사에게 번호를 전달할 경로가 없다.
//
...
function memoWithVehicle(order) {
  ...
  const rest = String(order.memo_customer || '').trim();
...
function pictureLinks(list) {
  ...
  return list
    .map((item) => {
      ...
      return String((item && (item.picLink || item.pic_link || item.url)) || '').trim();
    })
    .filter(Boolean);
}
