...
function parseCallmanerResponseBody(text) {
  ...
  const decoded = blowfishFor(keys.responseKey)
...
async function callCallmaner(servlet, cmd, rq, { timeoutMs } = {}) {
  ...
  if (callmanerEncoding() === 'encrypted') {
    const cipherText = Buffer.from(blowfishFor(callmanerCryptoKeys().requestKey).encode(payload)).toString('base64');
