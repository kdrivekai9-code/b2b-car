...
//
// 금액 상·하한(사용자 확정). 0은 "오지요금 안 받음"이다.
const REMOTE_AREA_FEE_MIN = 1000;
const REMOTE_AREA_FEE_MAX = 10000;

// 주소 문자열에서 찾을 때의 규칙 — 낱말 하나가 통째로 "…리"여야 한다.
// 도로명은 로/길로 끝나고 행정동은 동으로 끝나므로, 독립된 "…리" 토큰은 사실상 법정리다.
// 괄호 안에 들어오는 경우가 흔해서(“음암면 부산은골길17(부산리)”) 괄호도 경계로 본다.
const REMOTE_AREA_ADDRESS_RE = /(?:^|[\s(,])([가-힣]{1,6}리)(?=[\s),]|$)/;

function isRemoteArea(place) {
  const p = place || {};
  const dong = String(p.dong || '').trim();
  if (dong) return /리$/.test(dong);
  const address = String(p.address || '').trim();
  if (!address) return false;
  return REMOTE_AREA_ADDRESS_RE.test(address);
}
...
function remoteAreaFeeFor(extra, options = {}) {
  ...
  if (!isRemoteArea(origin) && !isRemoteArea(destination)) return 0;
  // 화면에서 막지만 API로 들어온 값이 범위를 벗어날 수 있다 — 계산 쪽에서도 가둔다.
  return Math.min(REMOTE_AREA_FEE_MAX, Math.max(REMOTE_AREA_FEE_MIN, fee));
}
...
async function calculateFare(branchId, distanceKm, options = {}) {
  ...
  // 오지요금은 구간요금 상한(max_fare)과 반올림 뒤에 더한다 — 상한은 "거리로 받는 요금"의
  // 한도이지 부가요금까지 묶는 값이 아니고, 반올림 전에 더하면 설정한 금액이 그대로 나오지 않는다.
  const remoteAreaFee = remoteAreaFeeFor(extra, options);
  fare += remoteAreaFee;
