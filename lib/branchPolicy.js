...
async function calculateFare(branchId, distanceKm) {
  if (!branchId) return { enabled: false };
  const extra = await db.get('SELECT * FROM fare_extra_settings WHERE branch_id = ?', [branchId]);
  if (!extra || !extra.fare_table_enabled) return { enabled: false };

  const tiers = await db.all('SELECT * FROM fare_rules WHERE branch_id = ? ORDER BY tier_seq', [branchId]);
...
async function findAnyFallbackFareBranch(preferredBranchId) {
  const withPreferredFilter = await db.get(
    `SELECT b.id, b.name
     ...
     ORDER BY b.id
     LIMIT 1`,
    [preferredBranchId || null, preferredBranchId || null]
  );
...
async function calculateFareWithFallback(branchId, distanceKm) {
  const primary = await calculateFare(branchId, distanceKm);
  if (primary.enabled) {
    return {
      ...primary,
      fallbackUsed: false,
      sourceBranchId: branchId ? Number(branchId) : null,
      sourceBranchName: null,
      representativeConfigMissing: false,
    };
  }

  const fallbackInfo = await findFallbackFareBranch(branchId || null);
