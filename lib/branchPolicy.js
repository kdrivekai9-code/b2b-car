...
async function loadSurchargeRules(table) {
  if (!table) return { placeRules: [], tollRules: [] };
  const where = table.groupId ? 'group_id = ?' : 'branch_id = ?';
  const id = table.groupId || table.branchId;
  if (!id) return { placeRules: [], tollRules: [] };
  const [placeRules, tollRules] = await Promise.all([
  ...
  return { placeRules: placeRules || [], tollRules: tollRules || [] };
}
...
async function calculateFare(branchId, distanceKm, options = {}) {
  ...
  const { placeRules, tollRules } = await loadSurchargeRules(table);
