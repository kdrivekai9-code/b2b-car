...
async function calculateFareWithFerry(branchId, distanceKm, options = {}) {
  ...
  if (hasSplitLegs) {
    const [beforeFare, afterFare] = await Promise.all([
      calculateFareWithFallback(branchId, beforeKm),
    ...
    base = await calculateFareWithFallback(branchId, distanceKm);
  }
  ...
  if (ferry.ferryApplied && ferry.ferryRouteCode && options.reservedDate && options.reservedTime
    ...
    try {
      ferryEstimate = await estimateFerryArrival({
        ...
        reservedDate: options.reservedDate,
        reservedTime: options.reservedTime,
