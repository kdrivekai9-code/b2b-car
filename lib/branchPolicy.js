...
// 이 오더가 지나는 특수구간(민자 교량 등)을 찾는다. 거리 없이 판정할 수 있어 따로 뺐다 —
// 오더 등록 시점에 정산 항목을 만들려면 요금 전체를 다시 계산할 필요가 없다.
//
// 클라이언트가 계산한 금액을 그대로 믿지 않는다. 청구되는 값이라 서버가 다시 판정한다.
async function findSpecialTolls(groupId, branchId, texts) {
  const table = await loadFareTable(groupId, branchId);
  if (!table) return [];
  const { tollRules } = await loadSurchargeRules(table);
  return fareSurcharge.matchSpecialTolls(table.extra, tollRules, texts);
}

// 이 오더에 적용되는 요금설정(포함/제외 등). 통행료 청구 규칙이 이 값으로 갈리는데,
// 호출부가 직접 테이블을 뒤지면 loadFareTable의 폴백 규칙(법인 → 지사)이 두 벌이 된다.
async function findFareExtra(groupId, branchId) {
  const table = await loadFareTable(groupId, branchId);
  return table ? table.extra : null;
}
...
// 일일기사 요금표의 **첫 구간**을 그대로 알려준다 — "5시간 90,000원부터"처럼 안내하는 데 쓴다.
//
// calculatePremiumFare는 시간을 받아 금액만 돌려주므로 기준 시간(base_hours)을 알 수 없다.
// 안내 문구에 숫자를 박아두면 관리자가 표를 바꿔도 문구가 조용히 낡는다 — 이 저장소는
// 운영시간 안내에서 같은 판단을 이미 했다(KB에 문구를 넣지 않고 테이블을 읽는다).
//
// 표를 고르는 순서는 다른 요금과 같다(법인 → 지사).
async function describeDailyDriverFare(groupId, branchId) {
  const normalizedGroupId = normalizeBranchId(groupId);
  let tiers = [];
  let source = 'branch';
  if (normalizedGroupId) {
  ...
  // 기준 시간이 가장 짧은 구간이 "…부터"의 기준이다.
  const sorted = tiers.slice().sort((a, b) => Number(a.base_hours) - Number(b.base_hours));
  const base = sorted[0];
  return {
    enabled: true,
    baseHours: Number(base.base_hours),
    baseFare: Number(base.fare_amount),
    extraPerHour: Number(base.extra_per_hour || 0),
    fareSource: source,
    // 구간이 여럿이면 표를 그대로 보여줄 수 있어야 한다 — 첫 구간만 알려주면 8시간·12시간
    // 구간을 따로 등록해 둔 지사에서 안내가 실제 청구와 어긋나 보인다.
    tiers: sorted.map((t) => ({
      baseHours: Number(t.base_hours),
      baseFare: Number(t.fare_amount),
      extraPerHour: Number(t.extra_per_hour || 0),
      note: t.note || null,
    })),
  };
}

// 법인대리(프리미엄) 편도 요금 — **탁송과 같은 거리 구간 방식.**
//
// 왜 별도 함수인가: 프리미엄은 요금표가 없어서 오더 등록 화면이 **탁송 표로** 요금을 냈다
// (routes/orders.js /fare-preview가 오더구분을 안 봤다). 사용자 확정(2026-09-08)으로 거리
// 기준 전용 표를 만들고 그 화면이 오더구분에 따라 표를 고르게 했다.
//
// ⚠ 아래 calculatePremiumFare와 헷갈리지 말 것. 그쪽은 이름과 달리 **일일기사** 시간 구간
// 요금이다(premium_fare_rules / group_daily_driver_fare_rules — 법인 쪽 이름이 그 증거다).
// 두 상품은 청구 기준이 다르다: 프리미엄=편도 거리, 일일기사=이용 시간.
//
// 표를 고르는 순서는 탁송과 같다(법인 → 지사). 등록된 구간이 없으면 { enabled: false }다 —
// 없는 값을 지어내지 않는다. 요금 문의는 그때 "등록되어 있지 않습니다"로 안내한다.
//
// 마이그레이션 전이면 표가 아예 없다. 그때도 접수는 그대로 진행돼야 하므로 조회 실패를
// 빈 목록으로 다룬다(배차 요금표와 같은 처리).
async function calculatePremiumOnewayFare(groupId, branchId, distanceKm) {
  const normalizedGroupId = normalizeBranchId(groupId);
  let tiers = [];
  let source = 'branch';
  if (normalizedGroupId) {
    tiers = await db.all(
      'SELECT * FROM group_premium_oneway_fare_rules WHERE group_id = ? ORDER BY tier_seq',
      [normalizedGroupId]
    ).catch(() => []);
    if (tiers.length) source = 'group';
  }
  if (!tiers.length) {
    ...
    if (!normalizedBranchId) return { enabled: false };
    tiers = await db.all(
      'SELECT * FROM premium_oneway_fare_rules WHERE branch_id = ? ORDER BY tier_seq',
      [normalizedBranchId]
    ).catch((e) => {
      console.error('프리미엄 편도 요금표 조회 실패(요금 없이 진행):', e.message);
      return [];
    });
  }
  if (!tiers.length) return { enabled: false };
  // 구간 계산은 탁송과 같은 함수를 쓴다 — 같은 방식이면 계산을 두 벌 만들 이유가 없다.
  const { fare, tier } = fareFromTiers(tiers, Number(distanceKm) || 0);
  return { enabled: true, fare, tierSeq: tier.tier_seq, fareSource: source };
}
...
async function calculateFare(branchId, distanceKm, options = {}) {
  ...
  const specialTolls = fareSurcharge.matchSpecialTolls(extra, tollRules, [
    ...
    // 실제 지나간 요금소 이름 — 주소만 보면 경로 중간의 교량이 잡히지 않는다.
    options.tollgates || (options.routeMeta && options.routeMeta.tollgates) || [],
  ]);
...
async function calculatePremiumFare(branchId, hours, options = {}) {
  ...
  return {
    ...
    tierSeq: tier.tier_seq,
    fareSource: source,
    // 어떻게 그 금액이 나왔는지 — 고객 안내가 "기준 5시간 90,000원 + 초과 3시간"처럼 풀어
    // 설명하려면 고른 구간을 알아야 한다. 금액만 돌려주면 안내 문구가 숫자를 따로 들고 있게
    // 되고, 그러면 관리자가 표를 바꿔도 설명만 옛날 값으로 남는다.
    baseHours: Number(tier.base_hours),
    baseFare: Number(tier.fare_amount),
    extraPerHour: Number(tier.extra_per_hour || 0),
    extraHours,
  };
}
...
module.exports = {
  ...
  calculatePremiumOnewayFare,
  describeDailyDriverFare,
