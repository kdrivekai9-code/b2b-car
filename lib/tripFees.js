...
// 한 오더에 청구되는 "운행요금" — 구간요금(할증 포함) + 대기요금 + 취소요금.
//
// 정산서와 다른 화면이 같은 숫자를 말해야 한다. 고객이 한 화면에서 9만원, 다른 화면에서
// 10만원을 보면 그 뒤 대화는 전부 그 차이를 해명하는 데 쓰인다.
//
// 도선료(ferry_fare_amount)는 여기 넣지 않는다 — 부대비용(기타 정산)으로 옮겼기 때문이다
// (사용자 지시). 부대비용 줄로 이미 집계되므로 여기서 더하면 두 번 세어진다.
// 할증은 fare_amount 안에 이미 들어 있다(fare_surcharges_json은 그 내역일 뿐 별도 금액이
// 아니다) — 그래서 따로 더하지 않는다. 정산서의 baseFare가 fare - surchargeTotal로 역산하는
// 것과 같은 전제다.
function billableTripFare(order) {
  const n = (v) => (Number(v) || 0);
  return n(order && order.fare_amount) + n(order && order.wait_fee_amount) + n(order && order.cancel_fee_amount);
}

module.exports = {
  waitFee, cancelFee, applyCancelFee, keysFor, billableTripFare,
