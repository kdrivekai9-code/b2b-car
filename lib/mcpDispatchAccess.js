...
// requesterGroupId를 넘기면 그 법인의 오더로만 좁힌다.
//
// 조회 범위를 정하는 건 created_by(= 어느 계정이 접수했나) 하나뿐이었다. 그래서 담당 계정
// 하나를 여러 법인 매핑이 함께 쓰면(카카오 채널 매핑에서 실제로 가능하다) 법인 A의 고객이
// 법인 B의 접수 건까지 보게 된다 — 법인이 매핑에 지정돼 있어도 조회 판정에는 쓰이지 않았기
// 때문이다. 법인이 확정된 채널(카카오 매핑)은 그 경계를 넘지 못하게 한다.
// 웹(로그인 사용자)은 넘기지 않는다 — 여러 법인을 담당하는 계정의 조회를 막을 이유가 없다.
async function loadUsageCids(user, limit, requesterGroupId) {
  if (!user || !user.id) return [];
  try {
    const params = [user.id];
    let groupClause = '';
    if (requesterGroupId) {
      groupClause = ' AND requester_group_id = ?';
      params.push(requesterGroupId);
    }
    params.push(Math.max(1, Math.min(Number(limit) || 5, 20)));
    const rows = await db.all(
      `SELECT origin_contact FROM orders
       WHERE created_by = ? AND origin_contact IS NOT NULL AND origin_contact <> ''${groupClause}
...
async function loadDispatchContext(user, options) {
  ...
  const usageCids = await loadUsageCids(user, MAX_USAGE_CIDS, options && options.requesterGroupId);
  const allowedCids = [];
  ...
  return {
    ...
    viewerScoped: false,
    // 법인 전체 조회를 제안할지 판단하는 데 쓴다(lib/mcpDispatchAgent.js). MCP 조회는
    // 연락처 기준이라 동료가 접수한 건은 안 보이는데, 웹에서는 본사 직원이 법인 전체를
    // 본다 — 같은 사람이 같은 회사 일을 묻는데 채널에 따라 답이 다르면 안 된다.
    requesterGroupId: (options && options.requesterGroupId) || user.group_id || null,
...
// 우리 오더번호(OID)로 콜마너 접수번호를 찾는다.
//
// 왜 필요한가: 봇은 "접수번호 OID1459(180891275)"처럼 둘을 함께 보여준다. 고객은 당연히 앞의
// OID를 그대로 말하는데, 소유 확인(assertOwnedOrder)은 콜마너 접수번호로만 찾고 있어서
// "고객님 주문 목록에서 찾지 못했습니다"가 나갔다(실사용 2026-08-25: OID1474 — 콜마너에도
// 우리 DB에도 멀쩡히 있는 진행 중 주문이었다).
//
// "OID1474", "oid1474", "1474" 모두 받는다 — 고객이 접두어를 빼고 말하는 경우가 흔하다.
async function loadCallmanerSlipByOid(value) {
  const raw = String(value || '').trim();
  if (!raw) return null;
  const digits = raw.replace(/\D/g, '');
  if (!digits) return null;
  const candidates = Array.from(new Set([raw.toUpperCase(), `OID${digits}`]));
  try {
    const placeholders = candidates.map(() => '?').join(',');
    const row = await db.get(
      `SELECT callmaner_conf_slip FROM orders
        WHERE UPPER(oid) IN (${placeholders}) AND callmaner_conf_slip IS NOT NULL
        ORDER BY id DESC LIMIT 1`,
      candidates
    );
    return row ? String(row.callmaner_conf_slip).trim() : null;
  } catch (e) {
    console.error('OID → 콜마너 접수번호 조회 실패:', e.message);
    return null;
  }
}
...
async function assertOwnedOrder(ctx, rcptNo) {
  ...
  // 고객이 우리 OID를 말했으면 콜마너 접수번호로 바꿔서도 찾아본다 — 봇이 둘을 함께 보여주므로
  // 고객은 앞의 OID를 그대로 말한다.
  const bySlip = await loadCallmanerSlipByOid(target);
  const keys = bySlip && bySlip !== target ? [target, bySlip] : [target];

  const active = await loadOwnedOrders(ctx, { includeHistory: false });
  for (const key of keys) {
    if (active.byRcptNo.has(key)) return { order: active.byRcptNo.get(key) };
  }

  const withHistory = await loadOwnedOrders(ctx, { includeHistory: true, historyPageSize: 20 });
  for (const key of keys) {
    if (withHistory.byRcptNo.has(key)) return { order: withHistory.byRcptNo.get(key) };
  }
