...
// 알아서 처리하도록).
async function getKakaoDirections({ origin, destination, waypoints, priority, avoid, departureTime }) {
...
// 법인 토글 조회 — 법인이 없거나(개인 요청 등) 마이그레이션 전 DB(컬럼 없음)면 기존 동작
// 유지 차원에서 켜짐으로 본다(supabase/migrations/20260813010000의 기본값과 동일한 원칙).
async function isRouteFareSearchEnabled(groupId) {
  if (!groupId) return true;
  try {
    const row = await db.get('SELECT route_fare_search_enabled FROM groups_tbl WHERE id = ?', [groupId]);
    if (!row) return true;
    return row.route_fare_search_enabled !== false;
  } catch (e) {
...
// 웹 AI 챗봇·카카오톡 상담이 공유하는 단 하나의 진입점. 토글이 꺼져 있으면 네트워크 호출
...
async function searchRouteAndFare({ groupId, branchId, originLat, originLon, destinationLat, destinationLon, vehicleType, waypoints, onRoute }) {
  const enabled = await isRouteFareSearchEnabled(groupId);
  if (!enabled) return { enabled: false };

  if (originLat == null || originLon == null || destinationLat == null || destinationLon == null) {
    return { enabled: true, ok: false, error: '좌표가 확인되지 않았습니다.' };
  }
  ...
  if (!directions.ok) return { enabled: true, ok: false, error: directions.error, detail: directions.detail };
  ...
  const fareResult = await calculateFareWithFerry(branchId, distanceKm, {
    vehicleType,
    hasFerryLeg: directions.hasFerryLeg,
