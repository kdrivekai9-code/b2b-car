...
async function getKakaoDirections({ origin, destination, waypoints, priority, avoid, departureTime, _avoidDropped }) {
  ...
  // 지나간 요금소 이름. 카카오는 톨비를 **합계 하나로만** 준다(실측 summary.fare.toll) —
  // 요금소별 금액은 없다. 대신 안내정보(guides)에 요금소 이름이 나오므로, "어느 특수구간을
  // 지났는가"는 이것으로 판정할 수 있다(금액은 우리가 등록한 표에서 가져온다).
  //
  // 예전에는 출발지·도착지 주소 문자열만 훑어서, 경로 중간의 교량·유료도로는 잡히지 않았다.
  const tollgates = [];
  (route.sections || []).forEach((section) => {
    (section.guides || []).forEach((g) => {
      const name = String((g && g.name) || '').trim();
      if (name && !tollgates.includes(name)) tollgates.push(name);
    });
  });
