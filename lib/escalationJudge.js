...
// 고객이 대놓고 상담원을 찾는 말인가.
//
// 왜 키워드로 따로 잡나: 예전에는 이 판정이 없어서 "상담원호출"이 LLM 분류의 unsupported로
// 떨어졌고, 그러면 배차 도우미(MCP)가 먼저 가로채 "상담원에게 연결해드릴까요?"라고 **되물었다**.
// 실사용에서 고객이 "네"라고 답했는데도 실제 인계도, 상담원 호출 알림도 나가지 않았다.
// 카카오 "상담원 연결" 버튼은 동의 절차 없이 곧바로 인계하는데(/receive/reference), 같은 뜻을
// 타이핑으로 말한 고객만 되묻기에 갇히는 것은 앞뒤가 맞지 않는다.
//
// 사람을 가리키는 말이 있고 그 바로 뒤에 요청 동사가 붙을 때만 잡는다. 로그에서 압도적으로
// 흔한 "기사님 전화번호 부탁드립니다", "고객님과 통화부탁드립니다"는 잡히면 안 된다 — 그건
// 봇이 답할 수 있는 조회이고, 사람을 부를 일이 아니다. 같은 이유로 `담당자`도 넣지 않았다
// (로그의 "담당자 통화가 안되는데"는 고객사 담당자를 가리킨다).
const AGENT_WORD = '(?:상담원|상담사|상담\\s?직원)';
const AGENT_REQUEST_RE = new RegExp(
  // "상담원 연결해주세요", "상담원호출", "상담사와 통화하고 싶어요"
  `${AGENT_WORD}\\s*(?:님)?\\s*(?:을|를|이|가|와|과|이랑|하고|한테|에게)?\\s*`
  + '(?:연결|호출|바꿔|바꾸|불러|통화|얘기|이야기|대화|요청|부탁|주세요|주실)'
  // "상담원"만 덩그러니 보낸 경우
  + `|^\\s*${AGENT_WORD}\\s*(?:님)?\\s*[?!.]*\\s*$`
  // "사람이랑 얘기하고 싶어요"
  + '|사람\\s*(?:이랑|하고|과|와|한테)\\s*(?:얘기|이야기|통화|대화|연결)'
);

function isAgentRequest(text) {
  return AGENT_REQUEST_RE.test(String(text || ''));
}
...
module.exports = {
  ESCALATION_RE, needsHumanByKeyword, judgeNeedsHuman, categoryLabel,
  AGENT_REQUEST_RE, isAgentRequest,
};
