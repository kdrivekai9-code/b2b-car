...
const INTAKE_EXTRA_ITEMS = [
  ...
  // 한 오더에 도선료는 하나뿐이다(금액 출처가 컬럼 하나) — 화면에서 중복 추가를 막는다.
  { chargeType: '도선료', label: '도선료', options: [], single: true, ferry: true },
];
...
function parseIntakeRows(body, feeExtra, fallbackDate) {
  ...
  for (let i = 0; i < types.length; i++) {
    ...
    const amount = normalizeAmount(amounts[i]);
    if (item.ferry) {
      // 도선료는 한 줄만 쓴다. 여러 번 와도 마지막 것을 쓴다 — 화면이 막고 있으므로 여기까지
      // 오는 건 조작된 요청뿐이고, 그때 여러 줄을 만들면 두 번 청구된다.
      ferry = { amount, settleMode: mode };
      continue;
    }
    rows.push({
  ...
  return { rows, ferry, knownIds: arr(body.intake_extra_known_id).map(Number).filter(Number.isFinite) };
}
...
async function loadIntakeRows(orderId) {
  const rows = await db.all(
    `SELECT id, charge_type, amount, option_code, settle_mode
      ...
      ORDER BY id`,
    [orderId, INTAKE_TYPES.filter((t) => t !== FERRY_TYPE)]
  );
  return rows.map((r) => ({
...
module.exports = {
  ...
  parseIntakeRows, saveIntakeRows, loadIntakeRows,
};
