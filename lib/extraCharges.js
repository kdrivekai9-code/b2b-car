...
// 실비 줄과 그 근거 사진(영수증)을 함께 읽는다.
//
// 왜 필요한가: 금액만 있고 사진이 어디 있는지 모르면 청구를 다투는 자리에서 아무것도 못 한다.
// 기사가 채팅으로 올린 영수증이 어느 줄의 근거인지는 chat_message_id가 잇는다
// (supabase/migrations/20260903010000_add_driver_chat.sql).
//
// 마이그레이션 전이면 그 칸이 없다 — 사진 없이 줄만 돌려준다. 영수증을 못 보여주는 것과
// 정산 화면이 통째로 죽는 것은 다르다.
async function loadWithReceipts(orderId) {
  const rows = await loadForOrder(orderId);
  if (!rows.length) return [];
  try {
    const linked = await db.all(
      `SELECT e.id AS charge_id, m.attachments_json, m.created_at
         FROM order_extra_charges e
         JOIN chat_messages m ON m.id = e.chat_message_id
        WHERE e.order_id = ?`,
      [orderId]
    );
    ...
    linked.forEach((r) => {
      let files = [];
      try {
        const v = JSON.parse(r.attachments_json || '[]');
        files = Array.isArray(v) ? v : [];
      } catch (e) { files = []; }
      byCharge.set(String(r.charge_id), { files, at: r.created_at });
    });
    return rows.map((r) => ({ ...r, receipt: byCharge.get(String(r.id)) || null }));
  } catch (e) {
    // 42703 = 없는 칸(chat_message_id). 마이그레이션 전이다.
    if (!(e && (e.code === '42703' || e.code === '42P01'))) throw e;
    return rows.map((r) => ({ ...r, receipt: null }));
  }
}
...
//
// 접수 시점에 금액을 정할 수 있는 항목은 **주유비 하나뿐이다**(사용자 확정 2026-09-02).
// "3만원어치 주유해 주세요"는 고객이 정한 확정금액이라 그대로 쓴다. 나머지는 접수 때 금액을
// 알 수 없다 — 얼마가 될지는 기사가 쓰고 영수증을 올려야 정해지는 실비다. 그래서 금액 칸을
// 아예 열지 않는다(fixedAmount). 칸이 열려 있으면 누군가 어림값을 넣고, 그 어림값이 영수증
// 없이 그대로 청구된다.
const INTAKE_EXTRA_ITEMS = [
  {
    chargeType: '주유비', label: '주유비',
    options: [{ value: 'full', label: '가득(full)' }, { value: 'amount', label: '금액지정' }],
    amountOption: 'amount',
    fixedAmount: true,
  },
  {
    ...
    options: [{ value: 'full', label: '가득(full)' }],
  },
  {
...
// 고객(client)에게 열어주는 항목 — 실비 넷뿐이다(사용자 확정 2026-09-02).
//
// 왜 열어주나: 지금까지 부대비용 입력을 통째로 감췄는데, 그 안에는 금액(청구액)과 **"무엇을
// 해달라"(지시)**가 섞여 있었다. "주유 가득"은 금액이 아니라 지시다 — 접수 때는 금액을 아무도
// 모르고, 기사가 쓰고 영수증을 올려야 정해진다. 지시를 넣을 칸까지 함께 사라져서 고객은
// 요청사항 본문에 글로 쓸 수밖에 없었고, 그 본문은 아무도 읽지 않았다.
//
// 왜 넷만인가: orders 컬럼에 저장되는 셋은 성격이 다르다.
//   · 도선료 — 경로탐색이 자동으로 채운다. 고객이 고를 일이 없다.
//   · 대기요금·취소요금 — 실비가 아니라 운행요금이고, 금액을 직접 넣는 항목이다.
const CLIENT_INTAKE_TYPES = INTAKE_EXTRA_ITEMS.filter((it) => !it.orderColumn).map((it) => it.chargeType);

function isClientAllowedType(chargeType) {
  return CLIENT_INTAKE_TYPES.includes(String(chargeType || '').trim());
}
...
function intakeOptionsFor(feeExtra, options = {}) {
  // 고객에게는 실비 넷만 내려준다 — 화면에서 감추는 것보다 아예 안 주는 편이 확실하다.
  const items = options.forClient
    ? INTAKE_EXTRA_ITEMS.filter((it) => isClientAllowedType(it.chargeType))
    : INTAKE_EXTRA_ITEMS;
  return {
    // 고객은 정산구분을 고르지 않는다 — 청구 방식은 계약이고 요금설정이 정한다.
    // 화면이 칸을 안 그리고, 서버도 보내온 값을 무시한다(parseIntakeRows).
    forClient: !!options.forClient,
    items: items.map((it) => ({
      chargeType: it.chargeType,
      label: it.label,
      options: it.options,
      amountOption: it.amountOption || null,
      // 접수 때 금액을 넣을 수 있는가. orders 컬럼 항목(도선료·대기·취소요금)은 금액이
      // 본질이라 언제나 넣을 수 있고, 줄로 남는 실비는 주유비만 열린다.
      fixedAmount: !!it.fixedAmount || !!it.orderColumn,
      single: !!it.single,
      ferry: !!it.ferry,
...
function parseIntakeRows(body, feeExtra, fallbackDate, opts = {}) {
  // 고객이 보낸 것은 두 가지를 무시한다 — 화면이 칸을 안 그려도 우회해 보낼 수 있다.
  //   · 정산구분 — 청구 방식은 계약이고 요금설정이 정한다.
  //   · 실비 넷 밖의 항목 — 도선료는 경로탐색이 채우고, 대기·취소요금은 운행요금이다.
  const asClient = !!opts.asClient;
  ...
  for (let i = 0; i < types.length; i++) {
    ...
    if (!item) continue;
    if (asClient && !isClientAllowedType(item.chargeType)) continue;
    const mode = (asClient ? null : normalizeMode(modes[i]))
      || fareSurcharge.settleModeOf(feeExtra, item.chargeType);
    const optionCode = normalizeOption(item, options[i]);
    // 금액을 정할 수 없으면 눌러둔다 — 화면이 칸을 안 그리지만 우회해 보낸 값이 영수증 없이
    // 청구되면 안 된다. 두 가지가 걸린다.
    //   · 항목 자체가 금액을 못 정하는 것(충전·세차·주차) — 얼마가 될지는 기사가 쓰고
    //     영수증을 올려야 정해진다.
    //   · 주유비인데 '가득'을 고른 것 — 가득은 접수 때 금액을 모르는 선택이다.
    //     amountOption('금액지정')을 골랐을 때만 금액이 뜻을 갖는다.
    const canFixAmount = (!!item.fixedAmount || !!item.orderColumn)
      && (!item.amountOption || optionCode === item.amountOption);
    const amount = canFixAmount ? normalizeAmount(amounts[i]) : 0;
    if (item.orderColumn) {
...
async function loadIntakeRows(orderId) {
  ...
  // chat_message_id는 마이그레이션(20260903010000)으로 생긴 칸이다. 없으면 그 칸 없이 다시
  // 읽는다 — 영수증 표시를 못 하는 것과 화면이 통째로 죽는 것은 다르다.
  const select = (extra) => `SELECT id, charge_type, amount, option_code, settle_mode${extra}
       FROM order_extra_charges
      WHERE order_id = ? AND charge_type = ANY(?::text[])
      ORDER BY id`;
  let rows;
  try {
    rows = await db.all(select(', chat_message_id'), [orderId, tableTypes]);
  } catch (e) {
    if (!(e && e.code === '42703')) throw e;
    rows = await db.all(select(''), [orderId, tableTypes]);
  }
  const out = rows.map((r) => ({
    id: r.id, chargeType: r.charge_type, amount: r.amount || 0,
    optionCode: r.option_code || '', settleMode: r.settle_mode || '',
    // 이미 영수증이 붙었는지. 올린 것을 또 올리라고 하면 기사가 같은 일을 두 번 한다.
    hasReceipt: !!r.chat_message_id,
  }));
...
module.exports = {
  EXTRA_CHARGE_TYPES, billableTypes, loadWithReceipts, billableTypesForOrder, normalizeType, normalizeDate,
  ...
  CLIENT_INTAKE_TYPES, isClientAllowedType,
