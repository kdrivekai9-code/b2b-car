...
async function detailForClient(orderId, scope) {
  ...
  const phases = [
    ...
    items: photos
      ...
      // 이름은 **단계마다 다르다** — 운행후는 계기판이 1번으로 앞에 붙어 한 칸씩 밀린다
      // (lib/callmanerPhotos.js PHOTO_ITEMS). 단계를 안 넘기면 운행후 이름이 전부 한 칸 어긋난다.
      .map((p) => ({
        ...p,
        label: callmanerPhotos.photoLabel(p.seq, g.phase),
        // 계기판 자리는 순번이 아니라 **항목**으로 가린다. 순번으로 가리면 단계가 바뀔 때 틀린다.
        isOdometer: callmanerPhotos.photoItem(p.seq, g.phase) === '계기판',
      })),
  }));
  ...
  return {
    ...
    // 계기판 표시는 이제 사진 줄의 isOdometer가 들고 있다(단계마다 순번이 달라서다).
    // 이 값은 화면 호환을 위해 남긴다 — 운행전 기준 순번이다.
    odometerIndex: callmanerPhotos.odometerPhotoIndex(branch, callmanerPhotos.PHASE_START),
