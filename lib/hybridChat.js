...
async function classifyAndExtract(text, pendingField, intent) {
  ...
  const fullPromise = generateJson(buildSystemInstruction(pendingField, intent), text, RESPONSE_SCHEMA);
  ...
  const light = await generateJson(buildLightIntentInstruction(), text, LIGHT_INTENT_SCHEMA, { thinking: false })
    .catch(() => null);
...
async function classifyPhaseReply(text, phase, extra) {
  ...
  return generateJson(buildPhaseReplyInstruction(phase, extra), text, buildPhaseReplySchema(phase, extra), { thinking: false });
}
