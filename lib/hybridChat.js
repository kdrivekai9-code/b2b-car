...
const RESPONSE_SCHEMA = {
  ...
  properties: {
    ...
    waypointAddressDetail: { type: 'STRING' },
...
- **주소가 셋 이상 언급되면 맨 처음이 출발지(originAddress), 맨 마지막이 도착지(destinationAddress), 그 사이가 경유지(waypointAddress)입니다.**
  "A에서 B까지, 그다음 B에서 C로"처럼 구간을 이어서 말한 경우가 여기 해당합니다 — 이때 B는 경유지이고 **C가 도착지**입니다.
  중간에 나온 주소를 도착지로 잡고 마지막 주소를 빠뜨리면 안 됩니다. 같은 주소가 두 번 나오면(…B까지, B에서…) 한 곳으로 봅니다.
...
- originAddressDetail / destinationAddressDetail / waypointAddressDetail: 주소에서 **행정구역+도로명+건물번호까지만** 주소 필드에 넣고, 그 뒤에 붙은 나머지를 여기에 분리해 넣으세요.
  · 상세위치(층수, 주차구역, 동/호수 등)뿐 아니라 **건물명·상호명·지점명·센터명도 여기로 분리합니다.**
  · 예: "부산 해운대구 선수촌로 187 KGM 해운대서비스센터" → destinationAddress="부산 해운대구 선수촌로 187", destinationAddressDetail="KGM 해운대서비스센터"
  · 예: "서울 강남구 테헤란로 152 강남파이낸스센터 지하3층" → address="서울 강남구 테헤란로 152", detail="강남파이낸스센터 지하3층"
  · 단, 도로명·건물번호 없이 상호명만 말한 경우("사당역 탐앤탐스", "인천공항 1터미널")는 그 자체가 검색해야 할 주소이므로 **주소 필드에 그대로** 두고 detail은 비웁니다.
...
function buildSystemInstruction(pendingField, intent) {
  ...
  // ⚠ RESPONSE_SCHEMA.intent의 실제 enum 값은 'proxy_order'다('premium_order'가 아니다) —
  // 이전에는 이 오타 때문에 이 분기가 절대 선택되지 않았다.
  const isPremiumOrDaily = intent === 'proxy_order' || intent === 'daily_driver_order';
...
// intent만 판단하는 가벼운 스키마 — 필드 추출(주소/일시/차량 등)은 절대 하지 않는다.
// 실측: intent만 볼 때는 thinking 없이도 8/8 정확했지만, 여기에 필드를 단 2개(구간 요금용
// 출발/도착지)만 조건부로 얹었을 뿐인데 정확도가 무너졌다(사고 재현 입력이 faq로 오분류됨).
// "필드를 하나라도 같이 시키면 thinking이 필요해진다"는 뜻이라, 이 가벼운 경로는 필드를 절대
// 섞지 않는다 — intent/requestedFeature/seemsFrustrated만 판단한다.
const LIGHT_INTENT_SCHEMA = {
  type: 'OBJECT',
  properties: {
    intent: { type: 'STRING', enum: ['dispatch_order', 'proxy_order', 'daily_driver_order', 'faq', 'unsupported'] },
    requestedFeature: { type: 'STRING' },
...
function buildLightIntentInstruction() {
  return `당신은 탁송(차량 배송)과 대리운전(프리미엄 서비스·일일기사)을 함께 취급하는 B2B 플랫폼의
하이브리드 챗봇 입력 분석기입니다. 사용자 메시지를 읽고 intent만 판단하세요(필드 추출은 하지 마세요):
- "dispatch_order": 탁송 오더 접수(신규 차량 픽업/배송 예약) 요청
- "proxy_order": 대리/프리미엄 오더 접수(대리운전·프리미엄 서비스 예약) 요청
- "daily_driver_order": 일일기사 오더 접수(일일 대리기사/기사 대절, 8시간 이상 대여 포함) 요청
- "unsupported": 오더 수정/변경, 취소, 배차·진행 상태 조회, 상담원 연결 요청 등. requestedFeature 필드에 요청한 기능을 짧은 한글 명사구로 채우세요.
...
메시지가 짧거나 값 하나만 온 경우라도 뚜렷한 질문 형태나 상담원 연결 의사가 없으면 오더 intent로 분류하세요.
seemsFrustrated: 사용자가 화가 났거나 답답해하는 것으로 보이면 true, 아니면 이 키를 아예 빼세요.`;
}

// "얼마/요금" 같은 말이 있으면 faq라도 구간 요금 계산에 originAddress/destinationAddress가
// 필요할 수 있다(lib/agentAssist.js buildFareSuggestion) — 그 경우는 가벼운 경로로 지름길을
// 내지 않고 전체 추출을 기다린다. lib/agentAssist.js의 FARE_QUESTION_RE와 같은 정규식이지만,
// 그쪽을 require하면 순환 참조가 생겨(agentAssist → hybridChat) 여기서 값만 복제해 둔다.
const FARE_QUESTION_HINT_RE = /(요금|얼마|비용|가격|견적|단가)/;
...
async function classifyAndExtract(text, pendingField, intent) {
  // 필드 30개짜리 스키마로 여러 줄 자유 문장을 해석하는 복잡한 추출이라 thinking을 켜 둔다
  // (기본값). 꺼두면(2026-08-10 실측) 도착지를 경유지로 잘못 분류하고 원문에 없는 예약시간을
  // 지어내는 사고로 이어진다 — 응답 속도보다 접수 정확도가 우선이다.
  const fullPromise = generateJson(buildSystemInstruction(pendingField, intent), text, RESPONSE_SCHEMA, { op: 'intake_extract' });
  // 실패 핸들러를 만들자마자 붙인다. 아래에서 light를 기다리는 동안 이 약속이 거부되면,
  // 그 시점에는 아직 아무도 듣고 있지 않아 Node가 "처리되지 않은 거부"로 보고 프로세스를
  // 통째로 죽인다 — 구글이 10초 동안 안 닿았을 때 실제로 서버가 그렇게 내려갔고, 오더 목록이
  // 500으로 떴다. 여기서 핸들러를 붙여도 아래 return fullPromise를 await하는 쪽은 그대로
  // 예외를 받는다(핸들러 부착은 "누가 듣고 있다"는 표시일 뿐 결과를 삼키지 않는다).
  fullPromise.catch(() => {});

  // pendingField/intent가 이미 지정된 호출(확인/후보선택 등 문맥이 좁혀진 되묻기, 프리미엄/
  // 일일기사 힌트가 있는 호출)은 아래 지름길의 검증 범위 밖이다 — 그대로 전체 추출만 기다린다.
  if (pendingField || intent) return fullPromise;

  // 전체 추출과 동시에 가벼운 intent 판단도 시작해둔다(thinking off, ~1초). 가벼운 쪽이
  // "필드가 필요 없는 의도"(요금 문의가 아닌 faq, 또는 unsupported)라고 답하면, 이미 시작된
  // 전체 추출을 기다리지 않고 곧바로 그 결과로 답한다 — 전체 추출은 배경에서 마저 돌게 두고
  // 결과는 버린다(실패해도 무시). 오더 접수 계열은 이 지름길을 타지 않고 항상 전체 추출(thinking
  // on)을 기다린다 — 필드 정확도가 걸린 경로는 절대 지름길로 보내지 않는다.
  const light = await generateJson(buildLightIntentInstruction(), text, LIGHT_INTENT_SCHEMA, { thinking: false, op: 'intent_light' })
    .catch(() => null);

  if (light && !ORDER_INTENTS.has(light.intent)) {
    const isFareQuestion = light.intent === 'faq' && FARE_QUESTION_HINT_RE.test(text);
    if (!isFareQuestion) {
      return light;
    }
  }
...
- "yes": 등록/진행에 동의함
- "no": 등록하지 말고 내용을 수정하고 싶다고 명확히 말함(예: "수정할게요", "차량번호가 틀렸어요", "시간을 바꿔주세요")
- "agent": 상담원 연결을 원함
- "unclear": 위 어디에도 명확히 해당하지 않음 — 등록 여부와 무관하게 무언가를 되묻는 질문(예: "출발지 픽업시간은 언제예요?", "요금이 얼마죠?")이면 수정 요청으로 넘겨짚지 말고 반드시 이걸로 분류하세요.`;
  }
...
- "field": 특정 항목을 이런 값으로 바꿔달라고 명확히 말함 — field에 해당 항목의 영문 id를 정확히 채우세요. 항목 이름을 언급만 하고 실제로는 그 항목에 대해 되묻는 질문(예: "출발지에는 몇시에 와요?")이면 이걸로 분류하지 마세요.
- "none": 사실 수정할 게 없다고 함(마음이 바뀜, 그냥 등록해도 된다는 의미)
- "agent": 상담원 연결을 원함
- "unclear": 위 어디에도 명확히 해당하지 않음 — 항목을 무엇으로 바꿀지가 아니라 그 항목에 대해 되묻는 질문이면 반드시 이걸로 분류하세요.`;
  }
...
async function classifyPhaseReply(text, phase, extra) {
  // "네/아니오/1번" 수준의 짧은 답변 하나를 정해진 enum 몇 개 중 하나로 고르는 단순 판단이라
  // (자유 문장에서 여러 필드를 뽑는 classifyAndExtract와 달리 다중 필드 추출이 없다) thinking을
  // 꺼도 정확도 저하가 없다 — 응답 속도를 아낀다.
  return generateJson(buildPhaseReplyInstruction(phase, extra), text, buildPhaseReplySchema(phase, extra), { thinking: false, op: 'phase_reply' });
}
