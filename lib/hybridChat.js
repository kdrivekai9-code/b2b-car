...
- memo: 기사요청사항 — 탁송기사가 실제로 업무를 수행하거나 수행 후 처리해야 할 내용(예: 사고 이력 안내, 차량 상태/스크래치, 서류 인수·전달 요청, 주차 위치 안내 등). 위 필드에 해당하지 않는 요청사항·차량상태·특이사항은 기본적으로 여기에 넣으세요.
- billingMemo: 업체요청사항 — 탁송 업무 자체가 아니라, 업체(고객사)가 계산서·내역서·명세서 등을 발행할 때 참고해야 하는 내용(예: "비고란에 'OOO'로 기재해주세요", 세금계산서 발행 관련 요청 등). memo와 겹치지 않게, 이 조건에 명확히 해당할 때만 채우세요.
...
const PHASE_REPLY_SCHEMA = {
  ...
  properties: {
    ...
    field: { type: 'STRING', enum: ['origin_address', 'origin_contact', 'destination_address', 'destination_contact', 'vehicle_number', 'reserved_date'] },
  },
  required: ['action'],
};
...
방금 챗봇이 "어느 부분을 수정해드릴까요?"라고 물었습니다(선택 가능 항목: 출발지 주소=origin_address, 출발지 연락처=origin_contact, 도착지 주소=destination_address, 도착지 연락처=destination_contact, 차량번호=vehicle_number, 예약일시=reserved_date). 답변을 분류하세요:
