...
// 이번 발화가 앞 접수를 **대체**하는가(보충이 아니라 갈아타기인가).
//
// 되묻기 중에는 이번 발화를 앞 원문에 이어붙인다("차량번호는 12가3456이요"만 와도 앞서 받은
// 출발지·도착지가 사라지지 않게). 그런데 고객이 도중에 다른 오더로 갈아타면 그 이어붙이기가
// 두 주문을 한 문장으로 만든다. 실측(2026-08-28):
//
//   "강남역에서 판교역으로 탁송 예약해줘" + "판교역에서 인천공항으로 탁송 하나 더 접수해줘"
//     → 출발지 강남역(옛 것), 도착지 없음. 새 주문의 출발지(판교역)가 아니다.
//   "강남역에서 판교역으로 탁송 예약해줘" + "아니요 사당역에서 서초동으로 바꿔주세요"
//     → 출발지·도착지가 **둘 다 사라진다**(모순된 두 경로를 읽다 아무것도 못 뽑는다).
//
// 판정은 좁게 둔다: **이번 발화 하나에 출발과 도착이 다 들어 있을 때만** 갈아타기로 본다.
// 진짜 보충 답변은 두 끝점을 함께 담지 않는다("판교역이요", "내일 3시요", "12가3456").
// looksSelfContained를 그대로 쓰면 안 된다 — 그쪽은 "내일 오후 3시에 탁송 예약해주세요"처럼
// 시각만 보충하는 답도 완결로 보므로, 그걸 갈아타기로 처리하면 멀쩡한 출발·도착이 날아간다.
function looksLikeRouteRestart(text) {
  return ROUTE_SHAPE_RE.test(String(text || '').trim());
}

// 갈아탔다는 사실은 알린다 — 앞서 말한 내용을 조용히 버리면 "아까 말한 건 어디 갔냐"가 된다.
const INTAKE_RESTARTED_NOTICE = '앞서 말씀하신 내용 대신 새로 알려주신 경로로 다시 접수하겠습니다.';

module.exports = {
  DEFAULT_TTL_MINUTES,
  INTAKE_EXPIRED_NOTICE,
  INTAKE_RESTARTED_NOTICE,
  looksSelfContained,
  looksLikeRouteRestart,
  loadPendingIntake,
