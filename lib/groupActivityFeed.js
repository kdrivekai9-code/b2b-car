...
//
// **개인 딜러는 받지 않는다**(사용자 확정 2026-09-04). 법인 단위 옵트인이지만 딜러는 본인
// 오더만 보는 자격이라(lib/clientScope.js), 같은 법인이라는 이유로 남의 접수 내역을 보게 하면
// 오더 상세에 걸어둔 가림막이 이 피드로 새어버린다. 요약에는 출발·도착 주소와 연락처가
// 들어 있어 그 자체로 영업 정보다.
//
// 딜러가 **한 일**은 본사 직원에게 간다. 본사는 소속 딜러까지 보는 자격이므로(clientScope와
// 같은 규칙) 막을 이유가 없다. 막는 것은 수신뿐이다.
const db = require('../db');
...
async function notifyWebMembers(groupId, actorUserId, text) {
  ...
  try {
    targets = await db.all(
      // client_type='dealer'는 제외한다. 값이 비어 있는 기존 계정은 본사 직원으로 본다
      // (lib/clientScope.js normalizeClientType과 같은 판정) — 마이그레이션만으로 누군가가
      // 조용히 알림을 못 받게 되면 안 된다.
      `SELECT id, draft_json FROM chat_sessions
       WHERE channel = 'web' AND status <> 'closed'
         AND user_id IN (
           SELECT id FROM users
            WHERE group_id = ? AND status = 'active'
              AND COALESCE(NULLIF(client_type, ''), 'hq') <> 'dealer'
         )
  ...
  try {
    targets = await db.all(
      `SELECT DISTINCT ON (kca.external_user_key)
       ...
       LEFT JOIN users u ON u.id = kca.user_id
       WHERE kca.requester_group_id = ? AND kca.enabled = true AND kca.external_user_key IS NOT NULL
         -- 웹과 같은 규칙으로 개인 딜러를 뺀다. 매핑에 사용자가 안 걸린 채널 전체 매핑
         -- (u.id가 NULL)은 누가 붙을지 알 수 없으므로 보내지 않는다 — 딜러가 그 통로로
         -- 들어오면 막을 방법이 없다.
         AND u.id IS NOT NULL
         AND COALESCE(NULLIF(u.client_type, ''), 'hq') <> 'dealer'
...
//
// 다른 법인 소속도 막는다 — 여러 법인을 담당하는 계정이 A 법인 오더를 고치면 그 요약이
// A 법인 사람들에게 가는데, 그 계정은 A의 동료가 아니다.
async function isGroupMember(actorUserId, groupId) {
  if (!actorUserId) return false; // 시스템·크론·MCP 자동 처리 — 사람이 한 일이 아니다
  try {
    const u = await db.get('SELECT role, group_id FROM users WHERE id = ?', [actorUserId]);
    return !!(u && u.role === 'client' && Number(u.group_id) === Number(groupId));
  } catch (e) {
    // 확인이 안 되면 보내지 않는다. 안 보내서 생기는 손해(동료가 한 건을 못 봄)보다
    // 잘못 보내서 생기는 손해(고객에게 엉뚱한 통보)가 크다.
    console.error('법인 공유 피드 작성자 확인 실패(보내지 않음):', e.message);
    return false;
  }
}
...
async function recordActivity({ groupId, orderId, oid, kind, summary, actorUserId, actorLabel }) {
  ...
  if (!(await isGroupMember(actorUserId, groupId))) return;
  try {
...
module.exports = {
  isGroupMember, isFeedEnabled, recordActivity, listRecentActivity, KIND_LABELS };
