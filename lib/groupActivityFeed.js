...
async function notifyWebMembers(groupId, actorUserId, text) {
  ...
  for (const session of targets) {
    let draft = null;
    try { draft = session.draft_json ? JSON.parse(session.draft_json) : null; } catch (e) { draft = null; }
    if (draft && draft.phase === 'collecting' && draft.pendingField) continue; // 응답 대기 중 — 끼어들지 않음
    try {
...
async function notifyKakaoMembers(groupId, actorUserId, text) {
  ...
  for (const row of targets) {
    if (row.intake_slots_json || row.mcp_pending_json) continue; // 접수/도우미 확인 진행 중 — 끼어들지 않음
    const sessionShape = {
