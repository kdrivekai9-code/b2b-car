...
// 차종별 대형/화물 할증. 같은 차종을 두 번 넣으면 어느 금액이 맞는지 알 수 없어 첫 줄만 남긴다
// (DB에도 유니크 인덱스가 있지만, 화면에서 걸러야 저장 실패 대신 조용히 정리된다).
function parseLargeCarRows(body) {
  ...
  for (let i = 0; i < ids.length; i++) {
    ...
    seen.add(id);
    // 금액 0은 "이 차종은 대형 할증을 안 받는다"는 뜻이라 그대로 저장한다.
    rows.push({ vehicleModelId: id, fee: Math.max(0, Math.round(Number(fees[i]) || 0)), seq: rows.length + 1 });
  }
  return rows;
}
