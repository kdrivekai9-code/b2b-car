...
function settingsColumns(body) {
  ...
  EXTRA_COST_ITEMS.forEach((item) => {
    // 정산 방식은 새 컬럼(*_mode)에 저장한다 — 포함 / 실비 월정산 / 실비 개별정산 셋이라
    // 0·1로는 담을 수 없다. 옛 컬럼(*_included)도 함께 맞춰 둔다: 그 값을 읽는 옛 화면이나
    // 마이그레이션 전 경로가 남아 있어도 "포함/제외"만은 어긋나지 않게 한다.
    const mode = fareSurcharge.EXTRA_COST_MODES.some((m) => m.value === body[item.modeKey])
      ? body[item.modeKey]
      : null;
    if (mode) cols[item.modeKey] = mode;
    if (item.settingKey) {
      const legacy = mode
        ? (mode === 'included' ? 1 : 0)
        : (body[item.settingKey] === undefined
          ? (item.defaultIncluded ? 1 : 0)
          : (Number(body[item.settingKey]) === 1 ? 1 : 0));
      cols[item.settingKey] = legacy;
    }
  });
  return cols;
}
