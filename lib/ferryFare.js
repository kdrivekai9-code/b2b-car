...
// ---------------- 도착 예상 시각(다음 배편 조회) ----------------
// 노선의 육지 쪽 항구명 — ferry_schedules.origin_port(육지→제주 방향)와 일치해야 한다.
const MAINLAND_PORT_BY_ROUTE = {
  WANDO_JEJU: '완도항',
  SAMCHEONPO_JEJU: '삼천포신항',
};
const DAY_COLUMN_BY_DOW = ['runs_sun', 'runs_mon', 'runs_tue', 'runs_wed', 'runs_thu', 'runs_fri', 'runs_sat'];
// 항구 도착 후 실제 승선 수속/차량 하선 등을 감안한 여유시간 — 화면에는 별도로 표시하지 않고
// 최종 도착 예상 시각 결과 자체에만 조용히 반영한다.
const ARRIVAL_BUFFER_MINUTES = 30;

// dateStr/timeStr("YYYY-MM-DD"/"HH:mm")를 KST 벽시계 시각으로 보고, 그 값을 UTC getter로 그대로
// 읽을 수 있는 Date로 만든다(lib/period.js의 kstNow() 관례와 동일 — 서버 타임존과 무관하게 동작).
function parseKstMoment(dateStr, timeStr) {
  const d = String(dateStr || '').match(/^(\d{4})-(\d{2})-(\d{2})$/);
  const t = String(timeStr || '').match(/^(\d{1,2}):(\d{2})$/);
  if (!d || !t) return null;
  const moment = new Date(Date.UTC(Number(d[1]), Number(d[2]) - 1, Number(d[3]), Number(t[1]), Number(t[2]), 0));
  return Number.isNaN(moment.getTime()) ? null : moment;
}
function kstDateStr(moment) {
  return moment.toISOString().slice(0, 10);
}
function addMinutes(moment, minutes) {
  return new Date(moment.getTime() + Number(minutes || 0) * 60000);
}
function formatKstLabel(moment) {
  ...
  return `${String(month).padStart(2, '0')}월 ${String(day).padStart(2, '0')}일 ${hh}시 ${mm}분`;
}

async function isSailingActiveOnDate(row, dateStr) {
  try {
    const exception = await db.get(
      'SELECT runs FROM ferry_schedule_exceptions WHERE route_code = ? AND exception_date = ? AND (ship_name IS NULL OR ship_name = ?)',
      [row.route_code, dateStr, row.ship_name]
    );
    if (exception) return !!Number(exception.runs);
  } catch (e) {
    if (!e || e.code !== '42P01') throw e;
  }
  const dow = new Date(`${dateStr}T00:00:00Z`).getUTCDay();
  const col = DAY_COLUMN_BY_DOW[dow];
  return row[col] == null ? true : !!Number(row[col]);
}

// departureFromOrigin: 출발지에서 실제로 출발하는 시각(KST moment, parseKstMoment 결과).
// beforeMinutes: 출발지→항구 이동 소요(분). 항구 도착 시각 이후 가장 빠른 운항편을 찾는다.
async function findNextSailing(routeCode, departureFromOrigin, beforeMinutes) {
  const mainlandPort = MAINLAND_PORT_BY_ROUTE[routeCode];
  if (!mainlandPort || !departureFromOrigin || !Number.isFinite(Number(beforeMinutes))) return null;

  const terminalArrival = addMinutes(departureFromOrigin, beforeMinutes);

  let rows;
  try {
    rows = await db.all(
      `SELECT * FROM ferry_schedules WHERE route_code = ? AND origin_port = ? AND is_active = 1 ORDER BY departure_time`,
      [routeCode, mainlandPort]
    );
  } catch (e) {
  ...
  // 최대 4일 뒤까지만 훑는다 — 그 안에 맞는 편이 없으면 스케줄 데이터가 잘못됐다고 보고 포기한다.
  for (let dayOffset = 0; dayOffset < 4; dayOffset += 1) {
    const candidateDate = new Date(terminalArrival.getTime() + dayOffset * 86400000);
    const dateStr = kstDateStr(candidateDate);
    const candidates = [];
    for (const row of rows) {
      const active = await isSailingActiveOnDate(row, dateStr);
      if (!active) continue;
      const depMoment = parseKstMoment(dateStr, row.departure_time);
      if (!depMoment) continue;
      if (dayOffset === 0 && depMoment.getTime() < terminalArrival.getTime()) continue; // 이미 지난 편
      candidates.push({ row, depMoment });
    }
    if (candidates.length) {
      candidates.sort((a, b) => a.depMoment.getTime() - b.depMoment.getTime());
      const picked = candidates[0];
      const durationMin = Number(picked.row.duration_minutes) || 0;
      return {
        shipName: picked.row.ship_name,
        boardingAt: picked.depMoment,
        portArrivalAt: addMinutes(picked.depMoment, durationMin),
        terminalArrivalAt: terminalArrival,
      };
    }
  }
  return null;
...
// 출발지 출발 시각 + 육로1 + 다음 배편(대기 포함) + 육로2 + 여유시간(비공개)까지 계산한
// 최종 도착 예상 시각. reservedDate/reservedTime은 "출발지에서 실제 출발하는 시각" 기준이다.
async function estimateFerryArrival(options = {}) {
  const routeCode = options.routeCode;
  const departureFromOrigin = parseKstMoment(options.reservedDate, options.reservedTime);
  const beforeMinutes = Number(options.beforeMinutes);
  const afterMinutes = Number(options.afterMinutes);
  if (!departureFromOrigin || !Number.isFinite(beforeMinutes) || !Number.isFinite(afterMinutes)) return null;

  const sailing = await findNextSailing(routeCode, departureFromOrigin, beforeMinutes);
  if (!sailing) return null;

  const finalArrivalAt = addMinutes(addMinutes(sailing.portArrivalAt, afterMinutes), ARRIVAL_BUFFER_MINUTES);
  return {
    shipName: sailing.shipName,
    boardingLabel: formatKstLabel(sailing.boardingAt),
    portArrivalLabel: formatKstLabel(sailing.portArrivalAt),
    finalArrivalLabel: formatKstLabel(finalArrivalAt),
  };
}
