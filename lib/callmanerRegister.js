...
async function registerOrderWithCallmaner(orderId, branchId) {
  try {
    ...
    // 콜마너가 고객용 위치조회 링크를 주면 남긴다. 지금까지 이 값은 읽히기만 하고 버려졌다
    // (lib/callmaner.js orderReceipt의 webUrl). 링크는 배차기사 현위치 전문이 빈 값을 줄 때
    // 고객에게 위치를 보여줄 유일한 통로일 수 있다.
    await saveCallmanerWebUrl(orderId, result.webUrl);
  } catch (e) {
...
// 링크는 부가정보다 — 칸이 없거나(마이그레이션 전) 저장에 실패해도 접수를 되돌리지 않는다.
async function saveCallmanerWebUrl(orderId, webUrl) {
  const url = String(webUrl || '').trim();
  if (!url) return false;
  const ok = await db.run('UPDATE orders SET callmaner_web_url = ? WHERE id = ?', [url, orderId])
    .then(() => true)
    .catch((e) => {
      if (!e || e.code !== '42703') console.error('콜마너 위치조회 링크 저장 실패:', e.message);
      return false;
    });
  // 한 번도 온 적이 없는 값이다 — 처음 오면 눈에 띄어야 한다.
  if (ok) console.log(`[콜마너] 고객용 위치조회 링크 수신 (order=${orderId}): ${url}`);
  return ok;
}

module.exports = { registerOrderWithCallmaner, tryUpdateWithErrorCodeColumn, saveCallmanerWebUrl };
