...
const { needsDateReask, buildDateQuestion, applyDateAnswer, buildRetryQuestion } = require('./reservationReask');
const { applyCorrection } = require('./intakeCorrection');
...
async function runWebIntakeTurnCore({ user, session, text }) {
  ...
  // 예약일을 다시 묻는 중이면 이 답은 보충 정보가 아니라 "날짜"다 — 원문에 이어붙여 다시
  // 파싱하면 옛 날짜 줄이 또 읽혀 같은 질문이 반복된다(lib/reservationReask.js 머리말).
  if (pending && pending.awaiting === 'reserved_date') {
    return handleReservedDateReply({ user, session, pending, text: raw });
  }
  ...
  if (pending && pending.awaiting === 'confirm') {
    ...
    // 그 외 답변은 "수정"이다.
    //
    // 이어붙여 다시 파싱하지 않는다. 그 방식은 **보충**에는 맞지만 **정정**에는 안 맞는다 —
    // 지시문("~로 변경해줘")이 값으로 흡수되고, 나중 값이 앞 값을 덮는 규칙이 없어 먼저 나온
    // 날짜가 그대로 이긴다. 실사용에서 "예약일을 2026년 9월 8일로 변경해줘"가 도착지 칸에
    // 들어가고 예약일은 2027-07-27 그대로였다(2026-09-08).
    //
    // 대신 지금 확정된 슬롯과 이번 발화를 함께 주고 **바뀐 항목만** 받아 얹는다.
    const corrected = await applyCorrection(pending.parsed, raw);
    if (corrected.ok) {
      // raw는 원문 그대로 둔다 — 정정 결과는 parsed에 들어갔고, 지시문을 raw에 남기면
      // 나중에 어떤 경로로든 재파싱될 때 그 문장이 다시 값으로 흡수된다.
      return finishParsed({ session, parsed: corrected.parsed, mergedRaw: pending.raw });
    }
    // 정정으로 못 읽었으면(질문·새 접수·모델 실패) 기존 공통 경로가 받는다.
  }
...
async function handleReservedDateReply({ user, session, pending, text }) {
  const parsed = pending.parsed;
  if (!parsed) {
    await clearPendingIntake(session);
    return { replyText: '접수 내용을 다시 확인할 수 없습니다. 처음부터 다시 알려주세요.', ok: true, awaitingConfirmation: false };
  }

  const applied = applyDateAnswer(parsed.when, text);
  if (!applied.ok) {
    // 못 알아들었으면 상태를 유지해 한 번 더 묻는다 — 여기서 상태를 지우면 다음 답이 새 접수로
    // 재분류돼, 방금까지 다 채운 내용이 통째로 사라진다.
    const extra = { awaiting: 'reserved_date', parsed };
    if (pending.category === 'premium_daily') {
      extra.category = 'premium_daily';
      extra.orderType = pending.orderType;
    }
    await savePendingIntake(session, pending.raw || '', [], extra);
    return { replyText: buildRetryQuestion(applied), ok: true, awaitingConfirmation: false, parsed };
  }

  parsed.when = applied.when;
  const mergedRaw = pending.raw || '';
  if (pending.category === 'premium_daily') return finishParsedPremium({ session, parsed, mergedRaw });
...
async function finishParsed({ session, parsed, mergedRaw, cache }) {
  ...
  // 지나간 날짜면 확인을 받기 전에 날짜부터 다시 묻는다 — 요약에 밀린 날짜를 얹어 "네"를
  // 받으면 고객은 자기가 적은 날짜가 그대로 있는 줄 안다(카카오와 같은 관문).
  const reask = needsDateReask(parsed.when);
  if (reask.ask) {
    await savePendingIntake(session, mergedRaw, [], { awaiting: 'reserved_date', parsed });
    return { replyText: buildDateQuestion(reask, parsed.when), ok: true, awaitingConfirmation: false, parsed };
  }
...
async function finishParsedPremium({ session, parsed, mergedRaw, cache }) {
  ...
  if (reaskPremium.ask) {
    await savePendingIntake(session, mergedRaw, [], {
      awaiting: 'reserved_date', category: 'premium_daily', orderType: parsed.orderType, parsed,
    });
    return { replyText: buildDateQuestion(reaskPremium, parsed.when), ok: true, awaitingConfirmation: false, parsed };
  }
