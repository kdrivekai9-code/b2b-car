...
// 사진 순번과 항목 이름.
//
// **찍는 순서와 등록되는 순번이 다르다.** 콜마너 기사 앱의 촬영 순서는 운행완료 때
// 계기판이 맨 앞이지만(사용자 확정), 콜마너가 우리에게 넘기는 링크 목록에서 계기판은
// 운행전과 똑같이 **맨 뒤(13번)**다. 즉 등록 순번은 두 단계가 같다.
//
// 근거는 실물 대조다(OID2075 운행후 13장을 내려받아 눈으로 확인, 2026-09-09):
// 1번 = 아우디 전면 그릴(번호판 248호9416), 5번 = 앞바퀴, 13번 = 계기판(속도계·주행거리).
//
// 전에는 촬영 순서를 등록 순번으로 믿고 운행후 표를 계기판부터 적었다. 그러면 항목 이름이
// 한 칸씩 밀려서, 이름으로 짝을 짓는 화면(pairByPhase)이 **전면 사진을 계기판 자리(맨 뒤)에**
// 놓았다 — 사용자가 본 증상이 이것이다. 계기판 인식도 전면 그릴을 읽으려 해서 실패했다
// (end/seq1 ocr_status='failed').
//
// 두 단계가 같은 순번을 쓰지만 표는 단계별로 남겨둔다 — 콜마너가 한쪽만 바꿀 수 있고,
// 그때 이 자리만 고치면 되도록.
const PHOTO_REGISTER_ORDER = [
  '전면', '보조석 측면', '운전석 측면', '운전석 옆면', '운전석 앞바퀴', '운전석 뒷바퀴',
  '뒷면', '운전석 뒤 측면', '보조석 뒤 측면', '보조석 옆면', '보조석 뒤 바퀴',
  '보조석 앞 바퀴', '계기판',
];
const PHOTO_ITEMS = {
  [PHASE_START]: PHOTO_REGISTER_ORDER,
  [PHASE_END]: PHOTO_REGISTER_ORDER,
};
...
// 계기판 사진의 순번. 두 단계가 같지만(위 표) phase를 계속 받는다 — 콜마너가 한쪽만
// 바꾸면 여기 손대지 않고 표만 고쳐서 끝나야 한다.
//
// branches.odometer_photo_index 재정의는 "우리 지사는 계기판이 N번"이라는 등록 순번에 대한
// 말이므로 두 단계에 함께 적용한다(현재 두 지사 모두 13 = 표와 같은 값).
...
async function syncOdometer(odometerOcr, branch, order, options = {}) {
  ...
  // 두 값이 다 읽혔는데 거리가 안 나오면(완료값이 시작값보다 작거나 터무니없이 큼) 둘 중
  // 하나를 잘못 읽은 것이다. 어느 쪽인지 모르므로 **완료값을 내보내지 않는다** —
  // odometer_end는 알림톡 변수로 고객에게 그대로 나간다(lib/kakaoOrderNotify.js).
  //
  // 실측 2026-09-09(OID2075): 시작 128422, 완료 128466이 맞는데 어두운 계기판에서 8을 6으로
  // 읽어 126466이 나왔다. distance_total은 음수라 걸러졌지만 완료값은 그대로 저장됐다.
  let publishedEnd = endKm;
  if (startKm !== null && endKm !== null && total === null) {
    publishedEnd = null;
    console.error(`계기판 값이 서로 안 맞음 (oid=${order.oid}): 시작 ${startKm} / 완료 ${endKm} — 완료값을 쓰지 않는다`);
    logIntegrationErrorAsync({
      source: 'callmaner', operation: 'odometer_conflict', refType: 'order', refId: order.id,
      message: `계기판 값 모순: 시작 ${startKm}km / 완료 ${endKm}km`,
      context: { oid: order.oid, startKm, endKm },
    });
  }

  await db.run(
    `UPDATE orders SET odometer_start = ?, odometer_end = ?, distance_total = ? WHERE id = ?`,
    [startKm, publishedEnd, total, order.id]
  ).catch((e) => {
    // 컬럼이 없는 DB(마이그레이션 전)에서는 조용히 넘어간다 — 주행거리 하나 때문에 동기화가
    // 막히면 안 된다.
    if (!e || e.code !== '42703') throw e;
  });
  return { startKm, endKm: publishedEnd, total, conflicted: publishedEnd === null && endKm !== null };
}
