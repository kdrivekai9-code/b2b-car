...
// 사진 순번이 무엇을 찍은 것인지.
//
// 콜마너 촬영 순서는 고정이라 순번만으로 항목을 알 수 있다. 화면에 "1, 2, 3"만 보여주면
// 운행전·운행후를 나란히 놓고도 무엇과 무엇을 비교하는 건지 알 수 없다.
//
// **확인된 것만 적는다.** 1번(전면)은 번호판 대조가 쓰고 있고(DEFAULT_PLATE_PHOTO_INDEX),
// 13번(계기판)은 주행거리 판독이 쓰고 있다 — 둘 다 실제 동작으로 검증된 자리다. 모르는
// 자리에 그럴듯한 이름을 넣으면 "운전석 휠"이라고 적힌 자리에 뒷범퍼 사진이 뜨고, 사고
// 처리에서 그 이름을 근거로 다투게 된다.
//
// 아래 이름은 **실제 사진을 두 오더에서 눈으로 대조해** 채웠다(2026-09-08, OID1455·OID2075의
// 운행전 13장 전부). 두 오더가 같은 것을 보여준 자리만 적었다:
//   1 전면(번호판) · 2·3 앞 사선 전경 · 5·6 휠 근접 · 7 후면(번호판) · 8·9 뒤 사선 전경 ·
//   10 측면 전체 · 11·12 휠 근접 · 13 계기판
// 4번은 두 오더가 달라 보였다(한쪽은 사선 전경, 한쪽은 문 근접) — 그래서 비워 "4번"으로 둔다.
//
// **좌/우(운전석·조수석)는 적지 않는다.** 사진만으로는 어느 쪽인지 가릴 수 없다. 휠 네 장의
// 순서가 앞→뒤인지 좌→우인지도 알 수 없어, 그걸 이름에 넣으면 틀린 자리를 근거로 다투게 된다.
// 그 구분이 필요하면 콜마너에서 촬영 순서 정의를 받아야 한다.
// (이 자리에 예전에 '2: 운전석 휠'이 적혀 있었는데, 2번은 휠이 아니라 차량 앞 사선 전경이었다 —
//  두 오더 모두. 사진을 안 보고 넣은 이름이 실제로 틀려 있었다는 뜻이다.)
//
// 콜마너가 순서를 바꾸면 여기도 바꿔야 한다. 지사별로 다르면 branches에 칸을 두는 방식으로
...
function photoLabel(seq) {
  const n = Number(seq);
  const name = PHOTO_LABELS[n];
  return name ? `${n}. ${name}` : `${n}번`;
}

// 운행전·운행후를 순번으로 짝지어 나란히 놓는다.
//
// 같은 자리를 비교해야 무엇이 달라졌는지 보인다 — 흠집이 언제 생겼는지가 사고 처리의 전부다.
// 한쪽만 있는 순번도 자리를 남긴다. 빠진 것이 보여야 "안 찍었다"를 알 수 있다.
function pairByPhase(photos) {
  const rows = Array.isArray(photos) ? photos : [];
  const seqs = [...new Set(rows.map((p) => Number(p.seq)).filter(Number.isFinite))].sort((a, b) => a - b);
  return seqs.map((seq) => ({
    seq,
    label: photoLabel(seq),
    start: rows.find((p) => p.phase === PHASE_START && Number(p.seq) === seq) || null,
    end: rows.find((p) => p.phase === PHASE_END && Number(p.seq) === seq) || null,
  }));
}
...
module.exports = {
  ...
  PHOTO_LABELS, photoLabel, pairByPhase,
  PHASE_START,
  PHASE_END,
  resolveOdometer,
  syncOdometer,
