...
- **주유비만** 금액을 받습니다. "3만원 주유해 주세요"처럼 고객이 정한 금액이면
  amount에 숫자만 넣으세요("3만원" → 30000). "가득"이면 0입니다 — 접수 때는 금액을 모릅니다.
- 충전·세차·주차는 amount를 항상 0으로 두세요. 본문에 금액이 적혀 있어도 그렇습니다 —
  얼마가 될지는 기사가 쓰고 영수증을 올려야 정해지는 실비입니다.
...
async function detectFromMemo(memo, feeExtra, options = {}) {
  ...
  out.items.forEach((raw) => {
    ...
    seen.add(info.code);
    // 금액은 주유비만 받는다. 지시로 막아도 모델이 넣을 수 있으므로 여기서 눌러버린다 —
    // 어림값이 영수증 없이 청구되는 것이 금액 없이 남는 것보다 나쁘다.
    // 접수 화면과 같은 규칙이다(lib/extraCharges.js INTAKE_EXTRA_ITEMS의 fixedAmount).
    const item = extraCharges.intakeItem(info.chargeType);
    const canFixAmount = !!(item && item.fixedAmount);
    rows.push({
      ...info,
      evidence: String((raw && raw.evidence) || '').trim().slice(0, 200),
      amount: canFixAmount ? Math.max(0, Math.round(Number(raw && raw.amount) || 0)) : 0,
    });
  });
  return rows;
}
...
// 고객에게 되돌려 보여줄 것 — 아직 판단하지 않은 후보만, 청구 이야기는 빼고.
//
// 왜 보여주나: 고객이 요청사항에 "주유 3만원"이라고 쓰면 지금은 아무 반응이 없다. 접수는
// 됐는데 그 말을 우리가 알아들었는지 알 길이 없어서 전화가 온다 — 이 채널이 없애려는 바로
// 그 통화다.
//
// 왜 부대비용 항목에 안 넣나: 그 자리는 고객이 **직접 고른 것**이 들어가는 곳이다. 추측을
// 섞으면 등록된 것으로 읽히고, 관리자가 기각하면 봤던 것이 사라진다. 없던 걸 보여줬다가
// 치우는 것이 처음부터 안 보여주는 것보다 나쁘다.
//
// 근거 원문을 함께 준다. 그게 있어야 고객이 "내가 쓴 게 이렇게 읽혔구나"를 알고, 틀렸으면
// 그 자리에서 잡는다 — 3만원을 30만원으로 읽었는지는 관리자보다 고객이 더 잘 안다.
//
// 정산구분(월정산·개별정산)은 빼고 준다. 계약 사항이라 고객이 정할 것이 아니고, 확정도 안 된
// 후보에 청구 방식까지 붙이면 이미 청구가 시작된 것처럼 읽힌다.
function customerViewFromOrder(order) {
  return pendingFromOrder(order).map((c) => ({
    label: c.label,
