...
function evaluateDispatchDelay({ st, statusCode, matched, 접수시각, 예약시각 }, nowMs) {
  ...
  if (elapsedMin < DELAY_OFFER_MINUTES) {
    return { 지연: false, 사유: `접수 후 ${elapsedMin}분 경과(기준 ${DELAY_OFFER_MINUTES}분)` };
  }
...
async function checkDispatchDelay({ user, sessionId }) {
  ...
  for (const row of labeled) {
    ...
    if (!row.배차지연) continue;
    if (!row.요금조정가능) continue;
    if (await alreadyOfferedRaise(ctx, row.도구용접수번호)) continue;

    const prepared = await prepareMutation(ctx, sessionId, 'raise_fare', { rcptNo: row.도구용접수번호 });
