...
async function prepareMutation(ctx, sessionId, toolName, args) {
  ...
  if (toolName === 'update_order' || toolName === 'cancel_order') {
    ...
    if (args.vehicleNumber || args.vehicleType) {
      const split = splitTypeAndPlate(args.vehicleType || null, args.vehicleNumber || null);
      // 번호판을 고치겠다고 했는데 형식이 안 잡히면(오타·자릿수 부족) 조용히 넘기지 않는다 —
      // 잘못된 번호가 오더에 박히면 현장에서 다른 차를 가져가는 사고가 된다.
      if (args.vehicleNumber && !split.vehicleNumber) {
        return { ok: false, error: '차량번호 형식을 알아보지 못했습니다. "335모6328"처럼 번호판을 다시 확인해 달라고 고객에게 물어보세요.' };
      }
      if (split.vehicleNumber) localChanges.vehicleNumber = split.vehicleNumber;
