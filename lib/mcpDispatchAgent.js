...
function summarizeOrders(orders, oidMap) {
  return (orders || []).map((o) => {
    ...
    return {
      ...
      //
      // 왜: 이 목록은 콜마너 MCP 프록시(call.list.active)가 준 것인데, 그 값이 콜마너 본체보다
      // 뒤처진다. 실측(2026-08-25):
      //   · OID1455 — 콜마너 단건조회는 wk_info "T11111*채정식"(기사 배정)인데 MCP는
      //     st:"scheduled", driver.matched:false. 우리 DB는 '기사배정'이고 배차 통보까지 나갔다.
      //   · OID1459 — 우리는 '완료'로 알고 운행시작·운행완료 통보를 이미 보냈는데, MCP는 여전히
      //     진행 중 목록에 넣어 주고 "예약 (기사 미배정)"으로 보였다.
      //
      // 우리 sync(routes/callmanerSync.js)는 콜마너 단건조회(OrderInfo)를 직접 읽어 상태와 기사
      // 정보를 채운다. 같은 사실을 통보는 우리 값으로, 조회는 MCP 값으로 말하면 고객에게 두
      // 시스템이 다른 소리를 하는 셈이다 — 이미 "완료됐다"고 통보받은 건을 "예약"이라 답했다.
      ...
      ...(() => {
        // 배차 지연 판정도 같은 값을 봐야 한다 — MCP의 matched:false를 그대로 믿으면 이미
        // 기사가 붙은(심지어 완료된) 오더를 "배차 지연"으로 안내하고 요금 인상을 권한다
        // (실측: 운행완료된 OID1459가 지연 건으로 안내됐다).
