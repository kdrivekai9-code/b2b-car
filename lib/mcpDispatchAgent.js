...
- 최근 접수건의 실제 이용고객(출발지) 연락처: ${ctx.usageCids.length ? ctx.usageCids.join(', ') : '없음'}
...
- 주문을 안내할 때 접수번호는 도구가 돌려준 **표시번호 값**을 그대로 쓰세요. 다만 "표시번호"라는
  말 자체는 쓰지 말고 "접수번호 OID1132(179098847)"처럼 자연스럽게 쓰세요.
  우리 시스템 오더번호와 콜마너 접수번호를 함께 보여줘야 고객이 자기 주문을 찾을 수 있습니다.
  단, 도구를 호출할 때 rcptNo에는 표시번호가 아니라 **접수번호(숫자만)** 를 넣으세요.
- 주문을 안내할 때는 표시번호, 출발지→도착지, 접수시각/예약시각, 상태(기사 배정 여부), 요금을 담으세요.
- 조회에 사용한 연락처(전화번호)는 답변에 그대로 쓰지 마세요 — 필요하면 "실제 이용 고객" 정도로만
  표현하세요. 고객이 직접 번호를 물어본 경우가 아니면 번호를 노출하지 않습니다.
...
function summarizeOrders(orders, oidMap) {
  return (orders || []).map((o) => {
    ...
    return {
      표시번호: mine && mine.oid ? `${mine.oid}(${rcptNo})` : rcptNo,
      접수번호: rcptNo, // 도구 호출에는 반드시 이 값을 쓴다
      우리오더번호: (mine && mine.oid) || null,
      출발지: place(o.departure),
      도착지: place(o.arrival),
      접수시각: o.requestedAt || (mine && mine.created_at) || null,
      예약시각: o.scheduledAt || null,
...
async function runReadTool(ctx, sessionId, toolName, args) {
  ...
  if (toolName === 'get_my_orders') {
    ...
    for (const cid of candidates) {
      const [profile, active] = await Promise.all([
        mcp.callTool('cust.get', { repNo: ctx.repNo, cid }),
      ...
      if (!profile.ok && !active.ok) {
        lastError = profile.error || active.error;
        continue;
      }
      const customer = (profile.data && profile.data.customer) || null;
      const orders = (active.data && active.data.orders) || [];
      const result = {
        ok: true,
        cid,
        조회순서: cid === ctx.primaryCid ? '접수자 본인 번호(2차)' : '실제 이용고객 번호(1차)',
        customer: customer ? {
        ...
        activeOrders: await withOidLabels(orders),
      };
      // 진행 중 주문이 있으면 그 결과를 그대로 쓰고, 고객은 있는데 주문이 없으면 다음 후보도
      // 마저 확인한 뒤(없으면) 이 결과를 돌려준다.
      if (orders.length) return { ...result, triedCids: tried };
      if (!profileOnly) profileOnly = result;
    }
    if (profileOnly) return { ...profileOnly, triedCids: tried };
    return {
      ok: false,
      error: lastError,
  ...
  if (toolName === 'get_order_history') {
    ...
    let lastError = null;
    for (const cid of candidates) {
      const callArgs = { repNo: ctx.repNo, cid, page: 1, pageSize: Math.min(Number(args.pageSize) || 10, 30) };
      ...
      tried.push(cid);
      if (!out.ok) { lastError = out.error; continue; }
      const orders = (out.data && out.data.orders) || [];
      if (orders.length || cid === candidates[candidates.length - 1]) {
        return {
          ok: true, cid,
          조회순서: cid === ctx.primaryCid ? '접수자 본인 번호(2차)' : '실제 이용고객 번호(1차)',
          orders: await withOidLabels(orders),
          triedCids: tried,
        };
      }
    }
...
function describeExecution(pending, result) {
  ...
  if (pending.mcpTool === 'call.update') {
    ...
    const lines = [`접수번호 ${(pending.callArgs && pending.callArgs.orderId) || ''} 주문을 변경했습니다.`];
