...
async function runDispatchAgent({ user, sessionId, text, history, viewerCid, requesterGroupId, speculative }) {
  if (!mcp.isConfigured()) {
  ...
  // 위 주석 (1) — 확인 대기 중이면 투기 실행은 물러난다.
  if (speculative && listState) return { handled: false, reason: 'speculative_pending' };
