  ...
  if (toolName === 'get_order_history') {
    ...
    for (const cid of candidates) {
      ...
      if (orders.length || cid === candidates[candidates.length - 1]) {
        return { ok: true, cid, 조회순서: cid === ctx.primaryCid ? '접수자 본인 번호(2차)' : '실제 이용고객 번호(1차)', orders, triedCids: tried };
      }
    }
    return {
      ok: false,
...
function describeExecution(pending, result) {
  ...
  if (pending.mcpTool === 'call.create') {
    ...
    // 예약으로 요청했는데 콜마너가 즉시 호출로 접수하는 경우가 실측으로 확인됐다(scheduledAt 무시).
    // 고객에게 "예약됐다"고 잘못 안내하지 않도록 그대로 알리고 상담원 확인을 유도한다.
    if (pending.callArgs && pending.callArgs.scheduledAt && data.serviceType === 'immediate') {
