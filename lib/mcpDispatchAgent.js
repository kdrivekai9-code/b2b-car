...
const { recordActivity: recordGroupActivity } = require('./groupActivityFeed');
const { lookupRegion, lookupRoadAddress } = require('./kakaoRegion');
const { splitTypeAndPlate, findPlate } = require('./vehicleInfo');
...
// 전화번호 하이픈 형식은 접수 파서와 같은 함수를 쓴다 — 규칙이 두 벌이 되면 화면마다 달라진다.
const { normalizePhone } = require('./kakaoIntakeParser');
// routes/orders.js가 export하는 updateOrderWithCallmaner를 재사용한다.
// 주의: routes/orders.js도 이 파일을 쓴다(의도분류와 함께 도우미를 미리 돌리는 경로). 그쪽은
// 그래서 최상단이 아니라 쓰는 순간에 require한다 — 양쪽 다 최상단에서 부르면 순환참조가 되어
// 여기서 undefined를 잡는다(실제로 그렇게 만들었다가 경고로 잡았다).
...
const TOOL_DECLARATIONS = [
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        dateBasis: {
          type: 'STRING',
          description: '기간을 무엇으로 셀지. "예약/출발/운행" 기준이면 reserved(기본), "접수/등록/신청/넣은" 기준이면 created. 고객이 쓴 말 그대로 고르세요 — 두 기준의 결과가 다릅니다.',
        },
  ...
  {
    name: 'update_order',
    description: '진행 중인 배차 주문의 예약시각/경로/요금/메모/차량번호를 변경합니다. 대상은 접수번호(rcptNo)로 지정하며, 사용자 확인을 한 번 받습니다.',
    parameters: {
      type: 'OBJECT',
      properties: {
        ...
        vehicleNumber: { type: 'STRING', description: '정정할 차량번호(번호판). 예: "335모6328"' },
        vehicleType: { type: 'STRING', description: '정정할 차종. 예: "코란도"' },
      },
      required: ['rcptNo'],
    },
  },
...
- 기간 조회(get_order_history)는 **고객이 쓴 말대로 기준을 고르세요**(dateBasis).
  "오늘 접수한 건 / 등록한 건 / 넣은 건" → created. "오늘 예약건 / 출발하는 건 / 운행" → reserved.
  어느 쪽인지 알 수 없으면 reserved입니다. 접수한 날과 예약한 날은 다를 수 있어(어제 접수한
  오늘 예약 건, 오늘 접수한 내일 예약 건) 기준을 잘못 고르면 다른 주문이 나옵니다.
...
- **물어본 것에만 답하세요.** 고객이 "내일 예약건"을 물으면 내일 것만 답합니다. 오늘 건이나
  다른 기간 건을 곁들이지 마세요. 도구 결과에 "기간밖제외"가 들어 있어도 그 수를 언급하지
  마세요 — 묻지 않은 정보이고, 답이 길어지면 정작 물어본 내용이 묻힙니다.
- 딱 하나 예외가 있습니다. 도구 결과에 **"다른기준"**이 들어 있으면 그건 같은 기간을 다른
  방식으로 센 결과입니다(같은 날의 접수 기준 ↔ 예약일 기준). 물어본 기준의 결과를 **먼저**
  답하고, 그 아래에 한 줄 띄워 그 기준 이름과 함께 덧붙이세요.
  **항목은 한 줄에 하나씩** 적으세요(" / "로 이어 붙이지 마세요 — 한 줄이 길어지면 읽기
  어렵습니다). 이 예시의 줄바꿈 형태를 그대로 따르세요:
    오늘 예약된 주문은 총 1건입니다.
    ...
    접수시간: 2026-09-08 11:27
    예약시간: 2026-09-09 18:30
    출발지: … (010-****-1240)
    도착지: … (010-****-3523)
    상태: 대기, 요금: 0원
  없으면(다른기준이 없으면) 아무 말도 붙이지 마세요. 사람은 접수와 예약을 섞어 쓰기 때문에
  이렇게 두 묶음을 나눠 보여주는 것이고, 뭉쳐서 한 건수로 세면 안 됩니다.
- 기간 조회 결과를 안내할 때는 첫 줄에 **무엇을 기준으로 셌는지** 밝히세요("오늘 접수된",
  "오늘 예약된"). 도구가 준 "기준" 값이 그 말입니다.
- **답할 대상은 마지막 메시지 하나뿐입니다.** 앞선 대화에 답을 못 받은 질문이 남아 있어도
  거기까지 함께 답하지 마세요. 상담원이 응대하는 사이 고객이 여러 번 물어보면 답 없는 질문이
  줄줄이 쌓이는데, 그걸 한꺼번에 답하면 고객이 방금 물은 것에 대한 답을 찾기 어려워집니다.
- 목록을 안내할 때는 첫 줄에 **무엇을 세었는지**와 건수를 밝히세요 — "총 3건입니다"만 말하면
  고객은 그게 오늘 건인지 전체인지, 취소된 건도 포함인지 알 수 없습니다.
  고객이 기간을 지정했으면 그 기간으로 말하고("내일 예약된 주문은 총 1건입니다"),
  지정하지 않았으면 도구가 준 "조회조건"을 그대로 밝히세요.
- **접수일 기준으로 보여주는 주문에는 접수시간과 예약시간을 둘 다 넣으세요.** 접수시간을
  예약시간 **대신** 쓰면 안 됩니다 — 순서는 접수번호 → 접수시간 → 예약시간 → 출발지입니다.
  이 규칙은 물어본 기준이 접수일일 때(dateBasis=created)와, 예약일로 물었을 때 아래에
  덧붙는 "다른기준"(접수일) 묶음에 **똑같이** 적용됩니다.
  왜 둘 다인가: 접수시간만 보이면 그 주문을 언제 수행하는지 알 수 없고(정작 필요한 값이다),
  예약시간만 보이면 그 건이 왜 오늘 것으로 세어졌는지 알 수 없습니다.
  도구가 "접수시각"과 "예약시각"을 각각 줍니다.
...
- **상태가 배차·기사배정·운행시작·완료면 기사 이름과 연락처를 상태 바로 아래 줄에 적으세요.**
  형식은 "기사: {기사명} {기사연락처}" 한 줄입니다(예: "기사: 채정식 010-5083-2243").
  그 상태에서 고객이 가장 먼저 찾는 값입니다 — 없으면 "기사님 연락처요"를 한 번 더 묻게 됩니다.
  도구 결과에 "기사명"/"기사연락처"가 없으면 아직 배차 전이라는 뜻이니 그 줄을 아예 빼세요
  (지어내지 마세요). 배차 전 상태(접수·대기·예약)에서도 이 줄을 넣지 마세요.
- **상세 정보를 물으면**("○○ 주문 상세정보", "자세히 알려줘") 항목을 한 줄씩 나열하되 순서를 지키세요.
  접수번호 → 예약시간 → 출발지 → 출발지 연락처 → 도착지 → 도착지 연락처 → 요청사항 → 상태 →
  기사(배차 이후에만) → 요금.
  **연락처는 각각 해당 장소 바로 아래**에 붙입니다(출발지 연락처를 도착지 아래에 쓰지 마세요).
  "도착지연락처"·"요청사항" 값이 없으면(null) 그 줄은 아예 빼세요 — "없음"이라고 쓰지 마세요.
- 고객이 목록의 순번으로 지목하면("1번 주문 상세정보", "두 번째 건", "첫번째 오더") **직전
  답변에서 그 순번에 해당하는 주문**을 뜻합니다. 접수번호를 다시 묻지 말고, 그 주문의
  접수번호로 도구를 호출해 최신 값을 확인한 뒤 상세를 안내하세요.
  **도구가 돌려주는 목록의 순서는 직전 답변의 순서와 다릅니다.** 순번은 반드시 당신이 방금
  보낸 답변에 적은 순서로 세세요 — 도구 결과의 첫 줄을 "첫 번째"로 착각하면 엉뚱한 주문의
  기사·연락처를 알려주게 됩니다(실사용에서 그렇게 틀렸습니다). 답변 첫머리에 그 주문의
  접수번호를 밝혀, 고객이 다른 주문을 뜻했다면 바로 잡을 수 있게 하세요.
...
// 기간을 무엇으로 셌는지 사람이 읽는 말. 조회조건 문구와 다른기준 블록이 같은 말을 쓴다.
const BASIS_LABELS = { reserved: '예약일', created: '접수일' };

// 기사가 붙은 뒤의 상태. 이 상태에서는 답변에 기사 이름·연락처를 함께 적는다
// (사용자 지정 2026-09-08: "상태가 배차/운행시작/완료면 상태 옆에 기사명·전화번호").
//
// 우리 상태('기사배정')와 콜마너 표기('배차')가 다르므로 둘 다 넣는다 — 한쪽만 보면
// 콜마너에만 있는 건에서 기사 줄이 빠진다.
const DRIVER_SHOWN_STATUSES = new Set(['배차', '기사배정', '운행시작', '완료']);

// "채정식 010-5083-2243" 한 줄. 카카오 항목표의 설명 칸이 23자라 이름+번호가 딱 들어간다
// (이름 6자 + 공백 + 안심번호 13자 = 20자). 둘 중 하나만 있으면 있는 것만 적는다.
function driverLine(order) {
  if (!order || !DRIVER_SHOWN_STATUSES.has(String(order.상태 || '').trim())) return '';
  const parts = [order.기사명, order.기사연락처].map((v) => String(v || '').trim()).filter(Boolean);
  return parts.join(' ');
}
...
function summarizeOrders(orders, oidMap) {
  return (orders || []).map((o) => {
    ...
    // 접수시각 — 우리 오더가 매칭되면 **우리 created_at을 먼저 쓴다**.
    //
    // 콜마너 MCP의 requestedAt은 예약 건에서 접수시각이 아니다(실측 2026-09-08):
    // OID2075(182353721)는 9/7 10:08에 접수됐는데 requestedAt은 9/8 09:00:01 —
    // 콜마너가 예약 건을 수행일 아침에 대기열로 올린 시각이다. 이 값으로 "오늘 접수한 건"을
    // 세면 어제 접수한 예약 건이 오늘 접수로 잡힌다.
    // 즉시 건은 두 값이 초 단위로 같아 우선순위를 바꿔도 달라지는 것이 없다.
    // 우리 오더가 없는 건(콜마너에서 직접 접수된 건)은 requestedAt밖에 없으므로 그대로 쓴다 —
    // 그 경우 예약 건의 접수일은 실제보다 늦게 보일 수 있다.
    const 접수시각 = formatDateTime((mine && mine.created_at) || o.requestedAt);
    ...
    return {
      ...
      접수시각,
      예약시각,
      // 상태와 기사배정은 우리 오더가 매칭되면 **우리 값을 쓴다**.
      //
      // 왜가 아닌 것부터: MCP 프록시가 콜마너 본체보다 **느려서가 아니다**. 같은 시각에 둘을
      // 나란히 부른 실측(2026-08-26)에서 값이 초 단위로 일치했다 —
      //   · OID1455: OrderInfo statusTime 20260825145001 / MCP updatedAt 2026-08-25T14:50:01
      //   · OID1459: MCP updatedAt 11:20:01, 우리 sync가 완료로 기록한 시각은 11:20:41 (MCP가 앞섰다)
      // 처음에 "MCP가 뒤처진다"고 적어뒀던 것은 서로 다른 시각에 찍은 스냅샷을 비교한 탓이다.
      //
      // 진짜 이유는 **투영하는 필드가 다르다**는 것이다. MCP의 driver.matched는 자기 상태코드에서
      // 파생된 값이라, 콜마너 상태가 '배차'로 바뀌어야 true가 된다. 그런데 예약 건은 기사가 붙어도
      // 콜마너 status가 한동안 '예약'에 머문다(OID1455: 기사는 오전에 붙었지만 '배차' 전환은
      // 14:50:01). 그 구간에서 기사 배정 사실을 담고 있는 필드는 OrderInfo의 wk_info뿐인데
      // MCP는 그 필드를 노출하지 않는다. 우리 sync(routes/callmanerSync.js)는 wk_info를 직접 읽으므로
      // 그 구간에서 우리 값이 더 이르고 더 정확하다.
      //
      // 기사 이름도 마찬가지다. MCP가 배정 후 주는 driver.name은 "배정 기사"라는 자리표시자이고,
      // 실명(채정식)은 wk_info를 읽는 우리 DB에만 있다.
      //
      // 덧붙여, 같은 사실을 통보는 우리 값으로 조회는 MCP 값으로 말하면 두 시스템이 고객에게 서로
      // 다른 소리를 하게 된다. 어느 쪽이 빠른지와 무관하게 한쪽으로 맞추는 편이 옳다.
      상태: (mine && mine.status) || ORDER_STATE_LABELS[o.st] || o.st || null,
      요금: o.fare != null ? o.fare : null,
      기사배정: mine ? hasOurDriver(mine) : !!(o.driver && o.driver.matched),
      ...
      ...(() => {
        ...
        // 하이픈을 넣어 준다. 콜마너가 주는 안심번호는 "05083246939"처럼 붙어 있어 읽기도
        // 어렵고, 카카오톡에서 번호로 인식되지 않아 눌러서 걸 수도 없다.
        // 접수 파서와 같은 함수를 쓴다 — 형식 규칙이 두 벌이 되면 화면마다 달라진다.
        return { 기사명: name || null, 기사연락처: phone ? normalizePhone(phone) : null };
      })(),
      ...(() => {
        // 배차 지연 판정도 같은 값을 봐야 한다 — 위에서 적은 대로 예약 건은 기사가 붙어도
        // MCP가 matched:false를 준다. 그걸 그대로 믿으면 이미 기사가 배정된 오더를 "배차 지연"으로
        // 안내하고 요금 인상까지 권하게 된다.
        const verdict = evaluateDispatchDelay({
          st: o.st, statusCode: o.statusCode,
          matched: mine ? hasOurDriver(mine) : !!(o.driver && o.driver.matched),
          접수시각, 예약시각,
        });
...
// 우리 DB 기준으로 기사가 붙었는지. 동기화가 이름 또는 연락처 중 하나만 받아오는 구간이 있어
// (연락처는 별도 API로 한 박자 늦게 온다) 둘 중 하나라도 있으면 배정된 것으로 본다.
function hasOurDriver(mine) {
  return !!(mine && (String(mine.callmaner_driver_name || '').trim() || String(mine.callmaner_driver_phone || '').trim()));
}

// 우리 기준으로 이미 끝난 건. "진행 중" 목록에서 빼는 데 쓴다.
const TERMINAL_LOCAL_STATUSES = new Set(['완료', '취소']);
...
async function runReadTool(ctx, sessionId, toolName, rawArgs) {
  ...
  if (toolName === 'get_my_orders') {
    ...
    if (anyOk) {
      const labeledAll = await withOidLabels(merged);
      labeledAll.forEach((row, i) => {
        const masked = access.maskPhone(merged[i].__cid);
        row.이용고객 = masked;
        if (!row.출발지연락처) row.출발지연락처 = masked;
      });
      // 우리가 이미 끝난 것으로 아는 건은 "진행 중" 목록에서 뺀다. 콜마너 MCP 프록시가 완료된
      // 오더를 계속 진행 중으로 돌려주는 것이 실측됐다(OID1459 — 운행완료 통보까지 보낸 뒤에도
      // 목록에 남아 "예약 (기사 미배정)"으로 보였다). 통보와 조회가 다른 말을 하면 안 된다.
      const labeled = labeledAll.filter((row) => !TERMINAL_LOCAL_STATUSES.has(String(row.상태 || '')));
      return {
        ok: true,
  ...
  if (toolName === 'get_order_history') {
    ...
    const basis = String(args.dateBasis || '').trim() === 'created' ? 'created' : 'reserved';
    const otherBasis = basis === 'created' ? 'reserved' : 'created';
    const pageSize = Math.min(Number(args.pageSize) || 10, 30);
    // 기간을 지정했으면 최대치로 받아온다.
    //
    // 콜마너 MCP의 call.list.history는 startDate/endDate를 **무시한다**(실측 2026-09-08:
    // 09-08 / 09-07 / 08-25 하루짜리 세 범위에 모두 같은 9건이 돌아왔고, 그중 그 날짜에
    // 해당하는 건은 하나도 없었다). 즉 날짜로 걸러내는 일은 전부 아래 우리 코드가 한다.
    // 그런데 목록은 최신순으로 pageSize만큼만 오므로, 10건만 받으면 그 뒤의 기간 내 주문이
    // 조회에서 통째로 빠진다 — 있는 주문이 없는 것으로 답하게 된다.
    const fetchSize = (args.startDate || args.endDate) ? 30 : pageSize;
    const perCid = await Promise.all(candidates.map(async (cid) => {
      const callArgs = { repNo: ctx.repNo, cid, page: 1, pageSize: fetchSize };
    ...
    if (anyOk) {
      ...
      // 기간을 지정한 조회면 기간 밖의 건은 뺀다.
      //
      // 진행 중 목록(call.list.active)에는 날짜 조건이 안 걸린다 — 오늘 접수한 건이 이력에
      // 아직 안 올라오는 문제 때문에 일부러 같이 부르는 것이라(위 주석) 날짜와 무관하게 전부
      // 딸려온다. 그래서 "오늘 예약건 조회"에 어제 예약건이 섞여 나왔다(2026-08-26 실사용:
      // 8/25 15:00 예약인 OID1455가 기사배정 상태로 남아 있어 오늘 목록에 올라왔다).
      // 고객이 날짜를 말했으면 그 날짜의 건만 답이다.
      //
      // **무엇으로 세는지**를 나눈 이유(실사용 지적 2026-09-08): "오늘 접수한 건 조회해줘"에
      // 오늘 예약인 OID2075(접수는 어제)가 나오고, 정작 오늘 접수한 OID2150(예약은 내일)은
      // 빠졌다. 예전에는 예약시간 한 칸으로만 걸렀기 때문이다. 접수와 예약은 다른 날일 수
      // 있고, 사람은 두 말을 섞어 쓴다.
      //
      // 그래서 고객이 쓴 말대로 세고(dateBasis), 다른 기준으로 걸린 건은 버리지 않고 따로
      // 묶어 함께 돌려준다 — 물어본 쪽을 먼저 답하고 나머지를 덧붙이면, 어느 말로 물었든
      // 찾는 주문이 보인다. 미래 기간에는 이 덧붙임을 하지 않는다: 접수는 과거에만 일어나므로
      // "내일 접수된 건" 같은 집합은 존재하지 않는다.
      const dayOf = (row, b) => String(
        (b === 'created' ? row.접수시각 : (row.예약시각 || row.접수시각)) || ''
      ).slice(0, 10);
      // true(기간 안) / false(기간 밖) / null(날짜를 못 읽음 — 판단 불가)
      const within = (row, b) => {
        const day = dayOf(row, b);
      ...
      // 판단할 수 없는 건은 남긴다 — 숨기면 있는 주문이 없는 것이 된다.
      const labeled = hasRange ? labeledAll.filter((row) => within(row, basis) !== false) : labeledAll;

      // 물어본 기간이 오늘이거나 과거를 포함할 때만 다른 기준을 덧붙인다.
      const todayISO = toDateStr(kstNow());
      const touchesToday = !args.startDate || String(args.startDate).trim() <= todayISO;
      const alsoRows = (hasRange && touchesToday)
        ? labeledAll.filter((row) => within(row, basis) === false && within(row, otherBasis) === true)
        : [];

      // 어느 쪽에도 안 들어간 건이 있으면 그 사실만 수로 알려준다 — 모델이 "주문이 없습니다"로
      // 단정하는 것을 막는다.
      const 제외건수 = labeledAll.length - labeled.length - alsoRows.length;

      const 기간 = hasRange
        ? `${args.startDate || '처음'} ~ ${args.endDate || '오늘'}`
        : `기간 지정 없음(최근 ${pageSize}건)`;
      const 기준라벨 = BASIS_LABELS[basis];
      return {
        ok: true,
        조회조건: hasRange
          ? `주문 이력 ${기간} — ${기준라벨} 기준, 이용고객 ${candidates.map(access.maskPhone).join(', ')} 기준 (그 기간의 진행중 + 지난이력)`
          : `주문 이력 ${기간}, 이용고객 ${candidates.map(access.maskPhone).join(', ')} 기준 (진행중 + 지난이력 모두 포함)`,
        기준: hasRange ? 기준라벨 : undefined,
        기간밖제외: 제외건수 || undefined,
        orders: stripInternal(labeled),
        // 물어본 기준으로는 안 걸리지만 다른 기준으로는 이 기간에 해당하는 건.
        다른기준: alsoRows.length ? {
          기준: BASIS_LABELS[otherBasis],
          안내: basis === 'created'
            ? '접수는 다른 날이지만 예약일이 이 기간인 주문입니다.'
            : '예약일은 다른 날이지만 이 기간에 접수된 주문입니다.',
          orders: stripInternal(alsoRows),
        } : undefined,
        조회한연락처: candidates.map(access.maskPhone),
      };
    }
...
async function prepareMutation(ctx, sessionId, toolName, args) {
  ...
  if (toolName === 'update_order' || toolName === 'cancel_order') {
    ...
    // 차량번호·차종은 콜마너 MCP(call.update)의 changes에 섞지 않는다.
    //
    // 왜: 콜마너 오더접수/수정 요청에는 차량 전용 필드가 아예 없다(정의서 v1.7 전수 확인 —
    // 차량 관련은 driver_option의 `탁송(yn)` 플래그뿐이다). 모르는 키를 넣어 보내봐야
    // 반영되지 않는다. 우리 orders를 고친 뒤, 적요1에 번호를 실어 보내는 자체 연동
    // (updateOrderWithCallmaner → OrderModify)으로 콜마너에 반영한다.
    //
    // 실사용 근거: 상담 로그에 "차량번호 수정 합니다. 코란도 157서6830 → 코란도 335모6328"
    // 형태의 정정 요청이 반복해서 나오는데, 시간·주소·요금은 되고 차량번호만 안 돼서 그때마다
    // 상담원 인계로 빠지고 있었다.
    const localChanges = {};
    if (args.vehicleNumber || args.vehicleType) {
      // 번호판을 고치겠다고 했는데 형식이 안 잡히면(자릿수 오타 "8083692", 한글 없음) 조용히
      // 넘기지 않는다 — 잘못된 번호가 오더에 박히면 현장에서 다른 차를 가져가는 사고가 된다.
      // splitTypeAndPlate만으로는 못 거른다: 형식이 어긋나도 원문을 그대로 돌려주기 때문에
      // "8083692"가 번호판인 양 저장된다(이 검사를 붙이면서 실제로 그렇게 통과했다).
      let plate = null;
      if (args.vehicleNumber) {
        const found = findPlate(args.vehicleNumber);
        if (!found) {
          return { ok: false, error: '차량번호 형식을 알아보지 못했습니다. "335모6328"처럼 번호판을 다시 확인해 달라고 고객에게 물어보세요.' };
        }
        plate = found;
      }
      // 모델이 "코란도 335모6328"처럼 한 칸에 몰아넣는 경우가 있어, 번호판을 뺀 나머지를
      // 차종으로 받아준다(따로 준 vehicleType이 있으면 그쪽이 우선이다).
      const split = splitTypeAndPlate(args.vehicleType || (plate && plate.rest) || null, null);
      if (plate) localChanges.vehicleNumber = plate.plate;
      if (split.vehicleType) localChanges.vehicleType = split.vehicleType;
    }

    if (Object.keys(changes).length === 0 && Object.keys(localChanges).length === 0) {
      return { ok: false, error: '무엇을 변경할지 확인되지 않았습니다. 변경할 항목(예약시각/출발지/도착지/요금/메모/차량번호)을 고객에게 물어보세요.' };
    }
    ...
    return {
      ...
      pending: {
        ...
        summary: {
          동작: '배차 주문 변경',
          접수번호: await displayReceiptNo(rcptNo),
          // 확인 문구는 고객이 부탁한 것을 전부 보여줘야 한다 — 어디에 저장되는지는 고객의 관심사가 아니다.
          변경내용: { ...changes, ...localChanges },
        },
      },
    };
  }
...
async function syncLocalOrderAfterMcpUpdate(changes, rcptNo) {
  try {
    ...
    // 차량번호·차종은 우리 DB에만 있는 값이다(콜마너 payload에 차량 칸이 없다). 그래서 이
    // UPDATE가 곧 반영의 전부이고, 아래 orderModify로 다시 보낼 것이 없다.
    const vehicleChanged = changes.vehicleNumber !== undefined || changes.vehicleType !== undefined;
    if (changes.vehicleNumber !== undefined) {
      sets.push('vehicle_number = ?');
      params.push(changes.vehicleNumber);
    }
    if (changes.vehicleType !== undefined) {
      sets.push('vehicle_type = ?');
      params.push(changes.vehicleType);
    }
    ...
    async function placeUpdate(place, prefix) {
      ...
      // 장소검색(콜마너 place.find)은 {name, region, xy}만 주고 주소 문자열이 없다. 그래서
      // 예전에는 오더 주소 칸에 도로명주소 대신 장소명("판교역")이 들어갔다 — 좌표는 정확히
      // 오므로 그걸로 도로명주소를 되찾아 쓰고, 조회가 실패할 때만 장소명으로 떨어진다.
      // 장소명은 상세주소로 남겨 어느 지점인지 알 수 있게 한다(도로명주소만으로는 "판교역
      // 몇 번 출구"인지 드러나지 않는다).
      let address = place.address || null;
      let detail = null;
      if (!address && hasCoord) {
        const looked = await lookupRoadAddress(lat, lng).catch(() => null);
        if (looked) {
          address = looked;
          if (place.name && looked.indexOf(place.name) === -1) detail = place.name;
        }
      }
      if (!address) address = place.name || null;

      if (address) { sets.push(`${prefix}_address = ?`); params.push(address); }
      // 상세주소는 값이 있을 때만 덮어쓴다 — 못 찾았다고 기존 상세주소를 지우면 안 된다.
      if (detail) { sets.push(`${prefix}_address_detail = ?`); params.push(detail); }
      if (hasCoord) {
    ...
    // 차량 정정은 "무엇이 무엇으로" 바뀌었는지가 이력의 핵심이다 — 나중에 현장에서 차가 다르면
    // 이 줄 하나로 누가 언제 바꿨는지 가려진다. 웹 수정화면과 같은 표기를 쓴다(routes/orders.js).
    const noteParts = [`[MCP 챗봇] 주문 변경: ${Object.keys(changes).join(', ')}`];
    if (vehicleChanged) {
      const before = [order.vehicle_type, order.vehicle_number].filter(Boolean).join('/') || '(빈 값)';
      const afterType = changes.vehicleType !== undefined ? changes.vehicleType : order.vehicle_type;
      const afterNumber = changes.vehicleNumber !== undefined ? changes.vehicleNumber : order.vehicle_number;
      const after = [afterType, afterNumber].filter(Boolean).join('/') || '(빈 값)';
      noteParts.push(`차종/차량번호: ${before} → ${after}`);
    }
    await db.run(
      `INSERT INTO order_status_history (order_id, actor_user_id, old_status, new_status, note) VALUES (?, NULL, ?, ?, ?)`,
      [order.id, order.status, order.status, noteParts.join(' / ')]
    );

    // 이미 검증된 우리 자체 연동으로 콜마너에도 한 번 더 확실히 반영한다. 차량번호만 바뀐
    // 경우도 반드시 보낸다 — 콜마너에는 차량 칸이 없어 번호를 적요1 맨 앞에 실어 보내므로
    // (lib/callmaner.js memoWithVehicle), 이 호출을 건너뛰면 콜마너 적요가 옛 번호로 남는다.
...
// pending.summary(카카오 확인 문구에 쓰던 그 객체)를 한 줄로 — 법인 공유 피드에 그대로 쓴다.
// 이미 사람이 읽게 다듬어진 값이라 새로 문구를 짜지 않아도 된다.
function summaryLineFromPendingSummary(summary) {
  if (!summary) return '';
...
// 취소/변경/요금인상이 성공하면 법인 공유 피드에도 남긴다(옵트인 법인만, lib/groupActivityFeed.js
// 안에서 조용히 걸러진다). rcptNo로 우리 로컬 오더를 찾아야 requester_group_id를 알 수 있다 —
// call.update의 callArgs.orderId는 이름과 달리 실제로는 rcptNo다(위 prepareMutation 참고).
async function recordMcpActivity(ctx, pending) {
  const KIND = { 'call.cancel': 'cancelled', 'call.update': 'updated', 'call.raise': 'updated' };
  const kind = KIND[pending.mcpTool];
  if (!kind) return;
  const rcptNo = (pending.callArgs && (pending.callArgs.rcptNo || pending.callArgs.orderId)) || null;
  if (!rcptNo) return;
  try {
    const map = await access.loadOidsByCallmanerSlips([rcptNo]);
    const order = map.get(String(rcptNo));
    if (!order || !order.requester_group_id) return; // 우리 오더가 아니거나 법인 미지정
    recordGroupActivity({
      groupId: order.requester_group_id, oid: order.oid, kind,
      summary: summaryLineFromPendingSummary(pending.summary) || (kind === 'cancelled' ? '취소' : '변경'),
      actorUserId: ctx.userId, actorLabel: ctx.userName,
    });
  } catch (e) {
    console.error('MCP 액션 법인 공유 피드 기록 실패(무시):', e.message);
  }
}
...
async function executePending(ctx, sessionId, pending) {
  ...
  const isUpdate = pending.mcpTool === 'call.update';
  const mcpChanges = (pending.callArgs && pending.callArgs.changes) || {};
  const localChanges = pending.localChanges || {};
  // 차량번호만 정정하는 변경은 콜마너로 보낼 것이 없다(위 prepareMutation 주석) — 빈 changes로
  // call.update를 부르면 프록시가 거부하거나 아무 일도 하지 않으므로 아예 부르지 않는다.
  const skipMcp = isUpdate && Object.keys(mcpChanges).length === 0;
  ...
  if (isUpdate) {
    await syncLocalOrderAfterMcpUpdate({ ...mcpChanges, ...localChanges }, pending.callArgs.orderId);
  }
  recordMcpActivity(ctx, pending);
  return { ok: true, data };
}
...
function describeConfirmation(pending, options) {
  ...
  if (pending.mcpTool === 'call.create') {
    ...
    const vehicle = [changes.vehicleType, changes.vehicleNumber].filter(Boolean).join(' ');
...
// 뜻은 위와 같은데 "내역/현황/알려줘" 같은 말이 없어 위 패턴에 안 걸리던 표현 —
// "내 주문 어떻게 됐어?", "접수 잘 되고 있나요?" 처럼 상태를 묻는 꼬리를 붙인 문장이다.
// 실사용에서 흔한 말투인데 모델 경로로 새고 있었다(실측: 이 한 문장이 3.4초, 이 경로로 답하면
// 1.1초 — 모델 2라운드 2.3초가 통째로 빠진다).
//
// 금지어 검사는 위 패턴과 똑같이 먼저 통과해야 한다 — "어디"(위치)·"기사"·"요금"·"취소"·
// 날짜·순번이 섞이면 여기 오기 전에 걸러진다. 그래서 넓힌 건 "무엇을 묻는가"가 아니라
// "같은 질문의 다른 말투"뿐이다.
// 끝의 (\s*있…)은 "되고 있어요"/"진행되고 있나요"처럼 띄어쓰기 뒤에 붙는 꼬리를 받는다.
const FIXED_ACTIVE_STATUS_RE = /^(내|제|나의)?\s*(진행\s*중(인)?\s*)?(접수|주문|오더|배차)\s*(건)?\s*(은|는|이|가|을|를|좀)?\s*(어떻게|어찌|어떤지|잘)\s*(됐|됬|되었|되고|되어|돼|진행)[가-힣]*(\s*있[가-힣]*)?\s*[?？.!~]*$/;
...
function matchFixedQuery(text) {
  ...
  // 상태를 묻는 말투("어떻게 됐어")는 그 자체가 무엇을 묻는지 분명해서 아래 낱말 검사를
  // 따로 요구하지 않는다.
  if (FIXED_ACTIVE_STATUS_RE.test(t)) return 'active_list';
...
// 주문을 조회하는 질문인지. 살아 있는 데이터를 묻는 질문은 **반드시 도구를 거쳐야** 한다.
//
// 왜 필요한가(실사용 2026-09-08): 카카오 세션은 한 달째 살아 있어서 모델이 보는 이력에
// "내일 예약된 주문은 없습니다"라는 자기 옛 답이 반복해 들어 있었다. 모델은 도구를 부르지
// 않고 그 답을 그대로 베꼈다 — 실제로는 내일 예약이 1건(OID2150) 있었다. 같은 시각 웹에서는
// 도구를 불러 맞게 답했다(웹은 방문마다 세션이 새로 열려 그런 선례가 없다).
//
// 좁게 잡는다. 여기서 틀리면 멀쩡한 답변을 막게 되므로, 확실히 조회인 것만 본다 —
// 못 잡은 질문은 예전 그대로 동작할 뿐이고, 잘못 잡으면 답을 못 내보낸다.
const LOOKUP_NOUN_RE = /(접수|주문|오더|배차|예약|콜)/;
const LOOKUP_VERB_RE = /(조회|보여|알려|확인|내역|현황|목록|리스트|몇\s*건|있(나|어|는지|습니까|나요))/;
// 변경 요청과 방법 문의는 조회가 아니다. 요금·위치처럼 초점이 목록이 아닌 것도 뺀다.
const LOOKUP_EXCLUDE_RE = /(취소|변경|수정|바꿔|올려|인상|낮춰|접수해|예약해|방법|절차|얼마|요금|어떻게\s*(해|하나|하죠|하면))/;
function isOrderLookupQuestion(text) {
  const t = String(text || '').trim();
  if (!t || t.length > 60) return false;
  if (LOOKUP_EXCLUDE_RE.test(t)) return false;
  return LOOKUP_NOUN_RE.test(t) && LOOKUP_VERB_RE.test(t);
}

// 도구를 안 부르고 답하려 할 때 한 번 되돌린다. 이력에 있는 옛 답을 베끼지 말라고 못 박는다.
const TOOL_REQUIRED_NUDGE = [
  '조회 결과를 이전 대화에서 가져오지 마세요. 그 답은 지난 시점의 것이라 지금과 다를 수 있습니다.',
  '반드시 조회 도구를 호출해서 지금 값을 확인한 뒤에 답하세요.',
].join('\n');
...
// 같은 법인의 다른 담당자가 접수한 건까지 볼지 물어본다.
//
// 왜 필요한가: MCP 조회는 **연락처 기준**이다(lib/mcpDispatchAccess.js primaryCid + 이용고객
// 번호 + 연결 번호). 그래서 본사 직원이 물어도 자기 번호로 나간 건만 나오고, 옆자리 동료가
// 접수한 건은 안 보인다. 그런데 웹 화면에서는 본사 직원이 법인 전체를 본다
// (lib/clientScope.js) — 같은 사람이 같은 회사 일을 묻는데 채널에 따라 답이 다르다.
//
// 자동으로 넓히지 않고 물어본다. 법인 전체를 그냥 끼워 넣으면 "내 주문"을 물은 사람에게 남의
// 주문이 섞여 나오고, 목록이 길어져 정작 자기 건을 못 찾는다.
//
// 개인 딜러에게는 묻지 않는다. 딜러는 본인 오더만 보는 자격이라(clientScope) 물어봐도 줄 것이
// 없고, 물음 자체가 "볼 수 있는 게 더 있다"는 잘못된 신호가 된다.
async function countGroupOrdersBeyondMine(ctx) {
  const clientScope = require('./clientScope');
  if (!ctx || !ctx.userId || !ctx.requesterGroupId) return 0;
  if (clientScope.isDealer({ role: 'client', client_type: ctx.clientType })) return 0;
  try {
    const row = await db.get(
      `SELECT COUNT(*) AS c FROM orders
        WHERE requester_group_id = ? AND created_by IS DISTINCT FROM ?
          AND status NOT IN ('완료', '취소')`,
      [ctx.requesterGroupId, ctx.userId]
    );
    return Number(row && row.c) || 0;
  } catch (e) {
    console.error('법인 전체 건수 조회 실패(안내 생략):', e.message);
    return 0;
  }
}

// 법인 전체 목록. MCP를 다시 부르지 않고 우리 DB에서 읽는다 — 연락처가 아니라 법인이 기준이라
// MCP 도구로는 애초에 물어볼 수 없고, 우리 DB에는 접수한 순간부터 다 있다.
//
// **상태의 출처가 본인 목록과 다르다.** 본인 목록(get_my_orders)은 콜마너에 실시간으로 묻고,
// 이 목록은 우리 DB의 사본을 읽는다. 그 사본은 매분 도는 동기화가 갱신하므로 최대 1분쯤 늦고,
// 동기화가 밀리면 더 늦는다. 같은 주문이 한 대화 안에서 다른 상태로 보일 수 있다는 뜻이다.
//
// 그래서 숨기지 않고 밝힌다. 콜마너에 올라간 건은 마지막 확인 시각을 붙이고, 아직 안 올라간
// 건은 그 사실을 적는다 — 관리자가 확정해야 배차가 시작되는 단계라(대기), 기다리는 사람에게
// 그게 가장 중요한 정보다. 실측: 법인 1의 진행 중 48건 중 콜마너에 올라간 건은 2건뿐이었다.
async function listGroupOrders(ctx, limit) {
  const rows = await db.all(
    `SELECT o.oid, o.callmaner_conf_slip, o.status, o.callmaner_status, o.callmaner_synced_at,
            o.callmaner_last_error,
            o.reserved_date, o.reserved_time,
            o.origin_address, o.destination_address, o.vehicle_number,
            u.name AS requester_name
       FROM orders o
       LEFT JOIN users u ON u.id = o.created_by
      WHERE o.requester_group_id = ? AND o.status NOT IN ('완료', '취소')
      ORDER BY o.reserved_date ASC, o.reserved_time ASC, o.id ASC
      LIMIT ?`,
    [ctx.requesterGroupId, Math.max(1, Math.min(Number(limit) || 10, 30))]
  ).catch((e) => {
    console.error('법인 전체 주문 조회 실패:', e.message);
    return [];
  });
  if (!rows.length) return '법인 전체로도 진행 중인 주문이 없습니다.';

  const blocks = rows.map((o, i) => {
    const no = [o.oid, o.callmaner_conf_slip].filter(Boolean).join(' / ');
    const when = [o.reserved_date, o.reserved_time].filter(Boolean).join(' ');
    // 콜마너가 쓰는 말과 우리 말이 다르다(배차 / 기사배정). 콜마너에 올라간 건은 그쪽 말을
    // 함께 보여준다 — 고객이 콜마너 화면과 대조할 때 다른 단어면 다른 상태로 읽는다.
    const state = o.callmaner_status && o.callmaner_status !== o.status
      ? `${o.status} (배차 시스템: ${o.callmaner_status})`
      : (o.status || '-');
    // 콜마너 접수번호가 없다는 것은 "담당자가 확정을 안 했다"가 아니다. 우리는 접수 즉시
    // 콜마너에 올리고 항상 대기로 등록하므로(lib/callmaner.js), 번호가 없다면 전송이 실패했거나
    // 아예 나가지 않은 것이다 — 실측(법인1): 연동 이전 옛 데이터 36건, 전송 실패 10건
    // (지사캐시 부족·출발지 좌표 없음 등). 앞의 문구는 확정만 하면 진행되는 것처럼 읽혀서
    // 기다리는 사람을 잘못된 방향으로 보낸다.
    const note = o.callmaner_conf_slip
      ? (o.callmaner_synced_at ? `   최종 확인: ${o.callmaner_synced_at}` : null)
      : (o.callmaner_last_error
        ? '   배차 시스템 전송이 실패해 담당자가 확인 중입니다.'
        : '   아직 배차 시스템에 올라가지 않았습니다.');
    return [
      `${i + 1}. 접수번호 ${no}`,
      // 누가 접수했는지가 이 목록의 핵심이다 — 본인 것과 구분되지 않으면 넓힌 의미가 없다.
      o.requester_name ? `   접수자: ${o.requester_name}` : null,
      when ? `   예약시간: ${when}` : null,
      `   ${o.origin_address || '-'} → ${o.destination_address || '-'}`,
      `   상태: ${state}${o.vehicle_number ? ` · ${o.vehicle_number}` : ''}`,
      note,
    ].filter(Boolean).join('\n');
  });
  return [
    `법인 전체로 진행 중인 주문은 ${rows.length}건입니다.`,
    // 출처가 다르다는 사실을 한 줄로 밝힌다. 본인 목록은 실시간이고 이쪽은 사본이다.
    '(접수 기록 기준입니다. 배차 진행 상태는 최대 1분 늦을 수 있습니다.)',
    '',
    blocks.join('\n\n'),
  ].join('\n');
}

// 목록 답변 하나를 만든다 — 문장, 대기 상태, 다음 턴을 누가 받을지까지.
//
// 두 경로가 이 함수를 함께 쓴다: 정해진 문장에 걸리는 빠른 경로와 모델이 도구를 고르는 경로.
// 한쪽에만 제안을 붙이면 같은 질문에 표현만 달라도 답이 갈린다.
async function buildListAnswer(ctx, sessionId, result, usedTools, opts) {
  const { persistPending, dropPending, offerGroup } = opts;
  const total = ((result && result.activeOrders) || []).length;
  let message = formatActiveListAnswer(result, 0);

  // 본인 것을 먼저 답하고, 법인에 다른 담당자 건이 더 있으면 그때만 물어본다.
  // 이어보기가 걸린 턴에는 묻지 않는다 — 한 번에 두 가지를 물으면 "네"가 어느 쪽인지 모른다.
  let offered = false;
  if (offerGroup && total <= LIST_PAGE_SIZE) {
    const others = await countGroupOrdersBeyondMine(ctx);
    if (others > 0) {
      message += `\n\n추가로 법인 전체(다른 담당자 접수분 ${others}건)에서도 확인해드릴까요?`;
      offered = true;
    }
  }

  if (offered) {
    await persistPending(sessionId, { action: 'group_list_offer', createdAt: Date.now() });
  } else if (total > LIST_PAGE_SIZE) {
    // 5건이 넘으면 "다음"으로 이어볼 수 있게 위치를 남긴다.
    await persistPending(sessionId, { action: 'list_page', offset: LIST_PAGE_SIZE, createdAt: Date.now() });
  } else {
    await dropPending(sessionId);
  }
  ...
  return {
    ...
    message,
    usedTools,
    awaitingConfirmation: offered,
    // 남은 건이나 제안이 있으면 다음 메시지도 이 도우미가 먼저 받아야 한다.
    expectsReply: offered || total > LIST_PAGE_SIZE,
  };
}
...
function formatActiveListAnswer(result, offset) {
  ...
  const blocks = orders.map((o, i) => {
    ...
    // 배차 이후에는 기사 이름·연락처를 상태 바로 아래에 적는다 — 그 상태에서 고객이 가장
    // 먼저 찾는 값이고, 없으면 "기사님 연락처요"를 다시 물어야 한다(상담원 발화의 2.8%였다).
    const driver = driverLine(o);
    if (driver) lines.push(`   기사: ${driver}`);
...
async function runLocate(ctx, sessionId, pending) {
  ...
  // 좌표는 우리 오더를 찾아 lib/driverLocation.js에 맡긴다.
  //
  // 왜(실측 2026-09-08, OID2075): 여기서 쓰던 order.driver.xy는 MCP가 만들어낸 값일 수 있다
  // (source='route_interpolated' — 출발지↔도착지의 정확히 1/3 지점이었고 실제 좌표와 7.04km
  // 떨어져 있었다). 그 값을 역지오코딩해 "현재 기사님 위치는 ○○입니다"라고 답하면, 고객은
  // 7km 떨어진 동네 이름을 듣고 그 자리에서 기다린다. 오더상세 화면은 그 사이 콜마너 REST의
  // TrackingDriver로 옮겼는데, 챗봇이 옛 값을 계속 쓰면 같은 질문에 두 채널이 다른 답을 한다.
  //
  // 우리 오더로 이어지지 않는 접수번호(콜마너에만 있는 건)는 그 모듈이 볼 것이 없어, 예전처럼
  // MCP 값을 쓴다 — 다만 보간값이면 위치를 말하지 않는다.
  const ourOrder = await ourOrderBySlip(rcptNo);
  const loc = ourOrder ? await driverLocationLib().loadForOrder(ourOrder) : null;
  const synthesized = driverLocationLib().isSynthesizedSource(driver.source);
  const xy = (loc && loc.available) ? `${loc.lat},${loc.lon}` : (synthesized ? null : driver.xy);

  const place = await describeDriverPlace(xy);
  // ETA·거리는 MCP가 그 좌표 위에서 계산한 값이다. 좌표가 가짜면 이것도 가짜라 함께 버린다 —
  // 가짜 위치로 낸 "5분 뒤 도착"이 가장 나쁘다.
  const etaMin = synthesized ? null : formatEtaMinutes(driver.etaSecondsToPickup);
  const distKm = synthesized ? NaN : Number(driver.distanceKmToPickup);
  ...
  // 기준 시각은 MCP 좌표를 쓸 때만 있다. TrackingDriver는 시각을 주지 않으므로, 그 좌표에는
  // "몇 시 기준"을 붙이지 않는다 — 모르는 것을 아는 척하면 오래된 위치를 새것으로 읽는다.
  if (!(loc && loc.available) && driver.lastFixAt) {
    lines.push(`(위치 기준 시각 ${formatDateTime(driver.lastFixAt) || driver.lastFixAt})`);
  }

  // 글로 된 설명에 더해 지도 링크도 준다(사용자 지시). 카카오 평문은 앵커 텍스트를 지원하지
  // 않아 주소를 그대로 적는다 — 사진 모아보기 링크(lib/kakaoOrderNotify.js)와 같은 방식이다.
  //
  // 우리 오더로 이어지지 않는 접수번호(콜마너에만 있는 건)면 토큰이 없어 링크도 없다.
  // 그때는 줄을 통째로 뺀다 — 눌러도 안 열리는 주소를 보내면 문의만 늘어난다.
  const trackUrl = await trackingUrlForSlip(rcptNo);
  if (trackUrl) lines.push(`실시간 위치 보기: ${trackUrl}`);
  return { ok: true, message: lines.join('\n') };
}

// lib/driverLocation.js는 이 파일을 require하지 않으므로 순환은 없지만, 위치를 안 쓰는
// 호출 경로에서 괜히 붙지 않도록 필요할 때 읽는다(trackingUrlForSlip이 이미 같은 방식이다).
function driverLocationLib() {
  return require('./driverLocation');
}

// 콜마너 접수번호로 우리 오더 한 줄. 위치 조회에는 오더 전체가 필요하다(지사·연락처·상태).
async function ourOrderBySlip(rcptNo) {
  try {
    return await db.get(
      'SELECT * FROM orders WHERE callmaner_conf_slip = ? ORDER BY id DESC LIMIT 1',
      [String(rcptNo || '')]
    );
  } catch (e) {
    console.error('접수번호로 오더 조회 실패:', e.message);
    return null;
  }
}

// 콜마너 접수번호로 우리 오더의 추적 링크를 찾는다. 진행 중일 때만 준다 — 완료된 운행은
// 위치를 더는 수집하지 않아 링크를 열어도 빈 화면이다.
async function trackingUrlForSlip(rcptNo) {
  try {
    const driverLocation = require('./driverLocation');
    const row = await require('../db').get(
      'SELECT tracking_token, status FROM orders WHERE callmaner_conf_slip = ? ORDER BY id DESC LIMIT 1',
      [String(rcptNo || '')]
    );
    if (!row || !driverLocation.TRACKABLE_STATUSES.has(String(row.status || ''))) return '';
    return driverLocation.trackingLink(row);
  } catch (e) {
    // 링크는 부가정보다 — 못 만들어도 위치 설명은 그대로 나가야 한다.
    console.error('추적 링크 조회 실패:', e.message);
    return '';
  }
}
...
// speculative: 의도분류(intent_light)와 동시에 미리 돌려보는 실행. 분류 결과가 unsupported가
// 아니면 이 결과는 버려지므로, 버려도 뒤탈이 없는 범위 안에서만 움직여야 한다.
//
// 지켜야 할 두 가지:
//  (1) 확인 대기 상태(mcp_pending_json)가 이미 있으면 손대지 않고 물러난다 — 이 경로는
//      "네" 한마디로 실제 접수/취소를 실행(executePending)하는 자리다. 버려질지도 모르는
//      실행에서 건드릴 일이 아니다. (원래 이 상황에서는 클라이언트가 분류를 건너뛰고
//      /dispatch-agent를 직접 부르므로, 여기 걸리는 건 상태가 어긋난 경우뿐이다.)
//  (2) 모델이 변경 도구를 고르면 그 전에 멈춘다 — prepareMutation/savePending까지 가면
//      버려진 실행이 확인 대기 상태를 남겨, 다음에 온 "네"가 엉뚱한 행위의 동의로 소비된다.
// 두 경우 모두 handled:false로 물러나므로 호출부가 평소대로(투기 아님) 다시 부르면 된다.
//
// 조회 도구는 공유 상태를 바꾸지 않아(runReadTool) 그대로 돌려도 안전하다. 다만 조회 응답
// 중에도 "다음"을 위해 목록 위치를 남기는 자리가 있어서(list_page), 호출부는 결과를 버릴 때
// clearPending으로 정리한다 — (1) 때문에 투기 실행 전에는 대기 상태가 없었음이 보장된다.
// draftMode: 상담원 도우미 초안을 만들기 위한 실행(routes/kakaoConsult.js buildDispatchSuggestion).
//
// speculative와 다른 점: 변경 도구(등록/수정/취소/요금인상)를 만나도 물러나지 않고, 확인 문구를
// 만들어 돌려준다 — 상담원이 "이대로 보낼까" 판단할 재료가 되어야 하기 때문이다. 대신 **어떤
// 상태도 저장하지 않는다**: 확인 대기(savePending)를 남기면 채택되지도 않은 초안이 고객의 다음
// "네"를 소비해버린다. 실제 실행은 상담원이 채택해 봇이 이어받은 뒤 정식 경로에서 일어난다.
async function runDispatchAgent({ user, sessionId, text, history, viewerCid, requesterGroupId, speculative, draftMode }) {
  if (!mcp.isConfigured()) {
    return { handled: false, reason: 'not_configured' };
  }
  // 초안 실행은 대기 상태를 만들지도 지우지도 않는다.
  const persistPending = draftMode ? async () => {} : savePending;
  const dropPending = draftMode ? async () => {} : clearPending;
  ...
  // 위 주석 (1) — 확인 대기 중이면 투기 실행은 물러난다. 초안 실행도 마찬가지다: 고객이
  // 이미 봇의 확인 질문을 받아둔 상태인데 초안이 그 "네"를 대신 해석하면 안 된다.
  if ((speculative || draftMode) && listState) {
    return { handled: false, reason: speculative ? 'speculative_pending' : 'draft_pending' };
  }
  // 법인 전체 확인 제안에 대한 답. list_page와 같은 자리에서 다룬다 — 둘 다 "예/아니오"가
  // 아니라 목록 흐름의 이어짐이라, 아래 변경 확인 대기와 섞이면 안 된다.
  if (listState && listState.action === 'group_list_offer') {
    await clearPending(sessionId);
    // 이미 있는 판정을 쓴다 — 여기만 다른 규칙을 두면 "네 보여주세요"가 한쪽에서만 통한다.
    if (isAffirmativeReply(text, null)) {
      return {
        handled: true,
        message: await listGroupOrders(ctx, 10),
        usedTools: ['group_orders'],
        awaitingConfirmation: false,
        expectsReply: false,
      };
    }
    // 아니라고 했으면 이 자리에서 끝내지 않는다 — 다른 용건일 수 있으므로 아래 정상 경로로
    // 흘려보낸다. 대기 상태만 지운 것이 이번 처리의 전부다.
  }
  ...
  if (isLocationQuestion(text)) {
    // 이 경로는 "어느 주문인지 확인부터 받는" 흐름이라 확인 대기를 저장한다. 초안으로는 만들 수
    // 없어서(저장하면 채택되지도 않은 초안이 고객의 다음 답을 소비한다) 초안 실행에서는 비운다.
    if (draftMode) return { handled: false, reason: 'draft_locate_unsupported' };
  ...
  if (matchFixedQuery(text) === 'active_list') {
    ...
    if (quick && quick.ok) {
      // 빠른 경로와 모델 경로가 같은 규칙을 써야 한다 — 한쪽에만 제안을 붙이면 같은 질문에
      // 표현만 달라도 답이 갈린다(빠른 경로는 정해진 문장에만 걸린다).
      return buildListAnswer(ctx, sessionId, quick, ['get_my_orders(fast)'],
        { persistPending, dropPending, offerGroup: !speculative && !draftMode });
    }
  }
  ...
  // 조회인데 도구를 안 불러 한 번 되돌렸는지. 두 번은 되돌리지 않는다 — 무한 왕복이 된다.
  let nudgedForTool = false;
  ...
  // 이번 턴에서 성공한 주문 목록 조회 결과. 아래에서 답변 문장을 서버가 직접 만드는 데 쓴다.
  let lastOrderList = null;
  ...
  for (let round = 0; round < MAX_TOOL_ROUNDS; round += 1) {
    ...
    const turn = await generateWithTools({ systemInstruction, contents, tools: TOOL_DECLARATIONS, thinking: false, op: 'mcp_dispatch' });

    if (!turn.functionCalls.length) {
      const message = turn.text;
      if (!message) return { handled: false, reason: 'empty_response', usedTools };

      // 조회 질문인데 도구를 한 번도 안 불렀으면 그 답은 이력에서 베낀 것이다 — 내보내지 않는다.
      // 한 번 되돌려 도구를 부르게 하고, 그래도 안 부르면 답하지 않고 물러난다(상담원으로 간다).
      // 틀린 답을 자신 있게 내보내는 것보다 못 답하는 편이 낫다.
      if (!usedTools.length && isOrderLookupQuestion(text)) {
        if (!nudgedForTool) {
          nudgedForTool = true;
          contents.push({ role: 'model', parts: turn.parts });
          contents.push({ role: 'user', parts: [{ text: TOOL_REQUIRED_NUDGE }] });
          continue;
        }
        console.warn(`배차 도우미: 조회 질문인데 도구를 부르지 않아 답변을 버림 — "${String(text).slice(0, 30)}"`);
        return { handled: false, reason: 'lookup_without_tool', usedTools };
      }
      if (sawNotRegistered && !anyToolSucceeded) {
        // 기록을 남긴다. 이건 고칠 수 있는 문제인데 지금까지 아무 데도 안 남아서, 고객은
        // 상담원 연결로만 떨어지고 관리자는 왜 그런지 알 방법이 없었다(실측 2026-09-04:
        // 계정 전화번호가 콜마너에 없어 조회가 통째로 실패했는데 화면에는 "기능 준비 중"이
        // 떴다). 계정 전화번호를 콜마너에 등록하거나 고치면 그날부터 조회가 된다.
        require('./integrationLog').logIntegrationErrorAsync({
          source: 'mcp', operation: 'customer_not_registered',
          refType: 'user', refId: Number(ctx.userId) || null,
          message: '콜마너에 등록되지 않은 연락처 — 주문 조회가 불가합니다',
          context: {
            userName: ctx.userName || null,
            // 번호는 가려서 남긴다. 어느 계정인지는 refId로 찾는다.
            phone: access.maskPhone(ctx.primaryCid || ''),
            textHead: String(text || '').slice(0, 40),
          },
        });
        return { handled: false, reason: 'customer_not_registered', usedTools };
      }

      // 주문 목록만 물어본 턴이면 답변 문장을 모델이 아니라 서버가 만든다.
      //
      // 왜: 모델에게는 도구 결과가 구조화된 값(상태/기사배정/요금)으로 가고 문장은 모델이 쓴다.
      // 그 과정에서 사실이 뒤집힌 사고가 실제로 있었다 — `기사배정: false`인 주문을
      // "대기 (기사 배정됨)"으로 안내했고(실측 OID1455: callmaner_driver_name이 비어 있고
      // 콜마너 상태도 대기/01), 요금 미정(0)을 "요금: 0원"으로 적어 공짜처럼 읽히게 했다
      // (서버 포맷터는 formatFare가 0을 숨긴다 — 0은 무료가 아니라 미정이다).
      //
      // 이 저장소는 같은 판단을 이미 한 번 했다: 변경 도구의 확인 문구도 모델에게 맡겼다가
      // 실행되지 않은 일을 완료된 것처럼 쓰는 사고가 나서 서버가 직접 만들게 바꿨다
      // (describeConfirmation 주석). 조회도 사실관계가 걸린 답변이라 같은 원칙을 적용한다.
      //
      // 조회 도구가 get_my_orders뿐일 때만 바꾼다 — 위치·요금·이력이 섞인 답변은 목록 문장
      // 하나로 대신할 수 없어서, 그런 턴은 예전처럼 모델이 쓴다.
      //
      // **그리고 "목록을 물어본 턴"일 때만이다.** 이 조건이 없어서 실사용에서 엉뚱한 답이
      // 나갔다(2026-09-08): "OID2075 배차기사 전화번호와 이름은?", "첫번째오더 배차기사정보
      // 알려줘"에 진행 중 주문 목록 전체가 답으로 왔다. 기사 정보는 get_my_orders 결과에
      // 들어 있으므로 모델이 그 도구를 부른 것은 옳았는데, 여기서 모델 답변을 버리고 목록
      // 문장으로 갈아치우는 바람에 질문에 대한 답이 사라진 것이다.
      //
      // 특정 항목(기사·연락처·요금·위치)이나 특정 오더(접수번호·순번)를 물은 턴은 모델이
      // 답한다 — 판정 규칙은 고정 빠른 응답이 쓰는 것과 같은 것을 쓴다(FIXED_QUERY_BLOCKERS_RE).
      // 그쪽은 "이 낱말이 있으면 목록 하나로 답할 질문이 아니다"를 이미 정의해 뒀다.
      const askedSomethingSpecific = FIXED_QUERY_BLOCKERS_RE.test(String(text || ''));
      const listOnly = lastOrderList && usedTools.length > 0
        && usedTools.every((name) => name === 'get_my_orders')
        && !askedSomethingSpecific;
      if (listOnly) {
        const total = (lastOrderList.activeOrders || []).length;
        // 고정 조회 빠른 경로와 같은 규칙으로 이어보기 위치를 남긴다(그쪽 주석 참고).
        return buildListAnswer(ctx, sessionId, lastOrderList, usedTools,
          { persistPending, dropPending, offerGroup: !speculative && !draftMode });
      }
    ...
    if (turn.functionCalls.some((call) => MUTATING_TOOLS.has(call.name))) {
      // 위 주석 (2) — 변경 도구는 투기 실행에서 손대지 않는다.
      if (speculative) return { handled: false, reason: 'speculative_mutation', usedTools };
      for (const call of turn.functionCalls) {
        ...
        if (MUTATING_TOOLS.has(name)) {
          ...
          if (!prepared.ok) {
            ...
            // 초안 실행 — 확인 문구만 만들어 돌려주고 **대기 상태는 저장하지 않는다**. 저장하면
            // 채택되지도 않은 초안이 고객의 다음 "네"를 소비한다. 실제 실행은 상담원이 이 초안을
            // 채택해 봇이 이어받은 뒤, 봇의 정식 경로에서 확인을 다시 받고 일어난다.
            return {
              handled: true,
        ...
        if (name === 'get_my_orders' && response && response.ok) lastOrderList = response;
...
module.exports = {
  isLocationQuestion,
  // 위치 답변을 검사가 직접 돌려본다(scripts/check-driver-location-source.js).
  runLocate,
  runDispatchAgent,
  checkDispatchDelay,
  loadPending,
  ...
  // 조회 도구를 모델 없이 직접 돌려보는 데 쓴다(scripts/check-order-history-basis.js).
  runReadTool,
};
