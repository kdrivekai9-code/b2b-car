...
const MUTATING_TOOLS = new Set(['create_order', 'update_order', 'cancel_order']);
...
function describeConfirmation(pending) {
  const s = pending.summary || {};
  const lines = [];
  if (pending.mcpTool === 'call.create') {
    lines.push('아래 내용으로 배차 주문을 등록할까요?');
...
function isAffirmativeReply(text) {
  ...
  if (!s) return false;
  if (/(아니|아뇨|안\s?[돼되]|취소하지|하지\s?마|그만|나중에|보류)/.test(s)) return false;
  return /^(네|넵|예|응|어|그래|맞아요?|맞습니다|좋아요?|좋습니다|괜찮아요?|오케이|ok|okay|yes|y|진행|진행해|해주세요|해줘|부탁)[.!~\s]*$/i.test(s)
    || /(그대로\s*(진행|등록|해)|진행\s*(해|할|하)|등록\s*(해|할|하)|취소\s*(해|할|하)\s*주|맞습니다|맞아요|부탁\s*(해|드립|합니다))/.test(s);
}

function isNegativeReply(text) {
  return /(아니|아뇨|안\s?[돼되]|하지\s?마|그만|나중에|보류|취소하지\s?마|다시)/.test(String(text || ''));
}
