  ...
  if (toolName === 'get_order_history') {
    ...
    if (anyOk) {
      ...
      const inRange = (row) => {
        ...
        // 날짜를 못 읽는 건은 남긴다 — 판단할 수 없는 것을 숨기면 있는 주문이 없는 것이 된다.
      ...
      const labeled = hasRange ? labeledAll.filter(inRange) : labeledAll;
      // 걸러낸 건이 있으면 그 사실을 알려준다 — 고객이 "진행 중인 다른 건은 어떻게 됐나"를
      // 물을 수 있어야 하고, 모델이 "주문이 없습니다"로 단정하는 것도 막는다.
      const 제외건수 = labeledAll.length - labeled.length;
      ...
      return {
        ok: true,
        조회조건: hasRange
          ? `주문 이력 ${기간}, 이용고객 ${candidates.map(access.maskPhone).join(', ')} 기준 (그 기간의 진행중 + 지난이력)`
