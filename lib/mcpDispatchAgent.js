...
async function runReadTool(ctx, sessionId, toolName, args) {
  ...
  if (toolName === 'get_my_orders') {
    ...
    if (!profile.ok && !active.ok) {
      return {
        ok: false,
        error: profile.error || active.error,
        notRegistered: /CUSTOMER_NOT_FOUND/i.test(String(profile.error || active.error || '')),
        hint: '이 연락처는 배차 시스템에 등록된 고객이 아닙니다.',
      };
    }
  ...
  if (toolName === 'get_order_history') {
    ...
    if (!out.ok) return { ok: false, error: out.error, notRegistered: /CUSTOMER_NOT_FOUND/i.test(String(out.error || '')) };
