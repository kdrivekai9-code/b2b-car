...
// 모양이 나오면 그것대로 혼란스럽다.
function formatActiveListAnswer(result) {
  const orders = (result && result.activeOrders) || [];
  const 조건 = (result && result.조회조건) || '진행 중인 주문';
  if (!orders.length) return `${조건}으로 조회된 주문이 없습니다.`;

  const blocks = orders.map((o, i) => {
    const lines = [`${i + 1}. 접수번호 ${o.접수번호}`];
  ...
  const delayed = orders.filter((o) => o.배차지연);
  ...
  return [`${조건}으로 ${orders.length}건입니다.`, '', blocks.join('\n\n'), '', tail].join('\n');
}
...
async function runDispatchAgent({ user, sessionId, text, history }) {
  ...
  if (matchFixedQuery(text) === 'active_list') {
    ...
    if (quick && quick.ok) {
      return {
        handled: true,
        message: formatActiveListAnswer(quick),
