...
async function listGroupOrders(ctx, limit) {
  const rows = await db.all(
    `SELECT o.oid, o.callmaner_conf_slip, o.status, o.reserved_date, o.reserved_time,
  ...
  const blocks = rows.map((o, i) => {
    ...
    return [
      ...
      `   상태: ${o.status || '-'}${o.vehicle_number ? ` · ${o.vehicle_number}` : ''}`,
    ].filter(Boolean).join('\n');
  });
  return [`법인 전체로 진행 중인 주문은 ${rows.length}건입니다.`, '', blocks.join('\n\n')].join('\n');
}
