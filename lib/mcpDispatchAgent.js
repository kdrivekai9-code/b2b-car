...
async function runReadTool(ctx, sessionId, toolName, args) {
  ...
  if (toolName === 'get_my_orders') {
    ...
    for (const cid of candidates) {
      ...
      if (orders.length) return { ...result, 조회시도: tried.map(access.maskPhone) };
      if (!profileOnly) profileOnly = result;
    }
    if (profileOnly) return { ...profileOnly, 조회시도: tried.map(access.maskPhone) };
    return {
      ok: false,
      error: lastError,
      notRegistered: /CUSTOMER_NOT_FOUND/i.test(String(lastError || '')),
      조회시도: tried.map(access.maskPhone),
