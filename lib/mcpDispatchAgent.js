...
async function prepareMutation(ctx, sessionId, toolName, args) {
  ...
  if (toolName === 'update_order' || toolName === 'cancel_order') {
    ...
    // 차량번호·차종은 콜마너로 나가지 않는 값이라 changes에 섞지 않는다.
    //
    // 왜: 콜마너 오더접수/수정 payload(lib/callmaner.js buildOrderPayload)에는 차량 칸이
    // 아예 없다 — 우리 orders 테이블에만 있는 값이다. call.update의 changes에 넣어 보내면
    // 프록시가 모르는 키를 받게 되고, 반영도 안 된다. 우리 DB만 고치면 그게 곧 전부다.
    //
...
async function syncLocalOrderAfterMcpUpdate(changes, rcptNo) {
  try {
    ...
    // 이미 검증된 우리 자체 연동으로 콜마너에도 한 번 더 확실히 반영한다. 차량만 바뀐 경우는
    // 보낼 것이 없어 건너뛴다 — 같은 값을 다시 실어 보내는 외부 호출일 뿐이다.
    const onlyVehicleChanged = vehicleChanged
      && Object.keys(changes).every((k) => k === 'vehicleNumber' || k === 'vehicleType');
    if (!onlyVehicleChanged) await updateOrderWithCallmaner(order.id, order.branch_id);
  } catch (e) {
