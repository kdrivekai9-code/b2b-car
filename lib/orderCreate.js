...
const { runAfterResponse } = require('./afterResponse');
const branchPolicy = require('./branchPolicy');
const { isPostalRequested, generateReceiptToken, receiptMemoLine, RECEIPT_MEMO_LABEL } = require('./postalReceipt');
const { vehiclePrefix, withVehiclePrefix, withDriverMemoCopy } = require('./intakeMemoSplit');
...
// 일일기사 이용 시간. 하루를 넘는 값은 시간이 아니라 다른 숫자를 잘못 넣은 것이다
// (챗봇 요금 문의도 같은 상한을 쓴다 — lib/agentAssist.js parseUseHours).
function normalizeDailyDriverHours(v) {
...
const RESERVATION_BASES = new Set(['immediate', 'pickup', 'delivery']);
...
async function createOrder(input, options = {}) {
  ...
  const row = {
    ...
    // 예약 기준(즉시 / 픽업 / 도착). 콜마너로 예약시각을 실어 보낼지 이 값이 가른다
    // (lib/callmaner.js buildOrderPayload). 안 넘겨주는 접수 경로는 null 그대로 두고,
    // null은 예전처럼 예약으로 본다 — 모르는 것을 'pickup'으로 단정하지 않는다.
    reservationBasis: RESERVATION_BASES.has(String(input.reservationBasis || '').trim())
      ? String(input.reservationBasis).trim()
      : null,
    ...
    // 일일기사 이용 시간 — 요금 계산의 입력값이자 청구 근거다(마이그레이션 20260909010000).
    // reservationHoursBracket(세 구간)과 다른 값이다. 그쪽은 프리미엄→일일기사 자동 승격
    // 판정에만 쓰고, 청구에는 이 실제 시간을 쓴다.
    dailyDriverHours: normalizeDailyDriverHours(input.dailyDriverHours),
    ...
    // 기사 전달사항 맨 앞에 차종·차량번호를 붙여 **저장**한다(2026-09-07, 사용자 지시).
    //
    // 예전에는 콜마너로 보내는 순간에만 차량번호를 끼웠다(lib/callmaner.js memoWithVehicle).
    // 그래서 관리자 화면의 기사전달사항에는 그 값이 없고 무엇이 전달됐는지 확인할 수 없었다.
    //
    // 여기서 붙이는 이유는 순서다. 콜마너 접수는 오더 생성 직후 **응답 전에** 나가고
    // (routes/orders.js registerOrderWithCallmaner), 요청사항 재분류는 응답 **뒤에** 돈다
    // (splitClientMemo). 그래서 재분류에만 맡기면 첫 전송의 적요1에는 차량번호가 없다 —
    // 콜마너 payload에 차량 칸이 없어서 적요1이 그 값을 전달하는 유일한 통로다.
    memoCustomer: withVehiclePrefix(input.memoCustomer, vehiclePrefix(split.vehicleType, split.vehicleNumber)),
    memoBilling: input.memoBilling || null,
    // 콜마너 적요1(기사메모)에 실을 100Byte 요약. 없으면 전송 시점에 memo_customer를 쓴다.
    //
    // **요약이 없으면 만들지 않는다.** withVehiclePrefix에 빈 값을 넘기면 접두어만 남은
    // 문자열("토레스 150두8774")이 돌아오는데, 그것이 memo_driver_brief에 저장되면 전송이
    // 그 값을 골라서(memoWithVehicle이 요약을 먼저 본다) 기사 전달사항 본문이 통째로
    // 사라진다. 실제로 그렇게 만들었다가 실접수 확인에서 잡았다.
    memoDriverBrief: input.memoDriverBrief
      ? withVehiclePrefix(input.memoDriverBrief, vehiclePrefix(split.vehicleType, split.vehicleNumber))
      : null,
    // 상담원이 접수 화면에서 직접 쓴 기사 챗봇 전달사항. 아래에서 적요1 전문과 우편발송
    // 링크를 이 값 밑에 덧붙인다 — 세 가지가 한 칸에 들어가므로 조립을 한 곳에서 해야 한다.
    // 예전에는 호출부(routes/orders.js)가 생성 뒤에 이 칸을 통째로 덮어써서, 여기서 붙인
    // 링크와 전문이 조용히 지워졌다.
    memoDriverChat: input.memoDriverChat || null,
  ...
  // 예약 기준(즉시 / 픽업 / 도착). 콜마너로 예약시각을 실어 보낼지 이 값이 가른다
  // (lib/callmaner.js buildOrderPayload — 즉시면 reservation_time을 아예 안 싣는다).
  //
  // 위 memo_driver_brief와 **같은 이유로** INSERT에 끼우지 않는다. 선택 컬럼이 하나라도 없으면
  // INSERT 전체가 최소 컬럼으로 퇴화해 차종·좌표·정산메모까지 날아간다.
  //
  // 저장에 실패해도 접수는 그대로 진행한다 — 그 경우 기준이 null로 남고, null은 예전처럼
  // 예약으로 나간다(즉시 건이 예약으로 나가는 것은 지금까지의 동작이라 새 사고가 아니다).
  if (row.reservationBasis) {
    await exec.run('UPDATE orders SET reservation_basis = ? WHERE id = ?', [row.reservationBasis, orderId])
      .catch((e) => {
        if (e && e.code === '42703') return; // 마이그레이션 20260914030000 전 — 조용히 넘어간다
        console.error('예약 기준 저장 실패(무시):', e.message);
      });
  }
  ...
  try {
    ...
    if (needSurcharge || needTolls || needWait) {
      ...
      if (extraSettings) {
        if (needWait) {
          // 오더구분마다 다른 칸을 읽는다 — 탁송 기준 대기요금이 시간제 오더에 붙으면 과청구다.
          row.waitFee = require('./tripFees').waitFee(extraSettings, row.destinationWaitMinutes, row.orderType);
        }
        if (needTolls) {
        ...
        // 할증은 **탁송 전용**이다(사용자 지시). 수입차·야간 같은 할증 금액은 탁송 구간요금표에
        // 얹으라고 정한 값이고, 프리미엄/일일기사 요금은 시간 구간표에서 통째로 나온다
        // (lib/branchPolicy.js calculatePremiumFare — 할증을 더하지 않는다). 그런데 여기서는
        // 오더구분을 안 보고 근거를 박아버려서, 시간제 오더 정산서에 있지도 않은 "수입차 +5,000"이
        // 찍히고 구간요금이 그만큼 깎여 보였다.
        if (needSurcharge && row.orderType === 'dispatch') {
  ...
  // 기사 챗봇 전달사항에 적요1 전문을 남긴다(2026-09-07, 사용자 지시).
  //
  // 적요1은 100Byte라 뒤가 말없이 잘린다(실측 24.4%가 초과). 잘린 뒤에 무엇이 전달됐는지
  // 되짚을 근거가 필요한데, 이 칸은 길이 제한이 없어서 전문이 남는다.
  //
  // 우편발송 링크 붙이기(아래)보다 먼저 한다 — 그 순서라야 링크가 마지막 줄에 오고, 기사
  // 화면에서 눌러야 할 것이 눈에 먼저 걸린다.
  // 일일기사 이용 시간을 남긴다 — 이 값이 곧 fare_amount의 근거다.
  //
  // INSERT 컬럼 목록에 넣지 않고 따로 UPDATE한다. 그 목록은 이미 41칸이고 마이그레이션 미적용
  // DB를 위한 축소 재시도까지 딸려 있어서, 칸을 하나 더 붙일 때마다 두 곳을 같이 고쳐야 한다 —
  // 이 저장소가 다른 선택 컬럼들(memo_driver_chat 등)에 쓰는 방식과 같게 맞췄다.
  if (row.dailyDriverHours != null) {
    await exec.run(
      'UPDATE orders SET daily_driver_hours = ? WHERE id = ?',
      [row.dailyDriverHours, orderId]
    ).catch((e) => {
      if (e && e.code === '42703') return; // 마이그레이션 20260909010000 전 — 조용히 넘어간다
      console.error('일일기사 이용 시간 저장 실패(무시):', e.message);
    });
  }

  const driverChat = withDriverMemoCopy(row.memoDriverChat, row.memoCustomer);
  if (driverChat) {
    await exec.run(
      'UPDATE orders SET memo_driver_chat = ? WHERE id = ?',
      [driverChat, orderId]
    ).catch((e) => {
      if (e && e.code === '42703') return; // 마이그레이션 20260903020000 전 — 조용히 넘어간다
      console.error('기사 챗봇 전달사항 저장 실패(무시):', e.message);
    });
  }
  ...
  if (isPostalRequested(postalSource)) {
    ...
    // 업로드 링크는 기사 챗봇 전달사항에 남긴다(2026-09-07, 사용자 지시).
    //
    // 예전에는 콜마너 적요1에만 실었다. 적요1은 100Byte인데 링크가 38Byte라, 정작 기사가 봐야
    // 할 서류·키 위치 안내가 그만큼 밀려 잘려나갔다(lib/callmaner.js memoWithVehicle 주석).
    // 이 칸은 길이 제한이 없고, 기사 화면의 "해주실 일" 목록도 같은 링크를 버튼으로 띄운다
    // (routes/driverChat.js buildDriverTasks).
    //
    // 사람이 적어둔 안내가 있으면 그 아래에 한 줄로 덧붙인다 — 덮어쓰면 상담원이 쓴 말이 사라진다.
    const receiptLine = receiptMemoLine(token);
    await exec.run(
      `UPDATE orders SET postal_requested = true, receipt_upload_token = ?,
         memo_driver_chat = CASE
           WHEN COALESCE(memo_driver_chat, '') = '' THEN ?
           WHEN memo_driver_chat LIKE ? THEN memo_driver_chat
           ELSE memo_driver_chat || E'\n' || ?
         END
       WHERE id = ?`,
      [token, receiptLine, '%' + RECEIPT_MEMO_LABEL + '%', receiptLine, orderId]
    ).catch((e) => {
  ...
  //
  // 여기(오더 생성 한 곳)에서 하는 이유는 바로 위 우편발송 판정과 같다 — 접수 경로가
  // 웹 폼·카카오 상담톡·웹 챗봇·문의 전환으로 넷인데, 경로마다 두면 한쪽만 고쳐진다.
  // 실제로 그랬다: 웹 폼에만 걸려 있어서, 자유 문장이 가장 많이 들어오는 챗봇·상담톡 접수가
  // 통째로 빠져 있었다(2026-09-04 확인).
  //
  // 응답을 붙잡지 않는다. 모델 호출이라 실측 1.2~5.3초로 들쭉날쭉한데, 접수 응답이 그만큼
  // 늦어지면 고객은 접수가 안 된 줄 안다. 후보는 관리자가 확정 전에 보면 되는 값이라 몇 초
  // 늦어도 아무 문제가 없다.
  //
  // 실패해도 접수를 막지 않는다 — 못 찾은 것은 지금까지와 같은 상태다.
  if (String(row.memoCustomer || '').trim()) {
    const feeExtra = input.fareExtra
      || await branchPolicy.findFareExtra(row.requesterGroupId, row.branchId).catch(() => null);
    runAfterResponse(
      require('./memoExtraCosts').analyzeAndStore(orderId, row.memoCustomer, feeExtra, {
        // 접수 화면에서 이미 고른 항목은 후보에서 뺀다 — 같은 돈이 두 줄이면 두 번 청구된다.
        existingChargeTypes: (input.intakeChargeTypes || []),
      }),
      '요청사항 부대비용 분석'
    );
  }
