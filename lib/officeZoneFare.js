...
// 이 법인이 지점 구간요금을 쓰기로 했는가. 표를 등록해두고도 잠시 끌 수 있어야 한다
// (탁송 요금 화면의 "지점 구간요금 우선 적용" 체크박스).
//
// 컬럼이 없으면(마이그레이션 전) 켜진 것으로 본다 — 이미 등록해둔 법인의 동작이 조용히
// 바뀌면 안 된다.
async function isEnabled(groupId) {
  if (!groupId) return false;
  const row = await db.get('SELECT office_fare_enabled FROM groups_tbl WHERE id = ?', [groupId])
    .catch(() => null);
  if (!row) return true;
  return row.office_fare_enabled !== false;
}
...
async function findZoneFare(groupId, options = {}) {
  // 꺼져 있으면 표가 있어도 쓰지 않는다 — 거리 구간표로 돈다.
  if (!(await isEnabled(groupId))) return null;
