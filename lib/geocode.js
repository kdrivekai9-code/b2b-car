...
async function geocodeAddress(rawAddress) {
  ...
  for (const q of variants) {
    const addressDocs = await kakaoLocalSearch('search/address.json', q);
    const addressHit = addressDocs.find((d) => toCoord(d));
    if (addressHit) {
      const coord = toCoord(addressHit);
      const region = await lookupRegion(coord.lat, coord.lon);
      return {
        lat: coord.lat,
        lon: coord.lon,
        matchedBy: 'address',
  ...
  for (const q of variants) {
    const keywordDocs = await kakaoLocalSearch('search/keyword.json', q);
...
module.exports = { geocodeAddress, buildQueryVariants, isCallmanerReady };
