...
const { logIntegrationError } = require('./integrationLog');
...
// 장애가 해소되면 **복구를 알리고** 상태를 지운다.
//
// 지우는 이유는 예전부터 있었다 — 안 지우면 다음에 같은 장애가 나도 쿨다운에 걸려 조용하다.
// 알리는 쪽은 사용자 지시(2026-09-21)로 붙였다: 장애 알림은 오는데 복구 알림이 없어서,
// 받은 사람이 아직 고장 난 상태인지 풀린 상태인지 화면을 열어봐야만 알 수 있었다.
//
// 여기 남아 있는 키는 **실제로 알림을 보낸 것만**이다(recordSent가 써 넣는다). 그래서 고장을
// 알린 적 없는 건에 대해 "복구됐습니다"가 뜬금없이 가는 일은 없다.
//
// 보내기가 실패해도 상태는 지운다 — 위 알림 경로와 같은 판단이다(발송 실패가 상태 진행을
// 막으면, 실패가 이어질 때 5분마다 같은 것을 영원히 다시 시도한다).
async function clearResolved(activeKeys, send) {
  const rows = await db.all('SELECT * FROM system_alert_state').catch(() => []);
  const stale = (rows || []).filter((r) => !activeKeys.includes(r.alert_key));

  for (const row of stale) {
    const title = row.last_title || row.alert_key;
    const body = describeRecovery(row);
    if (send) {
      try {
        await send({
          branchId: null, eventType: 'system_alert', excludeUserId: 0,
          title: `✅ 복구: ${title}`, body, url: '/alerts',
        });
      } catch (e) {
        console.error('복구 알림 발송 실패:', e.message);
      }
      // 이력에도 남긴다 — /alerts 화면이 "알림이 나갔었나"를 되짚는 곳이라, 복구만 빠지면
      // 장애가 아직 진행 중인 것처럼 읽힌다. 키에 :resolved를 붙여 장애 알림과 구분한다.
      await db.run(
        'INSERT INTO system_alert_log (alert_key, title, body, value, sent_to) VALUES (?, ?, ?, ?, ?)',
        [`${row.alert_key}:resolved`, `✅ 복구: ${title}`, body, 0, 0]
      ).catch((e) => console.error('복구 알림 기록 실패:', e.message));
    }

    // 연동 오류 로그에도 남긴다(사용자 지시 2026-09-21).
    //
    // 알림을 못 본 사람도, 알림을 끈 사람도 /integration-errors는 본다 — 거기에 실패만
    // 쌓이고 복구가 없으면 "아직 고장 나 있나?"를 알 수 없다. 마지막 줄이 실패인 채로
    // 남아 있으면 다 끝난 장애를 진행 중으로 읽는다.
    //
    // error_code에 RECOVERED를 박아 화면이 실패와 구분해 그릴 수 있게 한다.
    const where = recoveryTarget(row.alert_key);
    await logIntegrationError({
      source: where.source, operation: where.operation,
      errorCode: 'RECOVERED', message: `복구 — ${body}`,
      context: { alertKey: row.alert_key, title },
    }).catch(() => {});
    await db.run('DELETE FROM system_alert_state WHERE alert_key = ?', [row.alert_key]).catch(() => {});
  }
  return stale.map((r) => r.alert_key);
}

// 알림 키에서 "어느 연동의 무슨 동작"인지 되짚는다. 연동 오류 로그가 source/operation으로
// 묶여 있어서, 복구 줄도 같은 자리에 서야 그 오류 바로 아래에 붙어 읽힌다.
//   error:callmaner:order_info → callmaner / order_info
//   stuck:kakao:send           → kakao / send
//   sync:backlog 등            → 연동이 아니라 우리 쪽 상태라 system으로 묶는다
function recoveryTarget(alertKey) {
  const m = /^(?:error|stuck):([^:]+):(.+)$/.exec(String(alertKey || ''));
  if (m) return { source: m[1], operation: m[2] };
  return { source: 'system', operation: String(alertKey || 'alert').replace(/^sync:/, 'sync_') };
}

// 복구 문구 — "무엇이 언제부터 몇 번"을 담는다. 이게 없으면 "복구됐습니다"만 와서,
// 받은 사람이 어느 장애 이야기인지 알 수 없다.
function describeRecovery(row) {
  const parts = ['더 이상 감지되지 않습니다.'];
  const count = Number(row.send_count) || 0;
  if (count > 0) parts.push(`알림 ${count}회`);
  if (row.last_sent_at) {
    const at = new Date(row.last_sent_at);
    if (!Number.isNaN(at.getTime())) {
      const mins = Math.max(0, Math.round((Date.now() - at.getTime()) / 60000));
      parts.push(`마지막 알림 ${mins}분 전`);
    }
  }
  return parts.join(' · ');
}
...
async function runChecks(options = {}) {
  ...
  const resolved = await clearResolved(alerts.map((a) => a.key), send).catch(() => []);
...
module.exports = {
  ...
  clearResolved, describeRecovery, recoveryTarget,
  checkStuckErrors,
