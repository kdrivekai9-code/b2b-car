...
async function checkSyncBacklog(cfg, syncLimit) {
  const row = await db.get(
    `SELECT COUNT(*) AS c
       FROM orders
      WHERE callmaner_conf_slip IS NOT NULL
...
async function runChecks(options = {}) {
  ...
  const syncLimit = Number(options.syncLimit || process.env.CALLMANER_SYNC_ORDER_LIMIT || 200);
  ...
  return { checked: alerts.length, sent, skipped, resolved, config: cfg, syncLimit };
}

module.exports = {
  runChecks, checkErrorSpikes, checkSyncBacklog, checkSyncStalled,
