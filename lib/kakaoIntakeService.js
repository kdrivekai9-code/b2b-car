...
async function insertOrder({ account, parsed, vehicle, reservation, geo, sessionId, split, sourceChannel }) {
  ...
  const created = await createOrder({
    ...
    //
    // 이게 빠져 있어서 상담톡으로 "즉시" 접수한 건이 **지금 시각 예약**으로 저장됐다 —
    // resolveReservation이 즉시 요청에도 date/time을 채우기 때문에(예약일시 칸을 비워둘 수
    // 없다) 저장된 값만 봐서는 즉시였는지 알 길이 없었다. 실측(2026-09-14): 상담톡 건
    // OID2218의 기준이 비어 있는 반면 같은 날 웹 건 OID2217은 immediate로 남아 있었다.
    //
    // **나뉜 건에는 붙이지 않는다** — 구간 릴레이는 각 구간 출발 일시가 앞 구간이 끝나는
    // 시각에 맞춰 따로 정해지므로 전부 예약이 맞다(routes/orders.js의 같은 판단과 같은 이유).
    // 여기서는 split이 나뉜 경우에만 값을 갖는다(호출부가 splitGroupId 유무로 넘긴다).
    reservationBasis: reservation.immediate && !split ? 'immediate' : null,
