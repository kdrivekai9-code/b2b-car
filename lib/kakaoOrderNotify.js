...
const db = require('../db');
// 통보 종류·기본 문구는 DB를 안 건드리는 모듈에 있다 — Next 페이지가 그쪽을 직접
// import할 수 있어야 하기 때문이다(lib/notifyEvents.js 머리말 참고).
const { DEFAULT_EVENT_SETTINGS, TEMPLATE_VARIABLES, EVENT_TYPES } = require('./notifyEvents');
...
// 알아낼 수 없으므로 환경변수로 둔다 — 기본값은 lib/publicUrl.js 한 곳에만 있다
// (예전에는 세 곳에 흩어져 있었고 그중 하나가 우리 도메인이 아니었다).

// 실시간 위치 링크 — 운행 중일 때만 준다. 완료된 운행은 위치를 더는 수집하지 않아 열어도
// 빈 화면이고, 그런 링크를 보내면 문의만 늘어난다.
function driverLocationLink(order) {
  const driverLocation = require('./driverLocation');
  if (!driverLocation.TRACKABLE_STATUSES.has(String((order && order.status) || ''))) return '';
  return driverLocation.trackingLink(order);
}
...
function renderTemplate(template, order, context = {}) {
  ...
  const values = {
    ...
    // 기사 현재 위치 — 콜마너 MCP에서 오므로 발송 직전에 호출부가 채운다(sendDue).
    // 조회에 실패하거나 배차 전이면 빈 값이라 그 줄이 통째로 사라진다: 통보 자체가 막히면
    // 안 되고, "위치 확인 불가" 같은 줄이 나가면 안 보내느니만 못하다.
    driver_place: String(ctx.driverPlace || '').trim(),
    driver_eta: ctx.driverEtaMinutes ? `약 ${ctx.driverEtaMinutes}분` : '',
    driver_location_link: driverLocationLink(order),
    // {photos}는 텍스트가 아니라 스위치다(buildMessage가 읽는다) — 문구에서는 지운다.
    photos: '',
  };
...
// 문구가 위치 변수를 쓸 때만 콜마너에 물어본다.
//
// 왜 조건부인가: 통보 크론은 1분마다 돌고 한 번에 최대 50건을 보낸다. MCP 왕복이 1초 안팎이라
// 무조건 부르면 발송이 50초씩 늘어 다음 주기를 침범한다. 위치를 안 쓰는 지사가 손해를 볼
// 이유가 없다.
//
// 실패하면 빈 값을 돌려준다 — 그 줄만 사라지고 통보는 정상 발송된다. 위치는 부가정보지 본문이
// 아니다(고객이 기다리는 것은 "배차됐다"는 사실이다).
async function loadLocationContext(setting, order) {
  const template = String((setting && setting.template) || '');
  if (!/\{driver_place\}|\{driver_eta\}/.test(template)) return {};
  try {
    const driverLocation = require('./driverLocation');
    const loc = await driverLocation.loadForOrder(order);
    if (!loc.available) return {};
    // 좌표가 오래됐으면 "지금 어디"라고 말하지 않는다 — 감추면 고객이 엉뚱한 곳에서 기다린다.
    if (loc.stale) return { driverEtaMinutes: null, driverPlace: '' };
    const region = await lookupDriverRegion(loc.lat, loc.lon);
    return { driverPlace: region || '', driverEtaMinutes: loc.etaMinutes || null };
  } catch (e) {
    console.error('통보용 기사 위치 조회 실패(통보는 그대로 발송):', e.message);
    return {};
  }
}

// 좌표를 그대로 문구에 넣으면 고객이 읽을 수 없다 — 행정구역으로 바꾼다(챗봇과 같은 방식).
async function lookupDriverRegion(lat, lon) {
  try {
    const kakaoRegion = require('./kakaoRegion');
    const r = await kakaoRegion.lookupRegion(lat, lon);
    if (!r) return '';
    return [r.sido, r.sigugun, r.dong].filter(Boolean).join(' ');
  } catch (e) {
    return '';
  }
}
...
async function loadNotifiableOrder(orderId) {
  ...
  // 접수 폼으로 넣은 건은 세션이 없다 — 그래서 통보를 하나도 못 받고 있었다.
  //
  // 실측(2026-09-08): 오더 49건 중 세션이 붙은 것은 10건뿐이고(카카오 8, 웹챗 2), 나머지
  // 39건은 배차·완료·취소 무엇도 안내되지 않았다. 법인 고객이 화면에서 접수하는 대부분이
  // 여기 해당한다.
  return notifyTargetForRequester(order);
}

// 접수한 고객의 웹 상담창을 통보 통로로 쓴다.
//
// **orders.chat_session_id는 건드리지 않는다.** 그 칸은 "이 오더가 어느 대화에서 접수됐나"라는
// 뜻이고, 다른 코드가 그렇게 읽는다(routes/kakaoConsult.js가 그 값으로 사진 요청의 대상 오더를
// 찾고, routes/chat.js가 세션의 지사를 유도한다). 통보 통로를 거기 적으면 두 뜻이 섞인다.
// 통보 행에는 이미 chat_session_id 칸이 있어서(kakao_order_notifications) 거기 남으면 충분하다.
//
// 카카오로는 보내지 않는다. 상담톡 발신은 그 사람의 카카오 사용자 키가 있어야 하는데, 접수
// 폼에는 그런 것이 없다. 법인에 걸린 채널 매핑으로 보내면 **다른 사람에게 갈 수 있다** —
// 그건 통보를 안 보내는 것보다 나쁘다.
async function notifyTargetForRequester(order) {
  if (!order.created_by) return null;
  // 직원이 대신 넣은 건을 직원 상담창에 꽂지 않는다. 고객 계정이 접수한 것만 통보한다.
  ...
  // 이미 쓰던 상담창이 있으면 그쪽에 모은다 — 통보마다 새 창이 생기면 고객은 어디를 봐야
  // 할지 모르고, 세션 목록도 통보용 껍데기로 채워진다.
  ...
  if (!session) {
    const inserted = await db.run(
      "INSERT INTO chat_sessions (user_id, status) VALUES (?, 'bot') RETURNING id",
      [requester.id]
    ).catch((e) => {
      console.error('통보용 상담창 생성 실패:', e.message);
      return null;
    });
    if (!inserted) return null;
    session = await db.get('SELECT * FROM chat_sessions WHERE id = ?', [Number(inserted.lastInsertRowid)]);
    if (!session) return null;
  }

  return { order, session, channel: 'web' };
}

// ---------------- 감지 ----------------

// 뒤늦게 알아낸 상태 변화는 고객에게 알리지 않는다.
//
// 왜(2026-09-08): 동기화가 오래 못 보던 진행중 오더를 다시 보게 되면서
// (routes/callmanerSync.js LIVE_LOOKBACK_DAYS) 오래전 사건이 오늘 처음 감지될 수 있다.
// 그대로 두면 "요청하신 탁송건이 운행완료 되었습니다"가 몇 주 뒤에 도착한다 — 고객은 이미
// 차를 받았고, 그 문구는 안내가 아니라 혼란이다.
//
// 무엇을 기준으로 재는가가 이 판단의 전부다.
//
// **예약일로 재면 틀린다.** OID1455는 예약이 8/25인데 실제 운행은 9/8에 일어났고 그날 14:46에
// 완료됐다. 예약일 기준으로 억제하면 6분 전에 끝난 진짜 완료를 "뒤늦은 것"으로 보고 안 보낸다.
// (그렇게 만들었다가 실측에서 발견해 바꿨다.)
//
// 그래서 **콜마너가 준 사건 시각**으로 잰다(orders.callmaner_status_at — OrderInfo의
// status_time / OrderHistory의 end_time). "언제 일어났나"와 "언제 알아냈나"가 갈리는 순간이
// 곧 뒤늦은 감지다. 몇 시간까지는 봐준다 — 폴링이 잠깐 멎었다고 진짜 사건을 안 보내면
// 그게 더 나쁘다.
//
// 사건 시각을 모르는 경우(마이그레이션 전 데이터, 사건 시각을 안 주는 경로)에는 예약일로
// 되돌아간다. 아무 판단도 안 하는 것보다는 낫고, 그 경로는 사라질 값이다.
// 어느 쪽이든 오더 이력에는 그대로 남는다 — 안 보내는 것은 고객 통보뿐이다.
const BACKFILL_GRACE_HOURS = Number(process.env.KAKAO_NOTIFY_BACKFILL_GRACE_HOURS || 6);
const BACKFILL_GRACE_DAYS = Number(process.env.KAKAO_NOTIFY_BACKFILL_GRACE_DAYS || 2);
...
function isBackfilled(order, detectedAt, opts = {}) {
  const graceHours = opts.graceHours != null ? opts.graceHours : BACKFILL_GRACE_HOURS;
  const graceDays = opts.graceDays != null ? opts.graceDays : BACKFILL_GRACE_DAYS;
  const detectedTs = parseKstText(detectedAt);

  const eventTs = parseKstText(order && order.callmaner_status_at);
  if (eventTs && detectedTs) {
    return (detectedTs - eventTs) > Math.max(0, Number(graceHours) || 0) * 3600 * 1000;
  }

  // 폴백 — 사건 시각을 모를 때만. 예약일에서 며칠 넘게 지나 감지된 것은 뒤늦은 것으로 본다.
  const base = String((order && order.reserved_date) || '').slice(0, 10);
  const detected = String(detectedAt || '').slice(0, 10);
  if (!/^\d{4}-\d{2}-\d{2}$/.test(base) || !/^\d{4}-\d{2}-\d{2}$/.test(detected)) return false;
  const days = Math.round((Date.parse(`${detected}T00:00:00Z`) - Date.parse(`${base}T00:00:00Z`)) / 86400000);
  return days > Math.max(0, Number(graceDays) || 0);
}
...
async function collectFromHistory() {
  ...
  const rows = await db.all(
    `SELECT id, order_id, old_status, new_status, created_at FROM order_status_history
  ...
  for (const row of rows) {
    ...
    // 한참 지난 일을 이제 알아낸 경우 — 커서는 넘기고 통보는 만들지 않는다.
    if (isBackfilled(target.order, row.created_at)) {
      backfilled += 1;
      console.log(`[통보] 뒤늦은 상태변화라 보내지 않는다: order ${target.order.oid} ${row.old_status} → ${row.new_status}`
        + ` (콜마너 ${target.order.callmaner_status_at || '시각없음'}, 예약 ${target.order.reserved_date}, 감지 ${row.created_at})`);
      continue;
    }
  ...
  return { scanned: rows.length, scheduled, disabled, backfilled, cursor: nextCursor };
}
...
async function sendDue(options = {}) {
  ...
  for (const notification of due) {
    ...
    const receipts = notification.event_type === RECEIPT_EVENT
      ...
      : [];
    // 기사 현재 위치는 오더 컬럼에 없다(콜마너 MCP에서 온다). 문구가 실제로 그 변수를 쓸 때만
    // 물어본다 — 안 쓰는 지사의 통보마다 MCP를 두드리면 발송이 그만큼 느려지고, 1분짜리 크론이
    // 다음 주기를 침범한다.
    const locationCtx = await loadLocationContext(setting, target.order);
    ...
    if (result && result.ok) {
      ...
      // 웹 통보에는 브라우저 알림을 함께 보낸다.
      //
      // 웹 통보는 상담창에 꽂히는데 창을 열지 않으면 도착한 줄을 모른다. 안읽음 배지를
      // 붙였지만 그것도 화면을 열어야 보인다 — 카카오는 앱 알림이 저절로 뜨는데 웹은
      // 그게 없었다(실사용 지적 2026-09-08). 카카오 채널은 알림톡이 이미 그 역할을 하므로
      // 중복해서 보내지 않는다.
      //
      // 실패해도 통보를 막지 않는다. 브라우저 알림은 덤이고 상담창 메시지가 본체다 —
      // 그래서 await하지 않고 결과도 보지 않는다(발송 대기줄이 여기서 느려지면 안 된다).
      if (target.channel === 'web' && target.order.created_by) {
        const firstLine = String(sentText || '').split('\n')[0].slice(0, 60);
        require('./push').notifyUser({
          userId: target.order.created_by,
          // 고객이 종류별로 껐다 켤 수 있다(push_subscriptions.event_types) — 어떤 사건인지
          // 넘겨야 그 선택이 적용된다. 안 넘기면 전부 보내던 예전 동작이 된다.
          eventType: notification.event_type,
          title: `[${target.order.oid || '오더'}] 알림`,
          body: firstLine,
          // 상담창으로 보낸다 — 통보 본문이 거기 있고, 배지도 그 화면에서 지워진다.
          url: '/orders/ai-intake',
        }).catch((e) => console.error('고객 브라우저 알림 실패(무시):', e.message));
      }
