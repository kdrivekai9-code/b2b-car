...
const DEFAULT_EVENT_SETTINGS = {
  ...
  // 유일하게 "상태가 바뀌어서"가 아니라 "바뀌지 않아서" 나가는 통보다. 그래서 delayMinutes의
  // 의미도 다르다 — 다른 사건은 "상태 변경 후 N분 뒤에 보낸다"지만 이건 "접수 후 N분이 지나도
  // 배차가 안 되면 보낸다"이다. 설정 화면이 사건마다 다른 라벨을 쓰는 이유다.
  not_dispatched: {
    label: '미배정 알림',
    enabled: true,
    delayMinutes: 30,
    template: '[{oid}] 배차가 지연되고 있습니다. 기사님을 계속 찾고 있으니 조금만 기다려주세요.',
  },
};

// 이 사건들은 상태 이력에서 찾는다. not_dispatched만 시간 경과로 찾아서 경로가 다르다.
const TRANSITION_EVENT_TYPES = ['dispatched', 'completed', 'dispatch_cancelled', 'cancelled'];

// 미배정으로 볼 상태들 — 접수됐지만 아직 기사가 붙지 않은 단계.
const UNASSIGNED_STATUSES = ['접수', '접수(배차중)', '대기', '대기(확인중)', '예약'];

// 미배정 알림을 찾을 때 며칠 치까지 볼지. 오래된 오더까지 훑어봐야 얻을 게 없고, 크론이 매분
// 도는데 스캔 범위가 계속 늘면 곤란하다.
const UNASSIGNED_LOOKBACK_DAYS = 3;
...
// 미배정 알림은 "상태가 바뀌지 않아서" 나가는 통보라 이력에서 찾을 수 없다 — 아무 일도 일어나지
// 않은 게 사건이다. 그래서 접수됐지만 아직 기사가 붙지 않은 오더를 시간으로 훑는다.
//
// 한 오더에 한 번만 보낸다(dedupe_key 없음). 계속 미배차라고 매분 알리면 그건 안내가 아니라
// 재촉이다. 배차가 됐다가 취소돼 다시 미배차가 되는 경우는 배차취소 통보가 이미 알린다.
async function collectUnassigned() {
  // created_at은 'YYYY-MM-DD HH24:MI:SS'(KST 벽시계) 문자열이라, 같은 벽시계 기준으로 비교한다.
  const candidates = await db.all(
    `SELECT o.id, o.branch_id, o.created_at, o.chat_session_id
     FROM orders o
     WHERE o.chat_session_id IS NOT NULL
       AND o.status = ANY(?)
       AND o.created_at::timestamp >= (now() at time zone 'Asia/Seoul') - (?::text || ' days')::interval
     ORDER BY o.id DESC LIMIT ${HISTORY_SCAN_LIMIT}`,
    [UNASSIGNED_STATUSES, String(UNASSIGNED_LOOKBACK_DAYS)]
  );

  let scheduled = 0;
  for (const candidate of candidates) {
    const setting = await loadEventSetting(candidate.branch_id, 'not_dispatched');
    if (!setting.enabled) continue;

    const dueAt = await db.get(
      `SELECT (?::timestamp + (?::text || ' minutes')::interval) <= (now() at time zone 'Asia/Seoul') AS due`,
      [candidate.created_at, String(Math.max(0, Number(setting.delayMinutes) || 0))]
    );
    if (!dueAt || !dueAt.due) continue;

    const target = await loadNotifiableOrder(candidate.id);
    if (!target) continue;

    const inserted = await db.run(
      `INSERT INTO kakao_order_notifications (order_id, chat_session_id, event_type, dedupe_key, scheduled_at)
       VALUES (?, ?, 'not_dispatched', '', now())
       ON CONFLICT (order_id, event_type, dedupe_key) DO NOTHING`,
      [target.order.id, target.session.id]
    );
    if (inserted && inserted.rowCount) scheduled += 1;
  }
...
const EXPECTED_STATUS_AT_SEND = {
  ...
  // 기다리는 사이에 배차가 됐다면 "아직 찾고 있습니다"는 보내지 않는다.
  not_dispatched: UNASSIGNED_STATUSES,
};
...
async function runKakaoOrderNotifications(options = {}) {
  try {
    ...
    const unassigned = await collectUnassigned();
    const delivered = await sendDue(options);
    return { collected, unassigned, delivered };
  } catch (e) {
