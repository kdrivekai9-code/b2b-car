...
async function sendKakaoPhotos(session, order, photos, options) {
  ...
  // 사진이 한 장도 못 나가면 남긴다. 예전에는 이 실패가 통보 행의 detail 문구
  // ("사진 5장 중 0장 발송")에만 남아서, 실제로 겪고도 아무도 몰랐다 —
  // 2026-08-13 OID1237이 그 경우다. 콜마너가 준 링크가 전부 404였는데(파일이 저장소에 없음)
  // 연동 오류에는 한 줄도 남지 않아, 사진 기능이 처음 발동한 사실조차 알아채지 못했다.
  //
  // 일부만 실패한 경우는 남기지 않는다 — 사진 몇 장이 빠지는 것은 링크 만료로도 생길 수 있고,
  // 그때마다 오류 화면이 채워지면 정작 전부 실패한 사고가 묻힌다.
  if (failed.length && !uploaded.length) {
    logIntegrationErrorAsync({
      source: 'callmaner',
      operation: 'notify_photos',
      refType: 'order',
      refId: order.id,
      message: `탁송사진 ${photos.length}장을 한 장도 보내지 못했습니다: ${failed[0] && failed[0].error}`,
      context: {
        oid: order.oid,
        confSlip: order.callmaner_conf_slip,
        // 링크가 왜 안 되는지는 이 값으로만 확인할 수 있다(콜마너 문의에 그대로 쓴다).
        sampleUrl: failed[0] && failed[0].photo && failed[0].photo.url,
