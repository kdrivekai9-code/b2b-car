...
// 그 세션에서 고객이 마지막으로 말한 시각. 봇/상담원/시스템 발화는 세지 않는다 — 우리가
// 방금 보낸 말 때문에 "대화 중"으로 보이면 안 된다.
async function lastCustomerMessage(sessionId) {
  const row = await db.get(
    `SELECT created_at FROM chat_messages WHERE session_id = ? AND sender = 'user'
...
async function sendDue(options = {}) {
  ...
  const due = await db.all(
    `SELECT * FROM kakao_order_notifications
     WHERE status = 'pending' AND scheduled_at <= now()
       ${only ? `AND order_id IN (${only.map(() => '?').join(',')})` : ''}
  ...
  for (const notification of due) {
    ...
    // 고객이 봇 질문에 답을 쓰는 중이면 끼어들지 않고 미룬다. 다만 "진행 중" 표시는 고객이
    // 답하지 않으면 스스로 사라지지 않으므로(확인 대기 mcp_pending_json 등), 마지막 고객
    // 발화 시각까지 함께 본다 — 안 그러면 몇 시간 전에 떠난 대화 때문에 배차 통보가 계속
    // 미뤄진다(OID1237 실측: 9시간 전 확인 대기가 남아 2분마다 defer).
    const lastCustomerMessageAt = await lastCustomerMessage(target.session.id);
    if (isSessionBusy(target.session, { lastCustomerMessageAt })
