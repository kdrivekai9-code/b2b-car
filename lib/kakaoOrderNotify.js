...
// 알아낼 수 없으므로 환경변수로 둔다.
// 웹 말풍선 안에 넣을 썸네일 수. 카카오는 버튼 하나로 전체를 여는 페이지를 주므로 이 값과
// 무관하다 — 웹에서만 말풍선이 너무 길어지지 않게 자른다.
  ...
  for (const notification of due) {
    ...
    try {
      if (target.channel === 'web') {
        ...
        // 카카오는 사진을 본문에 못 싣는다 — 이미지 메시지를 쓰려면 상담톡 업로드 API로 파일을
        // 먼저 올려야 하는데 그 서비스가 알파 서버에 없다(실측: /api/v1/upload/* 전부 404,
        // /send/plain은 422로 살아 있음). 게다가 탁송사진은 한 건에 13장이라 이미지로 다 붙일
        // 수도 없고, 평문은 앵커 텍스트를 지원하지 않아 "1, 2, 3…" 글자에 링크를 걸 수도 없다.
        //
        // 그래서 모아보기 페이지(/photos/:token)를 여는 버튼 하나를 붙인다. 링크가 없으면
        // (토큰 없는 옛 오더 등) 버튼 없이 본문만 나간다.
        const link = photos.length ? photoViewLink(target.order) : '';
        const button = link ? kakaoConsult.webLinkButton('사진 보기', link) : null;
        result = await send(target.session, text, button ? { buttons: [button] } : undefined);
        if (photos.length && !button) photoDetail = '사진 링크를 만들지 못해 버튼 없이 보냈습니다.';
      }
    } catch (e) {
