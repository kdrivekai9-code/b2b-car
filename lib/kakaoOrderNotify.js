  ...
  for (const notification of due) {
    ...
    try {
      if (target.channel === 'web') {
        ...
        //
        // 관리자가 문구에 {photo_link}를 직접 넣었으면 이미 들어가 있으므로 덧붙이지 않는다.
        const link = photos.length ? photoViewLink(target.order) : '';
        const alreadyInTemplate = /\{photo_link\}/.test(String(setting.template || ''));
        sentText = link && !alreadyInTemplate ? `${text}\n사진 보기: ${link}` : text;
        result = await send(target.session, sentText);
        if (photos.length && !link) photoDetail = '사진 링크를 만들지 못해 본문만 보냈습니다.';
      }
    } catch (e) {
