...
// 이보다 오래 하트비트가 없는 행은 죽은 연결로 보고 지운다.
//
// 왜 필요한가(실측 2026-08-30): 표에 1,546행이 쌓여 있는데 살아있는 것은 0건이었다
// (2026-07-25 ~ 08-29, 한 달치). 판정 자체는 틀리지 않는다 — 읽을 때 last_seen_at으로
// 거르기 때문이다. 문제는 **행이 영영 안 지워진다**는 것이다.
//
// stop()의 DELETE는 연결이 정상 종료될 때만 불린다. 서버리스에서는 인스턴스가 얼어붙거나
// 그대로 회수되는 일이 흔해서 그 경로가 통째로 건너뛰어진다. 배포·재시작도 마찬가지다.
// keep-alive 쓰기 실패로 잡는 이중 안전장치(routes/chat.js)도 그 인스턴스가 살아 있어야 돈다.
//
// 그래서 "지우는 쪽"이 아니라 "새로 들어오는 쪽"에서 청소한다. 접속할 때마다 한 번씩 훑으면
// 크론도 필요 없고, 아무도 안 쓰는 동안에는 표가 자라지도 않는다.
//
// 5분은 하트비트 주기(15초)의 20배다 — 잠깐 느린 연결을 살아있는데 지우는 일이 없다.
const PRESENCE_SWEEP_SECONDS = 300;
...
async function startAgentPresence(userId, userName) {
  ...
  // 죽은 연결 정리 — 새 연결이 들어올 때마다 한 번씩 훑는다(위 PRESENCE_SWEEP_SECONDS 주석).
  // 실패해도 접속 자체는 진행한다: 청소가 안 됐다고 상담원이 못 붙으면 그게 더 나쁘다.
  await db.run(
    `DELETE FROM chat_agent_presence WHERE last_seen_at < now() - interval '${PRESENCE_SWEEP_SECONDS} seconds'`
  ).catch((e) => console.error('죽은 상담원 접속행 정리 실패(무시):', e.message));
