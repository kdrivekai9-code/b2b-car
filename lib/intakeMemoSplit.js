...
// 기사 챗봇 전달사항에 붙이는 "적요1 전문" 표시.
//
// 왜 넣나(2026-09-07, 사용자 지시): 적요1은 100Byte라 뒤가 말없이 잘린다(실측 24.4%가 초과).
// 잘린 뒤에 무엇이 전달됐는지 되짚을 근거가 필요하다. 이 칸은 길이 제한이 없어서 전문이 남는다.
//
// 표시를 붙이는 이유는 다시 쓸 때 걷어내야 하기 때문이다. 상담원이 이 칸에 직접 쓴 말은
// 지우면 안 되고, 오더를 수정할 때마다 전문이 한 줄씩 쌓이면 그 칸이 그것만으로 찬다.
// 도착지 인도시각 줄을 기사메모에 넣을 때와 같은 방식이다(public/js/order-form.js).
const DRIVER_MEMO_COPY_LABEL = '[기사전달사항]';

// chatText에서 예전 전문 줄을 걷어내고 지금 값으로 다시 붙인다.
// driverMemo가 비면 붙이지 않고, 이미 있던 줄만 걷어낸다.
function withDriverMemoCopy(chatText, driverMemo) {
  const kept = String(chatText || '')
    .split(/\r?\n/)
    .filter((line) => line.trim().indexOf(DRIVER_MEMO_COPY_LABEL) !== 0);
  const body = String(driverMemo || '').trim();
  // 전문에 줄바꿈이 있으면 표시가 첫 줄에만 걸려 뒷줄이 다음 저장에서 안 걷힌다 — 한 줄로 만든다.
  const oneLine = body.replace(/\s*\r?\n\s*/g, ' ').trim();
  if (oneLine) kept.push(`${DRIVER_MEMO_COPY_LABEL} ${oneLine}`);
  return kept.join('\n').replace(/^\n+|\n+$/g, '').trim() || null;
}

// 사본 줄만 걷어낸 나머지. 기사 화면이 쓴다 — 그 화면은 "고객 요청사항"으로 전문을 이미
// 보여주므로, 사본을 그대로 내려보내면 한 화면에 같은 글이 두 번 뜬다.
function stripDriverMemoCopy(chatText) {
...
async function splitIntakeMemo(parsed, options = {}) {
  ...
  // 판정 대상은 이번에 나눌 문장뿐이 아니다. 이미 업체 쪽으로 잘못 간 오더를 다시 나눌 때,
  // 그 문구가 memo_customer에는 없고 memo_billing에만 남아 있어서 판정이 false가 된다 —
  // 그러면 재저장해도 스스로 고쳐지지 않는다(실제로 OID2075가 그랬다).
  // postal_requested를 정할 때 두 칸을 함께 보는 것과 같은 이유다(lib/orderCreate.js).
  const postalSource = [memo, options.postalSource].filter(Boolean).join('\n');
