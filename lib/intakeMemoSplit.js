...
// 요약(적요1)은 100Byte라 더 짧게. "등기"라는 말이 기사에게는 더 분명하다.
const POSTAL_BRIEF_NOTE = '출발지로 등기발송';
...
async function splitIntakeMemo(parsed, options = {}) {
  ...
  if (isPostalRequested(memo)) {
    ...
    freeCompanyText = joinParts(
      String(freeCompanyText || '').split(' / ').filter((part) => !isPostalRequested(part))
    ) || '';
    if (!isPostalRequested(freeDriverText)) {
      freeDriverText = joinParts([freeDriverText, POSTAL_DRIVER_NOTE]) || POSTAL_DRIVER_NOTE;
    }
    // 요약에도 반드시 남긴다. 적요1은 기사 앱에 보이는 유일한 칸이라 여기서 빠지면 못 본다.
    if (!isPostalRequested(freeBriefText)) {
      freeBriefText = joinParts([freeBriefText, POSTAL_BRIEF_NOTE]) || POSTAL_BRIEF_NOTE;
    }
  }

  const driver = joinParts([...byOption.driver, freeDriverText]);
