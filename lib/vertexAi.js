  ...
  const res = await fetchWithTimeout(url, {
    ...
    body: JSON.stringify({
      ...
      generationConfig: {
        ...
        responseSchema,
        // 의도 분류·필드 추출은 responseSchema로 형태가 강제되는 단순 작업이라 모델의 "thinking"
        // (응답 전 내부 추론 토큰)이 필요 없다. gemini-2.5-flash는 thinking이 기본 ON이라 이 호출이
        // 매번 1~2초씩 늘어났다(카카오 챗봇 질의 응답 지연의 주원인) — thinkingBudget:0으로 끄면
        // 같은 결과를 훨씬 빨리 준다. (도구 사용 에이전트 generateWithTools는 추론이 필요해 그대로 둔다.)
