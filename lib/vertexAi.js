...
async function generateJson(systemInstruction, userText, responseSchema, options) {
  const projectId = process.env.GOOGLE_CLOUD_PROJECT_ID;
  const location = process.env.GOOGLE_CLOUD_LOCATION || 'asia-northeast3';
  const accessToken = await getAccessToken();
...
async function generateJsonWithImages(systemInstruction, userText, images, responseSchema, options) {
  const projectId = process.env.GOOGLE_CLOUD_PROJECT_ID;
  const location = process.env.GOOGLE_CLOUD_LOCATION || 'asia-northeast3';
  const accessToken = await getAccessToken();
  const url = `https://${location}-aiplatform.googleapis.com/v1/projects/${projectId}/locations/${location}/publishers/google/models/${CHAT_MODEL}:generateContent`;
...
async function generateWithTools({ systemInstruction, contents, tools, timeoutMs, thinking }) {
  const projectId = process.env.GOOGLE_CLOUD_PROJECT_ID;
  const location = process.env.GOOGLE_CLOUD_LOCATION || 'asia-northeast3';
  const accessToken = await getAccessToken();
  const url = `https://${location}-aiplatform.googleapis.com/v1/projects/${projectId}/locations/${location}/publishers/google/models/${CHAT_MODEL}:generateContent`;
