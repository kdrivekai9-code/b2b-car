...
// 모델 엔드포인트 주소. 리전판과 global판은 **호스트 모양이 다르다.**
//
//   리전   https://asia-northeast3-aiplatform.googleapis.com/v1/projects/{p}/locations/asia-northeast3/...
//   global https://aiplatform.googleapis.com/v1/projects/{p}/locations/global/...
//
// 이걸 모르고 GOOGLE_CLOUD_LOCATION=global만 넣으면 global-aiplatform.googleapis.com이라는
// 없는 호스트를 부르게 되고, 구글 프런트엔드가 JSON이 아니라 HTML 404를 돌려줘서 "응답에
// 텍스트가 없습니다"처럼 엉뚱한 오류로 보인다(실측 2026-09-14).
//
// 왜 global을 쓸 수 있게 해두나: gemini-2.5-flash는 이 프로젝트에 전용 쿼터가 없어 공유 용량을
// 쓰는데, 그 용량이 리전보다 global 쪽이 훨씬 크다. 429가 잦을 때 환경변수 하나로 옮겨볼 수
// 있어야 한다(실측: global 200 OK 988ms / asia-northeast3 200 OK 1292ms — 더 빠르기까지 했다).
//
// **기본값은 그대로 asia-northeast3다.** global로 옮기면 요청이 한국 밖에서 처리될 수 있어
// 데이터 처리 위치가 달라진다 — 그건 우리가 임의로 정할 일이 아니라서 기본값을 바꾸지 않았다.
// 실패한 건만 다른 리전으로 넘긴다(리전 페일오버).
//
// 왜 이렇게 하나: asia-northeast3는 429가 잦은데(실측 60회 중 12건이 429, 8건 최종 실패)
// global·us-central1은 같은 조건 60회에서 429가 **한 건도** 없었다. 그렇다고 전부 옮기면
// 모든 요청이 한국 밖에서 처리된다 — 본문에 고객 주소·연락처·차량번호가 들어가므로 그건
// 가볍게 정할 일이 아니다.
//
// 평상시에는 지정 리전에 머물고, 그 리전이 거절한 건만 폴백으로 넘긴다. 그러면 밖으로
// 나가는 것은 "안 그랬으면 실패했을 요청"뿐이다.
//
// 값이 없으면(기본) 폴백은 꺼져 있고 지금까지와 완전히 동일하게 동작한다.
function fallbackLocation() {
  return String(process.env.VERTEX_FALLBACK_LOCATION || '').trim();
}

function modelUrl(location, projectId, model, method) {
...
// ── 일시적 거절(429/503) 재시도 ──────────────────────────────────────────────
//
// 왜 필요한가(실측 2026-09-14): 429가 09-09부터 나기 시작해 그날 180건 중 90건(50%)이
// 429로 떨어졌다. 그런데 **우리가 한도를 넘긴 게 아니다** — 분당 최대 호출이 27회인데 우리
// 자체 한도는 분당 60회고, 차단 기록(AI_RATE_LIMITED)은 0건이다. 호출량이 오히려 줄어든
// 날(09-08 487건 0건 → 09-09 410건 43건) 429가 시작됐다.
//
// gemini-2.5-flash는 이 프로젝트 쿼터 목록에 항목 자체가 없고(리전 토큰 한도는 -1=미적용),
// 과금은 정상이다. 전용 쿼터 없이 공유 용량을 쓰는 상태라, 429는 "우리 한도 초과"가 아니라
// "그 순간 자리가 없다"는 뜻이다. 올릴 쿼터가 없으니 **다시 부르는 것**이 올바른 대응이다.
//
// 503도 같이 본다 — 일시 장애라 성격이 같다. 4xx 중 429 말고는 재시도하지 않는다(요청이
// 잘못된 것이라 다시 불러도 같은 답이 온다).
//
// 전체 예산을 둔 이유: 한 번 호출의 타임아웃이 25초라, 예산이 없으면 최악의 경우 75초를
// 기다리게 된다. 고객이 화면 앞에서 기다리는 자리라 그건 실패보다 나쁘다. 429는 즉시
// 돌아오므로(대기가 아니라 거절) 실제로는 예산에 걸릴 일이 드물다.
...
// 지수 백오프 + 전체 지터. 지터가 없으면 같은 순간에 밀린 요청들이 **같은 순간에** 다시
// 몰려와 두 번째도 함께 밀린다.
function retryDelayMs(attempt, retryAfterHeader, random = Math.random) {
  const after = Number(retryAfterHeader);
  if (Number.isFinite(after) && after > 0) return Math.min(after * 1000, RETRY_MAX_DELAY_MS);
  const ceiling = Math.min(RETRY_BASE_MS * (2 ** (attempt - 1)), RETRY_MAX_DELAY_MS);
  return Math.round(ceiling * random());
}

// 실패 응답을 한 번만 읽어 같은 형식의 에러로 만든다 — 메시지 모양은 예전 그대로다
// (ai_call_logs에 쌓인 과거 기록과 대조할 수 있어야 한다).
async function vertexError(res, label) {
  const errText = await res.text();
  return new Error(`${label} (${res.status}): ${errText}`);
}

async function callVertex(url, init, timeoutMs, label, options = {}) {
  const doFetch = options.doFetch || fetchWithTimeout;
  const waitFn = options.sleepFn || sleep;
  const random = options.random || Math.random;
  const startedAt = Date.now();

  // 폴백으로 넘어갈 주소. 아직 안 써봤으면 첫 거절에서 곧바로 그쪽으로 간다.
  let fallbackUrl = options.fallbackUrl || null;
  let target = url;

  for (let attempt = 1; ; attempt += 1) {
    const res = await doFetch(target, init, timeoutMs);
    if (res.ok) return res;
    if (!RETRY_STATUSES.has(res.status)) throw await vertexError(res, label);

    // 다른 리전은 **다른 공유 용량**이다 — 같은 곳이 밀린다고 기다릴 이유가 없으니 곧바로
    // 넘긴다. 기다렸다 같은 리전에 다시 부딪히는 것보다 이쪽이 훨씬 잘 듣는다(실측: 같은
    // 리전 재시도는 22~67%만 건졌는데, 폴백 리전은 60회에서 429가 한 건도 없었다).
    if (fallbackUrl) {
      console.warn(`[Vertex ${res.status}] ${options.op || label} — 폴백 리전으로 넘긴다`);
      target = fallbackUrl;
      fallbackUrl = null;
      continue;
    }

    if (attempt >= RETRY_MAX_ATTEMPTS) throw await vertexError(res, label);

    const wait = retryDelayMs(attempt, res.headers && res.headers.get && res.headers.get('retry-after'), random);
    if (Date.now() - startedAt + wait > RETRY_BUDGET_MS) throw await vertexError(res, label);

    // 재시도는 조용히 넘어가면 안 된다 — 성공으로 끝나면 ai_call_logs에는 성공 한 줄만 남아
    // 공유 용량이 얼마나 빡빡한지 볼 방법이 사라진다.
    console.warn(`[Vertex ${res.status}] ${options.op || label} — ${wait}ms 뒤 재시도 (${attempt}/${RETRY_MAX_ATTEMPTS - 1})`);
    await waitFn(wait);
  }
}
...
async function embedTextRaw(text, taskType) {
  ...
  const url = modelUrl(location, projectId, EMBEDDING_MODEL, 'predict');
  // 이 리전이 거절하면 넘길 곳(VERTEX_FALLBACK_LOCATION). 안 정했으면 null이라 예전 그대로다.
  const fbLoc = fallbackLocation();
  const fallbackUrl = fbLoc && fbLoc !== location ? modelUrl(fbLoc, projectId, EMBEDDING_MODEL, 'predict') : null;

  const res = await callVertex(url, {
    method: 'POST',
  ...
  }, 20000, 'Vertex AI 임베딩 요청 실패', { op: `embed_${taskType || 'unknown'}`, fallbackUrl });
...
async function generateJsonRaw(systemInstruction, userText, responseSchema, options) {
  ...
  const location = process.env.GOOGLE_CLOUD_LOCATION || 'asia-northeast3';
  const accessToken = await getAccessToken();
  const url = modelUrl(location, projectId, CHAT_MODEL, 'generateContent');
  // 이 리전이 거절하면 넘길 곳(VERTEX_FALLBACK_LOCATION). 안 정했으면 null이라 예전 그대로다.
  const fbLoc = fallbackLocation();
  const fallbackUrl = fbLoc && fbLoc !== location ? modelUrl(fbLoc, projectId, CHAT_MODEL, 'generateContent') : null;
  ...
  }, 25000, 'Vertex AI generateContent 요청 실패', { op: options && options.op, fallbackUrl });
...
async function generateJsonWithImagesRaw(systemInstruction, userText, images, responseSchema, options) {
  ...
  }, (options && options.timeoutMs) || 40000, 'Vertex AI generateContent(이미지) 요청 실패', { op: options && options.op, fallbackUrl });
...
async function generateWithToolsRaw({ systemInstruction, contents, tools, timeoutMs, thinking }) {
  ...
  }, timeoutMs || 25000, 'Vertex AI 도구호출 요청 실패', { op: 'tools', fallbackUrl });
...
module.exports = {
  embedText, generateJson, generateJsonWithImages, generateWithTools, EMBEDDING_DIMENSIONS,
  // 검사에서 직접 부른다(scripts/check-vertex-retry.js) — fetch를 주입해 429 재시도를
  // 네트워크 없이 확인한다. 재시도는 조용히 사라지기 쉬운 종류라 실제로 돌려봐야 한다.
  callVertex, retryDelayMs, RETRY_STATUSES, RETRY_MAX_ATTEMPTS, RETRY_BUDGET_MS,
  // global/리전 주소 모양이 다르다 — 검사가 직접 확인한다.
  modelUrl, fallbackLocation,
};
