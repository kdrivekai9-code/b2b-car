...
async function searchKnowledgeBase(queryText, { limit = 3, threshold = 0.6, sessionId = null } = {}) {
  ...
  const retried = await searchOnce(
    `${previousQuestion}\n${queryText}`, limit, threshold + CONTEXT_RETRY_THRESHOLD_BUMP
  );
  if (!retried.length) return retried;

  // 재검색 1위가 "직전 질문만으로도 나왔을 항목"이면 버린다.
  //
  // 왜: 두 문장을 붙이면 의미가 직전 질문 쪽으로 기운다. 새 질문에 맞는 지식이 아예 없을 때
  // 특히 그렇다 — 그러면 직전 질문의 답을 그대로 다시 내놓는다. 실사용 사고(2026-08-19):
  //   "지금탁송접수 하면 가장빠른시간은?"  → #39(주말·연휴 접수) 0.780
  //   "결재는 후불되 되나?"                → 최고 0.664로 임계값 미달(정상: 못 찾음)
  //   두 문장을 붙이면                      → #39가 0.767로 통과
  // 결제 질문에 "24시간 운영합니다"가 그대로 한 번 더 나갔다.
  //
  // 임계값을 올리는 것(+0.05)만으로는 못 막는다 — 실제로 0.05를 올려둔 상태에서 뚫렸다.
  // "직전 질문의 답인가"를 직접 확인하는 편이 정확하다.
  const previousTop = await searchOnce(previousQuestion, 1, threshold).catch(() => []);
  if (previousTop.length && String(previousTop[0].id) === String(retried[0].id)) return [];
