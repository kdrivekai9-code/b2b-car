...
async function buildFareSuggestion(raw, options) {
  ...
  let extracted = null;
  if (!route) {
    // 규칙으로 못 잡으면 LLM 추출을 한 번 더 시도한다(접수 문장에 요금 문의가 섞인 경우).
    try {
...
module.exports = { buildSuggestion, buildFareSuggestion, extractRoute, buildIntakeReply, toIntakeFields, FAQ_THRESHOLD };
