...
async function buildFareSuggestion(raw, options) {
  ...
  if (product === 'premium') {
    ...
    return {
      ...
      text: lines.join('\n'),
      intake: null,
      fare: { total: premium.fare, distanceKm: quote.distanceKm, from: quote.origin.address, to: quote.destination.address, orderType: 'premium' },
    };
  }

  if (product === 'daily_driver') {
    if (!daily.enabled) {
      return {
        kind: 'fare',
        text: [lines[0], '일일기사는 요금 정보가 등록되어 있지 않습니다.', '상담원에게 연결해 드릴까요?'].join('\n'),
        intake: null,
        fare: null,
        offerAgent: true,
      };
    }
    // 금액을 못 내는 대신 **등록된 표를 그대로** 알려주고 시간을 받는다.
    lines.push(`일일기사는 이용 시간 기준입니다 — ${formatHours(daily.baseHours)} ${formatWon(daily.baseFare)}부터`
      + (daily.extraPerHour ? ` (초과 시 시간당 ${formatWon(daily.extraPerHour)})` : '') + '.');
    lines.push('몇 시간 이용하실 예정인지 알려주시면 계산해 드립니다.');
    return { kind: 'fare', text: lines.join('\n'), intake: null, fare: null };
  }
  ...
  return {
    ...
    text: lines.join('\n'),
    intake: null,
    fare: { total: quote.total, distanceKm: quote.distanceKm, from: quote.origin.address, to: quote.destination.address, orderType: 'dispatch' },
    offerAgent,
  };
}
...
module.exports = { buildSuggestion, buildFareSuggestion, buildHoursSuggestion, buildFreeTextIntakeSuggestion, extractRoute, buildIntakeReply, toIntakeFields, isHoursQuestion, isFareQuestion, FAQ_THRESHOLD };
