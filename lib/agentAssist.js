...
async function buildFareSuggestion(raw, options) {
  ...
  //
  // 이 계산은 거리 기준 탁송 요금표(fare_rules / group_fare_rules)만 본다. 프리미엄대리는
  // 시간 기준(premium_fare_rules의 base_hours)이라 출발·도착만으로는 금액이 나오지 않는다 —
  // 거리를 시간으로 환산해 추정할 수도 있지만 그건 우리가 만든 숫자이고, 실제 청구와 어긋나면
  // 금액을 안 알려준 것보다 나쁘다. 그래서 지금은 탁송임을 밝히는 것까지만 한다.
  // 프리미엄 요금 구조를 함께 안내하는 것은 별도 작업이다(문구를 정해야 한다).
  const lines = [
    `${quote.origin.query} → ${quote.destination.query}`,
    `탁송 예상 요금은 ${formatWon(quote.total)}입니다. (약 ${quote.distanceKm.toFixed(1)}km)`,
  ];
