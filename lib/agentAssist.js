...
async function buildFreeTextIntakeSuggestion(raw, options) {
  ...
  // 되묻기 초안에도 지금까지 파악한 내용을 함께 보여준다 — 상담원이 무엇이 잡혔는지 알아야
  // 그대로 보낼지 고쳐 보낼지 판단할 수 있다.
  const known = [
    parsed.origin.address ? `출발 ${parsed.origin.address}` : null,
    parsed.destination.address ? `도착 ${parsed.destination.address}` : null,
    parsed.vehicles.length ? `차량 ${parsed.vehicles.map((v) => [v.type, v.plate].filter(Boolean).join(' ')).join(', ')}` : null,
    parsed.when && parsed.when.raw ? `일시 ${parsed.when.raw}` : null,
  ].filter(Boolean);
  const text = known.length
    ? `${known.map((k) => '· ' + k).join('\n')}\n${question}`
    : question;
  return { kind: 'intake', text, intake: toIntakeFields(parsed), missing: parsed.missing };
}
