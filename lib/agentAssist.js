...
function toIntakeFields(parsed) {
  ...
  return {
    ...
    // 예약 기준 라디오까지 넘긴다.
    //
    // resolveReservation은 예전부터 immediate를 돌려줬는데 여기서 버렸다. 그래서 "즉시"로
    // 접수한 건이 상담관리 접수 미니폼·Next 우측 접수창에서 픽업 기준으로 보였다 — 대화는
    // 즉시로 확인해놓고 폼은 특정 시각 예약이라, 그대로 저장하면 뜻이 바뀐다(2026-09-11 지적).
    // 즉시가 아니면 아무것도 안 실어서 폼 기본값(픽업 기준)에 그대로 둔다. 픽업/도착 판별은
    // 원문이 있어야 해서 여기서는 못 한다 — 그건 /ai-intake/parse가 한다(lib/reservationBasis.js).
    ...(reservation.immediate ? { reservation_basis: 'immediate' } : {}),
