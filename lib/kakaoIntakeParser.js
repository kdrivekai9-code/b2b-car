...
function describeKnownFields(parsed, addressPreview) {
  ...
  if (waypoint && waypoint.address) lines.push(`· 경유 ${show(waypoint.address, 'waypoint')}`);
...
function buildParsedFromClassified(classified, text) {
  ...
  const promoted = promoteMislabeledWaypoint(
    join(classified.destinationAddress, classified.destinationAddressDetail),
    join(classified.waypointAddress, null),
    text
  );
