...
function buildMissingQuestion(missing, parsed) {
  const labels = (missing || []).map((m) => MISSING_PROMPTS[m]).filter(Boolean);
  if (!labels.length) return null;
  const ask = `접수하려면 아래 항목이 더 필요합니다.\n· ${labels.join('\n· ')}\n알려주시면 바로 접수하겠습니다.`;
  const known = describeKnownFields(parsed);
  return known ? `${known}\n\n${ask}` : ask;
}

// 되묻기 앞에 붙일 "확인된 내용" 요약. 아직 아무것도 못 잡았으면 null이라 문구가 그대로 나간다.
function describeKnownFields(parsed) {
  if (!parsed) return null;
  const lines = [];
  if (parsed.origin && parsed.origin.address) lines.push(`· 출발 ${parsed.origin.address}`);
  if (parsed.destination && parsed.destination.address) lines.push(`· 도착 ${parsed.destination.address}`);
