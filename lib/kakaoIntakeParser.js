...
// 연도를 적어준 날짜 표기. "2026-07-27" / "2026.7.27" / "2026/07/27" / "2026년 7월 27일".
const EXPLICIT_YEAR_RE = /(\d{4})\s*(?:[-./]|년\s*)(\d{1,2})\s*(?:[-./]|월\s*)(\d{1,2})/;
...
//
// 연도가 폼에 안 적히므로(로그 전수에서 연도 표기 0건) 올해로 채우되, 그 결과가 **하루라도**
// 과거면 내년으로 넘긴다 — 12월 말에 "1/3" 같은 요청이 들어오는 경우를 위한 처리다.
// (여기 "60일 넘게 과거면"이라고 적혀 있었는데 코드에 그런 여유는 없다. 주석만 그랬다.)
//
// 이 규칙은 멀쩡한 날짜도 1년 뒤로 민다. 9월에 "07/27"이 들어오면 43일 과거라 2027-07-27이
// 된다 — 실제로 그렇게 접수돼 콜마너까지 등록된 건이 있다(OID2075, 2026-09-07).
//
// 그래도 넘기는 쪽을 택한 이유: 과거 날짜로 배차 오더를 만들면 조회에서 아예 안 잡히고
// 아무도 모른 채 지나간다. 1년 뒤로 밀린 것은 목록에서 표시되고(lib/reservationSanity.js
// 90일 규칙) 확인 문구에도 "연도를 안 적으셔서 내년으로 봤습니다"로 드러나므로 잡힌다
// (lib/intakeSummary.js). 틀리더라도 보이는 쪽으로 틀린다.
function parseFormDate(text) {
  ...
  // 연도를 적어준 표기는 그대로 쓴다 — 추정할 것이 없으니 넘길 일도 없다.
  // "2026년 7월 27일"도 여기서 받아야 한다. 아래 "월/일" 규칙에 먼저 걸리면 적어준 연도를
  // 버리고 올해로 계산한 뒤 내년으로 밀어버린다(연도를 적었는데 2027년이 되는 셈).
  const iso = text.match(EXPLICIT_YEAR_RE);
  if (iso) {
    const im = Number(iso[2]);
    const id = Number(iso[3]);
    // 범위를 벗어나면 날짜가 아니다(전화·번지 등). 아래 규칙에 기회를 넘긴다.
    if (im >= 1 && im <= 12 && id >= 1 && id <= 31) {
      return { date: `${iso[1]}-${String(im).padStart(2, '0')}-${String(id).padStart(2, '0')}`, rolled: false };
    }
  }
...
function maskExplicitDate(line) {
  return String(line).replace(new RegExp(EXPLICIT_YEAR_RE.source + '\\s*일?', 'g'), ' ');
}
...
function pickWhen(lines) {
  for (const raw of lines) {
    ...
    if (!line) continue;
    // 주소 힌트를 볼 때는 날짜 표기를 먼저 가린다. ADDRESS_HINT_RE의 \d+-\d+(번지)가
    // "2026-07-27"에 걸려서 일시 줄이 주소로 오인되고, 날짜와 시각을 **둘 다** 잃는다.
    // 가리는 건 연도가 붙은 형태만이라 실제 번지("922-1")는 그대로 남는다.
    if (ADDRESS_HINT_RE.test(maskExplicitDate(line)) && !IMMEDIATE_WORDING_RE.test(line)) continue;
...
function buildPremiumParsedFromClassified(classified, text, overrides) {
  ...
  // "즉시"는 값이 없는 상태가 아니라 고객이 고른 답이다.
  //
  // 예전에는 날짜·시각을 못 잡으면 무조건 immediate:true로 두면서 reserved_date는 null로
  // 남겼다. 그래서 요약에는 "· 일시 즉시"가 뜨는데 질문은 계속 "예약시간을 말씀해주세요?"가
  // 나갔고(모순), 고객이 "즉시"라고 답해도 Gemini가 날짜를 못 뽑으니 같은 상태로 되돌아와
  // 같은 질문이 무한히 반복됐다(실사용 2026-08-24).
  //
  // 이제 아무 말이 없으면 "아직 모름"이고, 고객이 즉시 계열로 답했거나(overrides.immediate)
  // 원문에 즉시 표현이 있을 때만 즉시로 본다. 없어→declined와 같은 방식이다.
  const immediate = !!ov.immediate || IMMEDIATE_WORDING_RE.test(String(text || ''));
  const when = (date || time)
    ? { immediate: false, date, time, dateRolled: false, raw: [date, time].filter(Boolean).join(' ') }
    : { immediate, date: null, time: null, dateRolled: false, raw: null };
  ...
  const values = {
    trip_type: tripType,
    // 즉시도 채워진 값이다 — 이게 없으면 예약시간 질문이 영원히 반복된다.
    reserved_date: date || time || (immediate ? '즉시' : null),
...
const FAILURE_MESSAGES = {
  ...
  // 도착지 좌표가 없으면 콜마너에 도착지 없는 오더가 등록된다(lib/kakaoIntakeService.js preflight
  // 주석 참고) — 그대로 접수하면 기사가 어디로 갈지 모른다. 고객이 스스로 고칠 수 있는 사유다.
  destination_geocode_failed: '도착지 주소를 확인할 수 없습니다. 더 정확한 주소로 다시 알려주세요.\n(예: 서울 강남구 테헤란로 1 / 강남역 5번출구)',
...
module.exports = {
  ...
  // 되묻기 답 한 줄에서 날짜·시각만 다시 읽는 데 쓴다(lib/reservationReask.js).
  parseFormDate, parseFormTime,
};
