...
// 번호판 — 지역명이 앞에 붙는 구형/영업용 번호판("경기35바5081")도 통째로 잡는다. 지역명을
// 빼고 뒷부분만 잡으면 남은 지역명 토큰이 바로 앞 토큰이 되어 **차종으로 오인**된다(실사용
// 로그 재생에서 type:"경기"로 잘못 들어갔다).
//
...
//
// ⚠ `■ 출발지` 꼴(2024-05 초기 폼)은 **일부러 받지 않는다.** 표기만 다른 게 아니라 폼 구조가
// 다르다 — `■ 키수령지`/`■ 키반납지`처럼 출발지·도착지 말고 제3의 장소가 따로 있고(키를 받는
// 곳과 차가 있는 곳이 도보 10분 거리로 떨어져 있다), 지금 슬롯 스키마에는 그걸 담을 칸이 없다.
// 마커만 추가해서 인식시켜 보면 키수령지 주소가 통째로 사라진 채 "성공"으로 등록되고 주소에도
// 담당자명·깨진 괄호가 섞여 들어간다(로그 재생으로 확인). 인식 실패로 두면 상담원에게 넘어가
// 사람이 처리하므로, 오등록보다 그쪽이 안전하다.
...
function parseKakaoIntake(rawText) {
  ...
  return {
    ...
    origin: { ...splitRoadAddress(originAddress), contact: originContact },
    destination: { ...splitRoadAddress(destAddress), contact: destContact },
