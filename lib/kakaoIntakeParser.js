...
function missingLabel(key) {
  const labels = shortLabelsFor([MISSING_TO_FIELD_ID[key] || key]);
...
function buildMissingQuestion(missing, parsed, addressPreview) {
  const labels = (missing || []).map(missingLabel).filter(Boolean);
  if (!labels.length) return null;

  // 항목 이름만으로는 어떤 형태로 답해야 할지 모르는 것들이 있다 — "차종 / 차량번호"를 한 줄에
  // 적어야 한다는 건 예시를 봐야 안다. 예시가 있는 항목만 뒤에 붙인다.
  const examples = (missing || [])
    .map((key) => exampleFor(MISSING_TO_FIELD_ID[key] || key))
    .filter(Boolean);
