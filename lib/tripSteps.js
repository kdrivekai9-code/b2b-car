...
//
// 오더 상태(orders.status)는 아직 건드리지 않는다. 지금은 배차 상태의 주인이 콜마너이고
// (배차·알림톡은 콜마너 그대로), 여기 기록은 "기사가 언제 무엇을 했나"로 따로 남는다 —
// 섞으면 콜마너 동기화가 우리 기록을 덮어쓴다.
//
// **다만 향후 이 단계 변경을 콜마너와 동기화할 예정이다**(사용자 확정 2026-09-08). 그래서
// 이 표는 "우리만 쓰는 기록"이 아니라 그 동기화의 원장이 될 값이다 — 단계·시각·대기시간을
// 그대로 보존한다(집계해서 버리지 않는다). 막힌 것은 우리 쪽이 아니라 전문이다: 콜마너에
// 상태를 **올리는** API를 아직 못 찾았다(docs/callmaner-external-api-notes.md "콜마너에
// 확인이 필요한 것" 4번). 그 답이 오면 여기 기록을 그대로 실어 보내면 된다.
