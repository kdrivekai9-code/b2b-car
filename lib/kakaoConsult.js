...
// 평문을 "본문 + 항목표"로 나눈다. 표로 만들 수 없으면 null — 호출부가 평문으로 보낸다.
//
// 왜 발송 경계에서 변환하나: 통보 문구는 관리자가 고칠 수 있고(지사·법인 설정), 조회 답변은
// 모델이 쓴다. 그 둘을 각각 표로 만들면 규칙이 두 곳으로 갈린다 — 나가는 글을 한 곳에서
// 훑어 "라벨: 값" 모양만 표로 올린다.
function splitForItemList(text) {
  ...
  lines.forEach((line) => {
    ...
    if (!trimmed) { body.push(''); return; }
    if (/^[-*·•]?\s*접수번호/.test(trimmed)) receiptLines += 1;

    // "라벨: 값" — 라벨은 6자 이내여야 표 제목이 될 수 있다. 값에 링크가 있으면 표에 넣지
    // 않는다(23자를 넘고, 표에서는 누를 수도 없다 — 링크는 버튼으로 나간다).
    const m = trimmed.match(/^[-*·•]?\s*([^:]{1,20})\s*:\s*(.+)$/);
    if (m) {
      const title = m[1].trim();
      const desc = m[2].trim();
      if (title.length <= ITEM_TITLE_MAX && desc.length <= ITEM_DESC_MAX && !/https?:\/\//.test(desc)) {
  ...
  // 오더가 여럿인 목록 답변("총 3건입니다")은 표로 만들지 않는다 — 표는 한 덩이라 어느 값이
  // 어느 오더 것인지 알 수 없게 된다.
  if (receiptLines > 1) return null;
  if (rows.length < ITEM_MIN || rows.length > ITEM_MAX) return null;

  return { body: body.join('\n').replace(/\n{3,}/g, '\n\n').trim(), rows };
}

function itemListBody(bodyText, rows, buttons) {
  const section = {
    type: 'item_list',
    data: String(bodyText || '').slice(0, MAX_TEXT_LENGTH),
    attachment: {
      item: {
        list: rows.slice(0, ITEM_MAX).map((r) => ({
  ...
  if (Array.isArray(buttons) && buttons.length) section.attachment.buttons = buttons;
  return { version: 1, chapters: [{ sections: [section] }] };
}
...
async function sendMessage(session, text, options) {
  ...
  if (asTable) {
    const rich = await postJson('/send/rich', session, itemListBody(asTable.body, asTable.rows, buttons));
    if (rich.ok) {
      ...
      if (!loggedRichAccepted) {
        ...
        logIntegrationErrorAsync({
          ...
          message: `항목표 발송이 받아들여졌다(행 ${asTable.rows.length}개). 화면이 평문으로 보이면 채널의 리치 메시지 설정을 확인해야 한다.`,
          context: { rows: asTable.rows.map((r) => r.title) },
        });
      }
      return rich;
    }
    ...
    logIntegrationErrorAsync({
      ...
      context: { rows: asTable.rows.length, bodyLength: (asTable.body || '').length },
    });
