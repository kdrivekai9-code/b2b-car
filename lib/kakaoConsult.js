...
// 일시적 거부는 잠깐 뒤 다시 보낸다.
//
// 왜(실사용 지적 2026-09-21, 기록으로 확인): "한동안 대화를 안 하다가 오랜만에 말을 걸면 첫
// 메시지에 아무 반응이 없고 두 번째부터 답이 온다." 세션 문제가 아니었다 — 세션은 정상으로
// 찾아 쓰고 봇도 답을 만들어 저장했는데, **고객에게 내보내는 발신만** 중계서버가 502로
// 거부했다. 우리 대화창에는 봇 답변이 남아 있어 관리자 화면은 멀쩡해 보인다.
//
// 실측(세션 735):
//   09-17  마지막 대화로부터 4,489분(3.1일) 만의 첫 발신이 502
//          → 4초 뒤 두 번째 발신은 성공
//   09-14  첫 질문에 대한 발신 두 번이 모두 502(고객은 아무것도 못 봄)
//          → 고객이 다시 물은 뒤부터는 정상
//
// 중계서버가 돌려준 문구가 "카카오 API 네트워크 오류"라 저쪽의 일시적 실패다. 한 번 보내고
// 포기하면 그 답은 영영 고객에게 닿지 않는다 — 다시 부르는 것 말고 우리가 할 수 있는 게 없다.
//
// **일시적인 것만 다시 보낸다.** 5xx와 네트워크·타임아웃이 대상이고, 4xx(인증·형식)는 다시
// 보내도 결과가 같아서 지연만 늘린다. 이미 닿았는데 502로 돌아온 경우에는 같은 말풍선이 두 번
// 갈 수 있다 — 아무것도 못 받는 쪽이 더 나쁘다고 보고 재시도를 택했다(횟수는 2회로 묶는다).
//
// 발신은 응답을 보낸 뒤(runAfterResponse)에 돌기 때문에 이 지연이 웹훅 응답을 늦추지 않는다.
const SEND_RETRY_DELAYS_MS = [600, 1800];

function isTransientSendFailure(status, verdictReason) {
  if (status >= 500) return true;
  // 중계서버는 200으로 받고 본문 code에 실패를 담아 보내기도 한다(judgeSendResponse).
  const m = /code=(\d{3})/.exec(String(verdictReason || ''));
...
async function postJson(path, session, body, options) {
  ...
  let last = { ok: false, error: `${path} 시도하지 못했습니다` };
  ...
  for (let attempt = 0; attempt <= SEND_RETRY_DELAYS_MS.length; attempt += 1) {
    ...
    try {
      ...
      if (!res.ok) {
        last = { ok: false, error: `${path} 실패 (${res.status}): ${raw.slice(0, 300)}` };
        transient = isTransientSendFailure(res.status, null);
      } else {
        let parsed = null;
        try { parsed = raw ? JSON.parse(raw) : null; } catch (e) { parsed = null; }
        const verdict = judgeSendResponse(parsed);
        if (verdict.ok) return { ok: true, response: parsed, attempts: attempt + 1 };
        last = { ok: false, error: `${path} 거부 (${verdict.reason}): ${raw.slice(0, 200)}` };
        transient = isTransientSendFailure(0, verdict.reason);
      }
    } catch (e) {
      // 네트워크 끊김·타임아웃 — 저쪽이 받았는지조차 알 수 없다. 일시적인 것으로 본다.
      last = { ok: false, error: e.message };
      transient = true;
    }
    if (!transient) break;
    if (attempt < SEND_RETRY_DELAYS_MS.length) {
      console.warn(`카카오 발신 일시 실패 — ${SEND_RETRY_DELAYS_MS[attempt]}ms 뒤 재시도(${attempt + 1}/${SEND_RETRY_DELAYS_MS.length}): ${last.error}`);
    }
  }
  return { ...last, attempts: SEND_RETRY_DELAYS_MS.length + 1 };
}
