...
function buildSummaryLines(intake, options = {}) {
  ...
  (data.waypoints || []).forEach((w) => {
    ...
    lines.push(`${bullet} 경유지: ${w.address}${w.contact ? ` (${w.contact})` : ''}`);
  ...
  if (labeled) {
    ...
    // 카카오는 한 줄에 구간을 묶는다 — 말풍선이 짧아야 읽힌다.
    const originText = joinAddress(origin.address, origin.detail);
    const destText = joinAddress(destination.address, destination.detail);
    if (originText) lines.push(`${bullet} ${originText} → ${destText || '(도착지 미기재)'}`);
  }
...
function fromParsed(parsed, reservation) {
  ...
  return {
    ...
    origin: { address: p.origin && p.origin.address, contact: p.origin && p.origin.contact },
    destination: { address: p.destination && p.destination.address, contact: p.destination && p.destination.contact },
