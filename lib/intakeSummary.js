...
// 주소 + 상세주소를 한 줄로. 상세주소가 이미 주소 안에 들어 있으면 다시 붙이지 않는다.
//
// 두 칸으로 나눠 저장하지만(orders.origin_address / _detail) 주소 칸에 상세까지 들어가 있는
// 행이 실제로 있다 — 사람이 주소창에 통째로 붙여넣거나, 챗봇이 상호명을 주소에 이어 붙인
// 경우다. 그대로 합치면 조회 답변에 같은 말이 두 번 나온다(실측 2026-09-08, OID2075:
// "서울 강서구 양천로53길 30 서서울모터리움 803호 KGM인증중고차 서서울모터리움 803호
// KGM인증중고차"). 고객에게 읽어주는 값이라 눈에 바로 띈다.
function joinAddress(address, detail) {
  const a = String(address || '').trim();
  const d = String(detail || '').trim();
  if (!d) return a;
  if (!a) return d;
  // 띄어쓰기·쉼표만 다른 경우도 같은 것으로 본다("양천로 53길 30, 서서울" vs "…30 서서울").
  const flat = (v) => v.replace(/[\s,]+/g, '');
  if (flat(a).includes(flat(d))) return a;
  return `${a} ${d}`;
}
...
function buildSummaryLines(intake, options = {}) {
  ...
  // 즉시 요청은 계산해 채운 구체 시각(예: "2026-08-12 21:43")을 보여주지 않는다 — 고객이 실제로
  // 밝힌 건 "즉시"인데 구체 시각까지 같이 보이면 마치 그 시각을 콕 집어 요청한 것처럼 읽힌다
  // (사용자 확정 규칙, 2026-08-13).
  const when = data.immediate ? '즉시' : [data.reservedDate, data.reservedTime].filter(Boolean).join(' ');
  // 연도를 우리가 추정했으면 그 사실을 함께 밝힌다.
  //
  // 고객은 "07/27"처럼 연도를 안 적는다(로그 전수에서 연도 표기 0건). 그 날짜가 올해 기준으로
  // 이미 지났으면 파서가 **내년으로 넘긴다**(lib/kakaoIntakeParser.js parseFormDate) — 연말에
  // "1/3"이 들어오는 경우를 위한 처리다. 그런데 확인 문구에는 결과만 "2027-07-27"로 찍혀서,
  // 읽는 사람은 그게 고객이 말한 값인지 우리가 민 값인지 알 수 없다. 실제로 그렇게 접수돼
  // 1년 뒤 예약으로 콜마너까지 등록된 건이 있다(OID2075, 2026-09-07).
  //
  // 막지는 않는다 — 몇 달 뒤 차량을 미리 잡는 일이 실제로 있다. 다만 "네"라고 답하기 전에
  // 보이게는 해야 한다.
  const rolledNote = !data.immediate && data.reservedYearGuessed ? ' (연도를 안 적으셔서 내년으로 봤습니다)' : '';
  if (when) lines.push(`${bullet} ${labeled ? '예약: ' : ''}${when}${rolledNote}`);
...
function fromParsed(parsed, reservation) {
  ...
  return {
    ...
    // 파서가 연도를 내년으로 민 경우(parseFormDate의 rolled). 즉시 요청은 날짜 자체를 오늘로
    // 되돌리므로(resolveReservation) 알릴 것이 없다.
    reservedYearGuessed: !!(p.when && p.when.dateRolled) && !(reservation && reservation.immediate),
    ...
    options: p.options || {},
    // 요청사항을 기사 몫/업체 몫으로 나눈 결과가 있으면 그걸 쓴다(lib/intakeMemoSplit.js).
    // 확인 카드에서 두 줄이 갈려 보여야 고객이 잘못 들어간 것을 그 자리에서 고칠 수 있다.
    // 나누기 전(되묻기 중)이거나 분류가 실패했으면 예전처럼 요청사항 전부를 기사 쪽에 둔다.
    memoCustomer: p.memoSplit ? (p.memoSplit.freeDriver || null) : (p.memo || null),
    memoBilling: p.memoSplit ? (p.memoSplit.freeCompany || null) : null,
  };
}

module.exports = { buildSummaryLines, buildSummaryText, describeOptions, fromParsed, joinAddress };
