...
test.describe('고객 화면', () => {
  ...
  test('오더 리스트 — 지사는 없고, 담당자가 요청 법인 자리에 온다', async ({ page }) => {
    await page.goto(`${BASE}/orders`, { waitUntil: 'domcontentloaded' });
  ...
  test('오더 리스트 — 담당자 칸에 회사명 없이 이름만 나온다', async ({ page }) => {
    await page.goto(`${BASE}/orders`, { waitUntil: 'domcontentloaded' });
  ...
  test('사진 전송리스트 — 목록이 열리고 사진보기·다운로드로 이어진다', async ({ page }) => {
  ...
  test('사진 전송리스트 메뉴가 사이드바에 있다', async ({ page }) => {
    await page.goto(`${BASE}/orders`, { waitUntil: 'domcontentloaded' });
